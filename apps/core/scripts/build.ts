...
import { existsSync, readdirSync, writeFileSync, cpSync } from "fs";
...
async function main() {
	...
	for (const [type, content] of Object.entries(individuals)) {
		...
		if (Object.keys(input).length) {
			...
			// gz3: moderned_characters为空时（当前武将包已全部是plain JS，不走vite单独编译），
			// 不能再用rollup编译（input为空rollup会直接报错），改成直接把文件复制过去。
			for (const file of plainFiles) {
				cpSync(join(root, type, file), join(root, "dist", type, file), { recursive: true });
			}
		}
	}
