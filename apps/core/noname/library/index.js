...
export class Library {
	...
	characterAppend = {};
	characterInitFilter = {};
	characterGuozhanFilter = ["mode_guozhan", "homebrew"];
	dynamicTranslate = {};
	cardPack = new Proxy(
		{},
		{
	...
	mode = {
		...
		guozhan: {
			...
			config: {
				...
				// gz3: "国战·三将"规则的总开关，本来是独立的一个模式(mode/gz3)，
				// 现在合并成国战自己的一个规则开关，跟"群雄割据"并列。
				// 开启后：不使用晋势力、不禁用势力、双势力武将不再需要选归属（这两条
				// 效果上跟开启"群雄割据"一样，所以这里代码里会强制视为割据，不需要
				// 玩家再额外勾选"群雄割据"）、每人多选1个武将进公共池、公共池按出牌
				// 反序选出第三个武将，从一开始就明置且不计入势力判定。
				sanjiang: {
					name: "三将",
					init: false,
					frequent: true,
					restart: true,
					intro: "开启后进入“国战·三将”规则：去掉晋势力和禁将，双势力武将不再需要选归属，每名角色额外拥有一个从公共池反序选出的第三武将（从一开始就明置，不计入势力判定）。",
				},
				banGroup: {
					name: "势力禁用",
					init: false,
