...
import { chooseThirdCharacter } from "./patch/content.js";

// gz3: 去晋势力/去禁将/去双势力这些是"武将池"层面的取舍（选哪些武将包、
// 禁用哪些武将），由玩家自己在"武将"设置页里配置，不需要在规则代码里写死。
// "三将"开关这里只负责它真正独有的规则：主副将不需要同势力（复用"群雄割据"
// 效果）+ 额外的第三武将选将流程。
...
export const start = async (event, trigger, player) => {
	...
	if (_status.connectMode) {
		...
		// gz3: "三将"规则里主副将不需要同势力，效果上等同于开启"群雄割据"，
		// 不需要玩家再额外去勾选一遍。
		if (get.config("separatism") || get.config("sanjiang")) {
			// @ts-expect-error 祖宗之法就是这么写的
			_status.separatism = true;
		}
	...
	// gz3: 第三个武将的选将流程，需要playerFirst来确定反序选将顺序，
	// 所以放在这里（playerFirst算出来之后、gameStart触发之前）。
	// 联机模式暂不支持，跳过。
	if (!_status.connectMode && get.config("sanjiang")) {
		await chooseThirdCharacter(playerFirst);
	}
