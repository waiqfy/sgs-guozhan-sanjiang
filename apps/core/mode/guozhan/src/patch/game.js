...
export class GameGuozhan extends Game {
	 ...
	 * 获取武将选择
	 *
	 * gz3: 原版会把候选池里"只出现1次"的势力踢出去，尽量凑出"至少2个同势力"
	 * 的候选（大概率是为了保证候选里总能凑出同势力的珠联璧合组合）。这个模式
	 * 不需要这个设计，势力分布完全随机即可——调用方在传入list前已经做过
	 * randomSort，这里直接顺序取num个即可。
	 *
	 * @param {string[]} list - 所有武将的数组（已随机排序，本函数会原地消耗）
