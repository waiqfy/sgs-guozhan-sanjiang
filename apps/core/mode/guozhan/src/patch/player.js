...
export class PlayerGuozhan extends lib.element.Player {
	...
	/**
	 * gz3: 判断某个技能该显示/触发哪张武将牌的头像特效（"main"/"vice"/"third"）。
	 * 官方原版的checkShow完全不认识name3——只按lib.character[name1/name2][3]这两个
	 * 静态数组做正向查找；任何"不在角色自带技能数组里、而是运行时动态addSkill给第三将"
	 * 的技能（比如换将换来的新武将技能）在name1/name2/name3三处都查不到，会一路落到
	 * 最后返回false，而上层trySkillAnimate()会把false兜底成"main"——导致第三将发动
	 * 技能特效时头像却显示成了主将。
	 * 这里在官方原版逻辑基础上加一层兜底：主将、副将都对不上，但玩家确实有第三将时，
	 * 直接判定为"third"，不再放任上层继续兜底成"main"。
	 * @param {string} skill
	 * @param {boolean} [showonly]
	 * @returns {"main" | "vice" | "third" | false}
	 */
	checkShow(skill, showonly) {
		...
		if (lib.character[name1] && (!showonly || unseen0)) {
			var skills1 = game.expandSkills(lib.character[name1][3].slice(0));
			if (skills1.includes(skill)) {
		...
		if (lib.character[name2] && (!showonly || unseen1)) {
			var skills2 = game.expandSkills(lib.character[name2][3].slice(0));
		...
		if (lib.character[name3] && !showonly) {
			// 第三将从被抽到那一刻起就是常亮的，不需要像主副将那样判断isUnseen/showCharacter，
			// 也不需要再单独按静态数组精确匹配——只要走到这里说明主副将都对不上，且玩家确实
			// 有第三将，就直接归到"third"，避免上层把false兜底成"main"。
			return "third";
		}
		return false;
	}
