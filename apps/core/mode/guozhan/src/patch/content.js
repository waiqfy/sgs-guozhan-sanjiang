...
export const chooseCharacterContent = async (event, _trigger, _player) => {
	...
	if (_status.brawl && _status.brawl.chooseCharacter) {
		...
		chooseList = game.getCharacterChoice(characterList, parseInt(get.config("choice_num")) || 7);
	}
	// gz3: 记下这批候选武将，选完主副将之后剩下没被选中的那些，会成为"三将"规则里
	// 这名玩家私下选1个放入公共池时的候选范围（而不是重新从剩余武将池里随机抽一批）。
	game.me._gz3Leftover = chooseList.slice(0);
	...
	game.me._gz3Leftover = game.me._gz3Leftover.filter(name => name != game.me.name1 && name != game.me.name2);
	...
	for (const player of game.players) {
		if (player != game.me) {
			const aiChooseList = game.getCharacterChoice(characterList, parseInt(get.config("choice_num")) || 7);
			event.ai?.(player, aiChooseList, characterList);
			// gz3: 同上，记下这名AI玩家选主副将时剩下没选中的候选武将
			player._gz3Leftover = aiChooseList.filter(name => name != player.name1 && name != player.name2);
		}
	}
	...
	function createChooseCharacterDialog() {
		...
		function filterButton(button) {
			// gz3: 去掉选将界面"珠联璧合"的绿框高亮和角色列表文字提示，用户不需要这个功能。
			// @ts-expect-error 祖宗之法就是这么写的
...
/**
 * gz3: "三将"规则的第三个武将选将流程（仅本地模式，联机模式暂不支持）。
 *
 * 规则：
 * 1. 每个玩家按座次顺序，从"自己选主副将时剩下没选中的候选武将"里私下选1个
 *    放入"公共武将池"（不是重新从全武将池里随机抽一批候选）。
 * 2. 公共武将池凑够玩家人数后，再从剩余的全武将池里随机补1个，凑成 N+1 个。
 * 3. 公开展示这 N+1 个武将。
 * 4. 按出牌顺序的反序依次选：最后行动的玩家先选，playerFirst最后选。
 * 5. 选完剩1个没人选，直接弃置。
 * 6. 选到的第三个武将从一开始就明置，不算势力归属，不触发"首次亮出/明置时"
 *    这类技能（因为不走showCharacter事件），但技能会被正常加入player身上，
 *    所以能在随后的"开局时"(gameStart)触发时机正常触发对应技能。
 *
 * @param {Player} playerFirst 本局第一个行动的玩家
 */
export const chooseThirdCharacter = async playerFirst => {
	 ...
	 * @returns {string[]}
	 */
	function pickCandidates(num) {
		pool.randomSort();
		return pool.slice(0, num);
	}

	// 第一步：每个玩家按座次顺序私下选1个，放入公共武将池。
	// 候选范围优先用这名玩家选主副将时自己剩下没选中的那几个（_gz3Leftover）；
	// 只有在这个玩家没有剩余候选（比如"再战"沿用了上局武将）时，才退回到
	// 从当前武将池里随机抽一批作为候选。
	/** @type {string[]} */
	const publicPool = [];
	for (const player of game.players) {
		// @ts-expect-error 祖宗之法就是这么写的
		let candidates = (player._gz3Leftover || []).filter(name => pool.includes(name));
		if (!candidates.length) {
			candidates = pickCandidates(parseInt(get.config("choice_num")) || 7);
		}
		// @ts-expect-error 祖宗之法就是这么写的
		delete player._gz3Leftover;
		if (!candidates.length) {
			break;
		}
		...
		pool.remove(picked);
		publicPool.push(picked);
	}

	// 第二步：从剩下的武将池里随机补1个，凑成N+1个
	if (pool.length) {
		pool.randomSort();
		const extra = pool[0];
		pool.remove(extra);
		publicPool.push(extra);
	}

	if (!publicPool.length) {
		return;
	}

	// 第三步：公开展示公共武将池
	game.log(`公共武将池：${publicPool.map(name => get.translation(name)).join("、")}`);
	...
	const pickOrder = order.slice().reverse();
	...
	for (const player of pickOrder) {
		...
		const result = await player
			...
			.forResult();
		const picked = result.links[0];
		publicPool.remove(picked);

		player.name3 = picked;
		player.node.avatar3g.setBackground(picked, "character");
		player.node.avatar3g.show();
		player.node.name3.innerHTML = get.slimName(picked);
		player.node.name3.show();
		game.log(player, "获得了第三个武将", "#b" + picked);
	...
	// 第五步：公共武将池里剩下的那个（如果有）直接弃置，不做任何处理
};
