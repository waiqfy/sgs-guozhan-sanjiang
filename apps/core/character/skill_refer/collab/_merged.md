# collab merged reference

## ol_chitu 名字:OL赤兔 势力:shu

## ol_le_menghuo 名字:OL乐孟获 势力:shu

## ol_niuma 名字:OL牛头马面 势力:shen

### oldianbu 名字:点簿
描述: 锁定技，你的首个回合开始时，你获得十三张牌。当你每个回合内首次打出一种“拘魂”组合后，你视为使用一张【无中生有】，且因此获得的牌不计入手牌上限。若为炸弹，刷新此技能。
```js
oldianbu: {
		audio: 2,
	}
...
### oljuhun 名字:拘魂
描述: 出牌阶段开始时，将你的手牌变为扑克牌（仅点数可见，随机赋予花色），直到此回合结束。你可根据以下组合打出扑克牌并执行效果：对子：视为使用一张基本牌（不计入次数）；三条：获得相邻角色各一张牌；炸弹：造成2点伤害；顺子：你将一名角色随机两张牌变为扑克牌。
```js
oljuhun: {
		audio: 5,
	}
...
## xiangxue_liushan 名字:刘禅 势力:shu

### xiangxue 名字:想学
描述: 锁定技，你每使用两张未使用过的牌名的牌，获得等量个“惘”标记并摸一张牌，然后若“惘”数为：6，交换上文中的两个数字；9，删除上文中的未；12，失去此技能。
```js
xiangxue: {
		audio: 5,
...
### zhiyong 名字:致用
描述: 每局游戏每项限一次，你可弃置对应数量张牌，视为使用一张普通【杀】：1.手牌数；2.2；3.“惘”数。若弃置牌数等于序号数，改为弃置一名其他角色对应数量张牌或摸等量张牌。
```js
zhiyong: {
		audio: 2,
		enable: "chooseToUse",
...
## haoxue_lvmeng 名字:吕蒙 势力:wu

### haoxue 名字:好学
描述: 锁定技，你于：出牌阶段外弃置牌时，失去1点体力；出牌阶段内获得牌时，回复1点体力。
```js
haoxue: {
		audio: 4,
...
### qiongshe 名字:穷涉
描述: 若以下值为质数，你可于对应时机亮出牌堆顶的八张牌，获得其中点数为质数的牌：手牌数，摸牌阶段开始时；因此获得牌数，出牌阶段；体力值，弃牌阶段结束时。
```js
qiongshe: {
		audio: 2,
		isPrime(num) {
...
## boxue_caochong 名字:曹冲 势力:wei

### boxue 名字:博学
描述: 每轮限一次，你可弃置所有弃牌堆中有的牌名的手牌，视为使用一张弃牌堆中没有且能指定目标的基本牌或普通锦囊牌。
```js
boxue: {
		audio: 2,
		enable: "chooseToUse",
...
### zhizhong 名字:置重
描述: 出牌阶段限一次，你可观看所有其他角色的手牌，选择并获得这些牌中点数和小于等于你手牌的牌，然后你弃置X张牌（X为本次获得牌与原手牌的点数和之差）。
```js
zhizhong: {
		audio: 3,
...
## natu_guanyu 名字:那兔关羽 势力:shu

### natuyi 名字:义
描述: 出牌阶段限一次，你可以令至多两名其他角色交给你一张牌，然后你选择一项：1.将这些牌当做无距离次数限制的任意【杀】对其中一名角色使用。此【杀】结算中，其不能使用与此【杀】转化前的牌相同颜色的手牌；2.将这些牌分配给未成为目标的角色，其使用因此获得的黑色牌指定其他角色为目标后，目标本回合非锁定技失效。
```js
natuyi: {
		audio: 2,
...
## natu_zhugeliang 名字:那兔诸葛亮 势力:shu

### natuxing 名字:兴
描述: 游戏开始时，你获得牌堆中所有牌名的能够指定目标的锦囊牌各一张，称为“兴”（不可使用或弃置且不计入手牌上限）。每回合限X次（X为你本回合使用伤害牌数），你造成伤害后，随机令一张“兴”可使用。若你的手牌中没有不可使用的“兴”，你使用牌无距离次数限制且不可被响应。
```js
natuxing: {
		audio: 2,
		locked: false,
...
### natufu 名字:抚
描述: `你使用锦囊牌后，可发动此技能。若此牌：1.与你手牌中的“兴”牌名均不同，你令体力值唯一最小的角色回复1点体力；2.本局游戏仅你使用过此牌名的牌，你对一名角色造成1点火属性伤害。${get.poptip("rule_chengshi")}：本次执行效果中所有数字+1。`
```js
natufu: {
		audio: 2,
...
## natu_zhaoyun 名字:那兔赵云 势力:shu

### natulie 名字:烈
描述: 你使用基本牌后，你可将手牌数调整至X张（X为本回合本技能发动次数+1且至多为5），若你因此弃置了牌，你可对一名角色造成1点伤害。
```js
natulie: {
		audio: 2,
		trigger: { player: "useCardAfter" },
...
### natuheng 名字:恒
描述: 每轮限一次，若你的手牌中没有：1.非伤害牌，你可视为使用或打出一张【闪】；2.伤害牌，你可视为使用或打出一张无次数限制的任意【杀】。
```js
natuheng: {
		audio: 2,
...
## ol_le_caopi 名字:OL乐曹丕 势力:wei

### olweidai 名字:魏代
描述: 锁定技，当魏势力角色对你使用牌后，你加1点体力上限，然后若你的体力上限为全场最大，你将体力上限调整至体力值并摸减少体力上限张牌；若摸牌数大于等于存活角色数，你令一名角色变更势力为魏。
```js
olweidai: {
		audio: 2,
		forced: true,
...
### olliangzi 名字:梁姿
描述: 锁定技，你的装备牌只能当【酒】使用。你每回合使用前X张【酒】无次数限制（X为魏势力角色数）。
```js
olliangzi: {
		audio: 2,
		forced: true,
...
### olfuyou 名字:福佑
描述: 锁定技。①你使用红色普通锦囊牌额外结算一次。②每回合限一次，你使用红色普通锦囊牌造成伤害后，你摸一张牌。
```js
olfuyou: {
		trigger: {
			player: "useCard",
...
## ol_le_zhugeliang 名字:乐诸葛亮 势力:shu

### oljiangwu 名字:讲武
描述: 锁定技，首轮开始时，你选择三个价值不同的“战法”获得。若如此做，接下来的每轮结束后，你进行一次消耗“虎符”的“战法”选择。每名角色的回合结束时，你获得1枚“虎符”。
```js
oljiangwu: {
		audio: 2,
		forced: true,
...
### olxinghan 名字:兴汉
描述: `限定技，出牌阶段，你可失去任意个战法，然后你购买的下等量个战法不消耗虎符，且你每以此法失去一个战法，依次获得以下一个专属战法：${get.poptip("zf_dongfeng")}、${get.poptip("zf_qiaoqi")}。`
```js
olxinghan: {
		audio: 2,
		limited: true,
...
## meituizhishen 名字:美腿之神 势力:shen

### dcshentui 名字:伸腿
描述: 出牌阶段限一次，你可令你的攻击范围+1并视为使用一张【杀】，你使用点数小于攻击范围的【杀】不限次数且不可响应，使用点数大于攻击范围的【杀】不限距离且伤害+1。
```js
dcshentui: {
		mod: {
...
### dcxurui 名字:蓄锐
描述: 锁定技，你的回合结束时，若你本回合未造成伤害，你摸攻击范围张牌（至多为5）。
```js
dcxurui: {
		trigger: {
			player: "phaseEnd",
...
## gongbaiwan 名字:宫百万 势力:qun

### dchaoshi 名字:好食
描述: 你造成或受到伤害后，你摸一张牌并将一张牌置于武将牌上，每回合结束时，若你武将牌上的牌点数总计大于等于100，你获得一个额外回合并获得这些牌。
```js
dchaoshi: {
		trigger: {
			global: ["phaseEnd"],
...
## nizhuanzhishen 名字:逆转之神 势力:shen

### dcfanzhuan 名字:反转
描述: 每轮限一次，每个回合开始时，你可以将你的体力值调整为已损失体力值并摸变化值张牌。
```js
dcfanzhuan: {
		audio: 1,
		trigger: {
...
### dcniyun 名字:逆运
描述: 锁定技，你的攻击范围，使用杀次数均+X（X为你已损失体力值）。
```js
dcniyun: {
		audio: 1,
		mod: {
...
## shuimianzhishen 名字:睡眠之神 势力:shen

### dckeshui 名字:瞌睡
描述: 你的回合限一次，你的每个阶段开始时你可跳过此阶段（准备与结束阶段除外），然后你使用牌堆顶一张牌，此牌结算后对随机一个目标额外结算一次，若此牌不可使用，你获得此牌并跳过本回合你的下个阶段且可再次发动此技能。
```js
dckeshui: {
		trigger: {
...
## bianhuanzhishen 名字:变幻之神 势力:shen

### dcbaibian 名字:百变
描述: 你的回合开始或受到伤害后，随机获得一个喵喵杀中的武将技能直到你的回合结束。
```js
dcbaibian: {
		trigger: {
...
## juezezhishen 名字:抉择之神 势力:shen

### dchuibian 名字:慧辨
描述: 出牌阶段，你可选择两名角色各一张手牌并猜测这两张牌颜色是否相同，若你猜对，你获得并展示这两张牌；若你猜错，你弃置这两张牌且此技能失效至本回合结束。
```js
dchuibian: {
		audio: 2,
		enable: "phaseUse",
...
## weiquzhishen 名字:委屈之神 势力:shen

### dcweiqu 名字:委屈
描述: 每回合限两次，你成为实体牌的唯一目标时，可选择一项执行：1.弃置装备区所有牌并令此牌无效，2.弃置所有手牌并摸等量牌。（当你在一回合内执行过不同的两项结算完成时，你摸两张牌）。
```js
dcweiqu: {
		audio: 2,
		trigger: {
...
## keaizhishen 名字:可爱之神 势力:shen

### dcmaimeng 名字:卖萌
描述: 你的体力值变化后，你可选择一项执行：1.摸X张牌并防止你本回合下次受到的伤害；2.令一名其他角色交给你X张牌且其本回合使用的下一张牌无效（X为本回合此技能发动次数）。
```js
dcmaimeng: {
...
## tizhongzhishen 名字:体重之神 势力:shen

### dcgunyuan 名字:滚圆
描述: 锁定技，①你每轮首次进入濒死时将体力值回复至1点并增加1点体力上限（至多+3）；②你的手牌上限等于你的体力上限。
```js
dcgunyuan: {
		trigger: {
			player: "dying",
...
### dczuandai 名字:钻袋
描述: 你的出牌阶段开始时，你可以从牌堆或弃牌堆随机使用一张装备牌并摸一张牌，然后立即结束出牌阶段。
```js
dczuandai: {
		trigger: {
...
## mb_chitu 名字:手杀赤兔 势力:qun

### mbjunkui 名字:骏魁
描述: 锁定技，游戏开始时，你将所有坐骑牌移出游戏；你使用【杀】的次数上限+1。
```js
mbjunkui: {
		audio: 2,
		trigger: {
...
### mbchiyuan 名字:驰原
描述: 你每回合使用的第一张【杀】无距离限制且需要额外使用一张【闪】响应。出牌阶段限一次，你可摸X张牌（X为当前连续被使用的红色牌数）。
```js
mbchiyuan: {
...
## mb_jueying 名字:手杀绝影 势力:wei

### mbjiguan 名字:骥冠
描述: 锁定技，游戏开始时，你将所有坐骑牌移出游戏；你的手牌上限+2。
```js
mbjiguan: {
		audio: 2,
		audioname: ["mb_dilu"],
...
### mbzhengpeng 名字:征蓬
描述: 一名角色的回合结束后，你可选择一名符合条件的角色并失去X点体力（X为此前本轮本技能发动次数），其于本回合内每满足一项，你摸一张牌：受到过伤害，失去过装备牌，非当前回合角色且获得过牌。乘势：重置本技能的X，然后你获得弃牌堆中每种类型的牌各一张。
```js
mbzhengpeng: {
		judge(player) {
			let num = 0;
...
### mbyuetan 名字:跃檀
描述: 与你距离1以内的角色成为伤害牌的目标后，你可交给其一张牌，此牌结算结束后，若其未受到伤害，你摸一张牌。你每以此法失去两张牌，你回复1点体力。
```js
mbyuetan: {
...
## mb_muniu 名字:手杀木牛流马 势力:shu

### mbshezi 名字:摄梓
描述: 锁定技，准备阶段，你选择一名角色并选择其一个区域，若其此区域里有装备牌，你获得其此区域里的所有牌。
```js
mbshezi: {
		audio: 2,
		trigger: {
...
### mbyixing 名字:易型
描述: 出牌阶段限一次，你可将所有“器”置入弃牌堆并摸等量的牌，然后你可将任意张装备牌置于你的武将牌上，称为“器”。你拥有“器”的所有效果。
```js
mbyixing: {
...
## jm_yuanshu 名字:集蜜袁术 势力:qun

### mbjimi 名字:集蜜
描述: 锁定技，游戏开始时，所有角色将所有手牌替换为等量张【桃】或【酒】。当有【桃】或【酒】不因使用进入弃牌堆后，你获得一张字数为X的伤害牌(X为本回合【桃】、【酒】进入弃牌堆的张数)。
```js
mbjimi: {
		audio: 4,
		forced: true,
...
### mbmaodie 名字:冒迭
描述: 锁定技，你使用牌后，若造成伤害，你本回合下一次使用的伤害牌需大于此牌字数；每回合限两次，若未造成伤害，你获得一张目标角色的初始手牌。
```js
mbmaodie: {
		audio: 4,
		forced: true,
...
## tw_dm_quyi 名字:TW魔麴义 势力:qun

### retieji
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### dmchongqi 名字:冲骑
描述: `每回合限一次，你使用【杀】指定目标后，可以弃置目标角色一张牌。${get.poptip("rule_youji")}：此【杀】伤害+1。`
```js
dmchongqi: {
		audio: 2,
		trigger: {
...
### dmfanquan 名字:反拳
描述: `你受到伤害后，可以选择一名其他角色，令其受到1点伤害。${get.poptip("rule_boji")}：其受到X点伤害，本回合你计算与其他角色距离+1（X为你受到的伤害值）。`
```js
dmfanquan: {
		audio: 2,
		trigger: {
...
## ol_le_caohong 名字:OL乐曹洪 势力:wei

### olmojin 名字:摸金
描述: `锁定技，游戏开始时，你进行一次${get.poptip({
		id: "olmojin_tip",
		name: "摸金",
		type: "character",
...
### oldingbao 名字:定宝
描述: `限定技，出牌阶段，你可以直接完成一次${get.poptip("olmojin_tip")}并结束此阶段。`
```js
oldingbao: {
		audio: 2,
		enable: ["phaseUse"],
...
## ol_le_liushan 名字:OL乐刘禅 势力:shu

### oltuoquan 名字:托权
描述: `锁定技，游戏开始时，你令所有其他角色获得${get.poptip("oldianzan")}。准备阶段，你沉迷享乐，失去因〖托权〗获得的技能，然后从“${get.poptip({
		id: "tuoquan_fuchen",
		name: "季汉辅臣",
		type: "character",
...
### olxianglv 名字:相旅
描述: `锁定技，游戏开始时，你将牌堆中不同牌名的基本牌各一张置于武将牌上。你每上阵一名“${get.poptip("tuoquan_fuchen")}”，随机获得其中一张基本牌。`
```js
olxianglv: {
		audio: 2,
		trigger: {
...
### olanle 名字:安乐
描述: `锁定技，你受到伤害后，移去上阵的“${get.poptip("tuoquan_fuchen")}”，然后与当前回合角色各摸一张牌。若你没有上阵的“${get.poptip("tuoquan_fuchen")}”，你视为拥有${get.poptip("xiangle")}。`
```js
olanle: {
		audio: 2,
		trigger: { player: "damageEnd" },
...
## you_zhugeliang 名字:有诸葛亮 势力:shu

### dcyingyou 名字:应有
描述: `①你的回合开始时、结束阶段、受到伤害后可选择一项执行并摸一张牌：1.随机获得一个五虎将持有的技能；2.将${get.poptip("real_zhuge")}置入装备区；3.获得10吨馒头。②你使用手牌时，可消耗此牌点数吨馒头，令此牌额外结算一次。`
```js
dcyingyou: {
		trigger: {
...
## bigsb_dengai 名字:OL乐邓艾 势力:wei

### olandu 名字:暗度
描述: `锁定技，你使用一张“${get.poptip({
		id: "andu_yinping",
		name: "阴平",
		type: "character",
...
### olqiqi 名字:期期
描述: 每轮限一次，你使用牌指定目标时，若牌名字数不小于你的体力值，你可以摸两张牌并令此牌额外结算一次，然后此牌结算后进行判定：若结果为♥，你减少1点体力上限。
```js
olqiqi: {
		audio: 2,
		trigger: {
...
## wild_liru 名字:狂李儒 势力:qun

### olhuaquan 名字:花拳
描述: 锁定技，你使用牌指定其他角色后，为此牌秘密选择一个效果：1.重拳，造成的伤害+1；2.轻拳，使用后你摸一张牌。然后其他目标角色依次猜测你选择的效果。
```js
olhuaquan: {
		audio: 2,
		trigger: { player: "useCardToPlayered" },
...
### olsanou 名字:三殴
描述: 锁定技，其他角色受到你的伤害后或猜错〖花拳〗的效果后，你摸一张牌并令其获得一个「击倒」标记。一名角色获得至少3个「击倒」标记后移除此标记并进入“击倒”状态。“击倒”状态的角色始终跳过出牌阶段。角色于“击倒”状态时，有10张牌离开牌堆或进入弃牌堆后，其脱离“击倒”状态。
```js
olsanou: {
		audio: 2,
		marktext: "👊",
...
## ren_jiangwei 名字:忍姜维 势力:shu

### renhuoluan 名字:惑乱
描述: `出牌阶段限一次，你可以与至多两名其他角色${get.poptip("rule_gongtongpindian")}并记录你的拼点牌，你可将拼点牌更改为任意点数。点数唯一最小的角色视为被参与拼点的其余角色各使用一张【杀】，点数唯一最大的角色摸体力值张牌，若均不为你，你重置〖惑乱〗。`
```js
renhuoluan: {
		audio: 2,
		enable: "phaseUse",
...
### renguxing 名字:孤星
描述: 限定技，回合开始时，你可以依次执行前X项，从牌堆或弃牌堆中：1.获得〖惑乱〗记录的牌；2.获得与〖惑乱〗记录牌点数花色均相同的所有牌；3.获得与〖惑乱〗记录牌牌名相同的牌。（X为游戏轮数，相同牌名的牌至多获得五张）
```js
renguxing: {
		audio: 2,
		trigger: {
...
## ren_dengai 名字:忍邓艾 势力:wei

### renneyan 名字:讷言
描述: 转换技，锁定技，你使用非装备牌时，阳：弃置一张牌并令此牌额外结算一次，否则此牌无效；阴：此牌无次数限制。
```js
renneyan: {
		audio: 2,
		trigger: {
...
### renqianyao 名字:潜曜
描述: 限定技，回合开始时，你可以摸X张牌，视为使用一张【杀】并依次执行前X项：1.此【杀】额外指定一个目标；2.此【杀】伤害+1；3.此【杀】不可被响应。（X为游戏轮数）
```js
renqianyao: {
		audio: 2,
		trigger: {
...
## strong_caochong 名字:曹冲 势力:wei

### strongduanti 名字:锻体
描述: 锁定技，①你从牌堆摸牌后，受到1点无来源伤害。②当你发动〖称象〗时，若你的体力值为全场最低，必定亮出一张【桃】或【酒】；若你的体力值为全场最高，必定亮出一张伤害牌或武器牌。
```js
strongduanti: {
		audio: 2,
		trigger: {
...
### stronglianwu 名字:练武
描述: 你使用【杀】指定唯一目标或成为【杀】的唯一目标后，使用者每满足一项便可弃置目标角色的一张牌：1.装备了武器牌；2.此【杀】受到了【酒】的影响。
```js
stronglianwu: {
		audio: 2,
		trigger: {
...
## taipingsangong 名字:张角三兄弟 势力:qun

### oltiangong 名字:天公
描述: `锁定技，回合开始时，你视为使用一张${get.poptip("leigong")}；回合结束时，你视为使用一张${get.poptip("younan")}。一名角色判定后，若为黑桃，你对另一名角色造成1点雷电伤害。`
```js
oltiangong: {
		audio: 2,
		forced: true,
...
### oldigong 名字:地公
描述: 锁定技，你使用没有「地公」的牌时，若为伤害牌，此牌伤害+1，若不为伤害牌，结算后当前回合角色判定，若为红色，你摸一张牌。你获得牌时，标记为「地公」，本轮结束时移去此标记。
```js
oldigong: {
		init(player) {
...
### olrengong 名字:人公
描述: 锁定技，若你连续使用了两张类型不同的牌，此牌结算后，你弃置一张牌，然后从牌堆获得一张其余类型的牌。
```js
olrengong: {
		audio: 2,
		trigger: { player: "useCardAfter" },
...
## yuanshaoyuanshu 名字:烈袁绍袁术 势力:qun

### dclieti 名字:烈悌
描述: 锁定技，游戏开始时，你获得两组初始手牌并增加武将名称标记；当你获得手牌时，根据当前武将为手牌增加武将名标记。你只能使用或打出当前武将标记的手牌。非当前武将标记的手牌不计手牌上限。
```js
dclieti: {
		trigger: {
...
### dcshigong 名字:世公
描述: 锁定技，你每回合使用的第一张手牌不受〖烈悌〗限制，然后此牌结算后你切换为对应标记的武将。若你因此切换为袁绍，你视为使用一张【万箭齐发】；若你因此切换为袁术，你摸两张牌。
```js
dcshigong: {
		locked: true,
		direct: true,
...
### dcluankui 名字:乱魁
描述: 你每回合第二次造成伤害后，可以弃置一张「袁绍」牌令自己本回合下次造成的伤害翻倍；你每回合第二次获得牌后，可以弃置一张「袁术」牌令自己本回合下次摸牌翻倍。
```js
dcluankui: {
		trigger: {
			source: ["damageSource"],
...
### dcweiji 名字:围计
描述: 当你使用牌指定其他角色为目标后，你可以从1-3中选择一个数字，然后令一名其他目标角色从1-3中选择一个数字，若你与其选择的数字不同，则你摸你选择数字张牌。
```js
dcweiji: {
		audio: 2,
		trigger: { player: "useCardToPlayered" },
...
### dcsaima 名字:赛马
描述: 当你使用坐骑牌后，你可以选择一名其他角色，与其进行三次拼点，若你至少赢两次，则你对其造成1点伤害。
```js
dcsaima: {
		audio: 2,
		trigger: { player: "useCardAfter" },
...
## ol_xiahouen 名字:OL夏侯恩 势力:wei

### olyinfeng 名字:引锋
描述: `锁定技。①游戏开始时，你获得一张${get.poptip("chixueqingfeng")}。②当${get.poptip("chixueqingfeng")}因弃置进入弃牌堆后，你失去1点体力并获得之。③当你的手牌被其他角色获得后，若你的手牌中有${get.poptip("chixueqingfeng")}，你对其造成1点伤害；若其本次获得的牌中有${get.poptip("chixueqingfeng")}，其对你造成1点伤害。`
```js
olyinfeng: {
		audio: 2,
...
### olfulu 名字:负掳
描述: ①你使用【杀】结算后，你可以交给其中一名目标角色一张手牌，然后获得其至多两张手牌。②体力小于你的角色使用【杀】结算后，若此【杀】的目标包括你，其可以交给你一张手牌，然后获得你至多两张手牌。
```js
olfulu: {
		audio: 2,
		trigger: { global: "useCardAfter" },
...
## hanshiwuhu 名字:五虎 势力:wei

### oljuejue 名字:玨玨
描述: 锁定技，你本局游戏首次使用【杀】/【闪】/【桃】/【酒】时，此牌伤害或回复值+1且不计入次数限制，结算后，你获得此牌。
```js
oljuejue: {
		audio: 2,
		trigger: {
...
### olpimi 名字:披靡
描述: 你使用牌指定其他角色为唯一目标后，或成为其他角色使用牌的唯一目标后，你可以弃置使用者一张牌，令此牌伤害或回复值+1。然后若使用者的手牌最多或最少，你摸一张牌且此技能本回合失效。
```js
olpimi: {
		audio: 2,
		trigger: {
...
## shi_cenhun 名字:岑昏 势力:wu

### dcxinggong 名字:兴功
描述: 出牌阶段限一次，你可以选择本回合进入弃牌堆的任意张牌获得。然后若X大于0，则你受到X点伤害（X为你本次获得的牌数-你的体力值）。
```js
dcxinggong: {
		enable: "phaseUse",
...
### dcbaoshi 名字:暴食
描述: 摸牌阶段结束时，你可以亮出牌堆顶的两张牌。若亮出牌的牌名字数之和小于等于10（【桃】或【酒】不计入牌名字数统计），你选择一项: 1、获得所有亮出的牌; 2、再亮出一张。
```js
dcbaoshi: {
...
## ol_zishu 名字:子鼠 势力:qun

### olzishu 名字:子鼠
描述: 出牌阶段限一次，你可以获得手牌数大于你的其他角色一张手牌，然后你可以重复此流程直到你的手牌数为全场最多。
```js
olzishu: {
		audio: true,
		enable: "phaseUse",
...
## ol_chouniu 名字:丑牛 势力:qun

### olchouniu 名字:丑牛
描述: 锁定技，一名角色的结束阶段，若你的体力值为全场最低，则你回复1点体力。
```js
olchouniu: {
		audio: true,
...
## ol_yinhu 名字:寅虎 势力:qun

### olyinhu 名字:寅虎
描述: 出牌阶段每种类别限一次，你可以弃置一张牌，然后对一名其他角色造成1点伤害。若有角色因此进入濒死状态，则此技能于本回合失效。
```js
olyinhu: {
		audio: true,
		enable: "phaseUse",
...
## ol_maotu 名字:卯兔 势力:qun

### olmaotu 名字:卯兔
描述: 锁定技，一名角色的濒死状态结算完毕后，你不能成为体力值大于等于你的其他角色使用牌的目标直到下个回合开始。
```js
olmaotu: {
		audio: true,
...
## ol_chenlong 名字:辰龙 势力:qun

### olchenlong 名字:辰龙
描述: 出牌阶段限一次，你可以失去至多2点体力，对一名其他角色造成等量伤害。若你因此进入濒死状态，则你减1点体力上限。
```js
olchenlong: {
		audio: true,
		enable: "phaseUse",
...
## ol_sishe 名字:巳蛇 势力:qun

### olsishe 名字:巳蛇
描述: 当你受到伤害后，你可以对伤害来源造成等量伤害。
```js
olsishe: {
		audio: true,
		trigger: { player: "damageEnd" },
...
## ol_wuma 名字:午马 势力:qun

### olwuma 名字:午马
描述: 锁定技。①你不能被翻面。②你的阶段不会被跳过。③当你成为其他角色使用锦囊牌的目标后，你摸一张牌。
```js
olwuma: {
		audio: true,
...
## ol_weiyang 名字:未羊 势力:qun

### olweiyang 名字:未羊
描述: 出牌阶段限一次，你可以弃置任意张不同类型的牌，然后令至多等量角色回复1点体力。
```js
olweiyang: {
		audio: true,
		enable: "phaseUse",
...
## ol_shenhou 名字:申猴 势力:qun

### olshenhou 名字:申猴
描述: 当你成为【杀】的目标时，你可以进行判定，若结果为红色，则此【杀】对你无效。
```js
olshenhou: {
		audio: true,
		trigger: { target: "useCardToTargeted" },
...
## ol_youji 名字:酉鸡 势力:qun

### olyouji 名字:酉鸡
描述: 锁定技，摸牌阶段，你多摸X张牌（X为游戏轮数且至多为5）。
```js
olyouji: {
		audio: true,
...
## ol_xugou 名字:戌狗 势力:qun

### olxugou 名字:戌狗
描述: 锁定技。①红色【杀】对你无效。②你使用红色【杀】无距离限制且造成的伤害+1。
```js
olxugou: {
		mod: {
...
## ol_haizhu 名字:亥猪 势力:qun

### olhaizhu 名字:亥猪
描述: 锁定技。①其他角色的黑色牌因弃置而置入弃牌堆后，你获得这些牌。②准备阶段，若你的手牌数为全场最多，你失去1点体力。
```js
olhaizhu: {
		audio: true,
		trigger: {
...
### olshouhun 名字:兽魂
描述: 锁定技。①摸牌阶段你额外摸[0]张牌，你的手牌上限+[1]，你的体力上限+[2]。②当你受到伤害时，你令此技能数值最低的一项数值+1（每项数值至多为4）。
```js
olshouhun: {
		trigger: {
			global: "phaseBefore",
...
## ol_jsrg_lvbu 名字:吕布 势力:qun

### olfengzhu 名字:逢主
描述: 锁定技。准备阶段，你选择一名未记录的其他男性角色，记录其为“义父”。然后摸其体力值张牌。
```js
olfengzhu: {
		audio: 1,
...
### olyuyu 名字:郁郁
描述: 锁定技，回合结束时，你令一名“义父”角色获得1枚“恨”标记。你于其回合内受到1点伤害或失去一张牌后，其获得1枚“恨“标记。
```js
olyuyu: {
		audio: 1,
...
### ollbzhiji 名字:掷戟
描述: `锁定技。当你使用牌指定“义父”角色为目标时，若此牌为非伤害牌，你进行X次判定，若判定结果为：装备牌，你获得${get.poptip("shenji")}；【杀】或【决斗】，你获得${get.poptip("wushuang")}和判定牌。否则，此牌伤害+X并移除其所有“恨”标记（X为其“恨”标记数）。`
```js
ollbzhiji: {
		audio: 2,
...
### oljiejiu 名字:戒酒
描述: `锁定技。①你不能使用【酒】。②你可以将【酒】当作除【酒】外的任意基本牌使用。③游戏开始时，你令所有其他女性角色将其武将牌上随机一个技能替换为${get.poptip("lijian")}。`
```js
oljiejiu: {
		audio: 1,
		mod: {
...
## weiqing 名字:卫青 势力:qun

### dcbeijin 名字:北进
描述: 出牌阶段，你可以摸一张牌（此牌使用时无次数限制）。若如此做，本回合你下次使用牌或发动此技能时，若你的手牌中有以此法得到的牌，则你失去1点体力。
```js
dcbeijin: {
		enable: "phaseUse",
		manualConfirm: true,
...
## liuxiecaojie 名字:刘协曹节 势力:qun

### dcjuanlv 名字:眷侣
描述: 当你使用牌指定异性角色为目标后，你可以令其选择一项：①弃置一张手牌；②令你摸一张牌。
```js
dcjuanlv: {
		audio: false,
		equipSkill: false,
...
### dcqixin 名字:齐心
描述: 转换技。①出牌阶段，你可以将性别变更为：阳，曹节--女；阴，刘协--男。②当你即将死亡时，你取消之并将性别变更为〖齐心①〗的转换状态，将体力调整至此状态的体力，然后你本局游戏不能发动〖齐心〗。
```js
dcqixin: {
		enable: "phaseUse",
		filter(event, player) {
...
## dc_zhaoyun 名字:经典神赵云 势力:shen

### boss_juejing
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### dclonghun 名字:龙魂
描述: 每回合限20次，你可以将你的牌按下列规则使用或打出：红桃当【桃】，方块当火【杀】，梅花当【闪】，黑桃当【无懈可击】。
```js
dclonghun: {
		audio: 2,
		mod: {
...
## dc_sunce 名字:经典孙策 势力:wu

### dcshuangbi 名字:双璧
描述: 出牌阶段限一次，你可以选择一项：①摸X张牌，本回合手牌上限+X；②弃置至多X张牌，随机对其他角色造成等量火焰伤害；③视为使用至多X张火【杀】或【火攻】（X为场上存活角色数）。
```js
dcshuangbi: {
		audio: 2,
		enable: "phaseUse",
...
### dcsantou 名字:三头
描述: 锁定技，当你受到伤害时，防止之，然后若以下有条件成立，你失去1点体力：1.你于本回合此前以此法防止过该伤害来源的伤害，且你的体力值不小于3；2.本次伤害为属性伤害，且你的体力值为2；3.本次伤害的渠道为红色的牌，且你的体力值为1。
```js
dcsantou: {
		audio: 2,
		trigger: { player: "damageBegin4" },
...
### dcfaqi 名字:法器
描述: 当你于出牌阶段使用装备牌结算结束后，你可以视为使用一张本回合未以此法使用过的普通锦囊牌。
```js
dcfaqi: {
		audio: 2,
		trigger: { player: "useCardAfter" },
...
## dc_caocao 名字:经典曹操 势力:wei

### dcjianxiong 名字:奸雄
描述: 当你受到伤害后，你可以摸一张牌并获得对你造成伤害的牌，然后你令此技能摸牌数+1（至多为5）。
```js
dcjianxiong: {
		audio: "rejianxiong",
		trigger: {
...
## dc_liubei 名字:经典刘备 势力:shu

### dcrende 名字:仁德
描述: 出牌阶段每名角色限一次。你可以获得一名其他角色两张手牌，然后视为使用一张基本牌。
```js
dcrende: {
		audio: "rerende",
		enable: "phaseUse",
...
## dc_sunquan 名字:经典孙权 势力:wu

### dczhiheng 名字:制衡
描述: ①出牌阶段限一次。你可以弃置任意张牌并摸等量的牌，若你在发动〖制衡〗时弃置了所有手牌，则你多摸一张牌。②每回合每名角色限一次。当你对其他角色造成伤害后，你令〖制衡①〗于此回合发动次数上限+1。
```js
dczhiheng: {
		audio: "rezhiheng",
		init: player => {
...
## zhutiexiong 名字:朱铁雄 势力:qun

### dcbianzhuang 名字:变装
描述: ①出牌阶段限一次，你可以从系统随机选择的两个技能中获得一个，并视为使用一张【杀】（无距离次数限制），然后失去以此法获得的技能。②当你使用装备牌后，你清空此技能的发动次数记录。③当你发动〖变装①〗后，若你发动〖变装①〗的次数大于2，则你换肤为武·朱铁雄，并将系统选择的技能数改为三个。
```js
dcbianzhuang: {
		audio: 2,
		enable: "phaseUse",
...
## xiaoyuehankehan 名字:小约翰可汗 势力:qun

### dctongliao 名字:通辽
描述: ①摸牌阶段结束时，你可以选择一张点数最小的手牌，将此牌标记为“通辽”。②当你失去一张具有“通辽”标签的牌时，你摸X张牌（X为此牌点数）。
```js
dctongliao: {
		audio: 3,
...
### dcwudao 名字:悟道
描述: 当你使用牌结算结束后，若你本回合使用的上一张牌与此牌类型相同，则你可以于本回合内获得如下效果：当你于回合内使用该类型的牌时，你令此牌不可被响应且伤害值基数+1。
```js
dcwudao: {
		audio: 3,
		trigger: { player: "useCardAfter" },
...
### dclbjiuxian 名字:酒仙
描述: ①你可以将额定目标数大于1的锦囊牌当做【酒】使用。②你使用【酒】无次数限制。
```js
dclbjiuxian: {
		audio: 2,
		enable: "chooseToUse",
...
### dcshixian 名字:诗仙
描述: 当你使用一张牌时，若此牌的牌名与你于本局游戏使用的上一张牌的牌名押韵，则你可以摸一张牌，并令此牌额外结算一次。
```js
dcshixian: {
		audio: 2,
		trigger: { player: "useCard" },
...
### dccibei 名字:慈悲
描述: 每回合每名角色限一次。当你对其他角色造成伤害时，你可以防止此伤害，然后摸五张牌。
```js
dccibei: {
		audio: 2,
...
### dcruyi 名字:如意
描述: `锁定技。①游戏开始时，你将${get.poptip("ruyijingubang")}置入装备区。②你手牌区内的武器牌均视为【杀】，且你不是武器牌的合法目标。③当你即将失去${get.poptip("ruyijingubang")}或即将废除武器栏时，取消之。④你不能将装备区内的${get.poptip("ruyijingubang")}当做其他牌使用或打出。`
```js
dcruyi: {
		audio: 2,
...
## longwang 名字:龙王 势力:qun

### dclonggong 名字:龙宫
描述: 每回合限一次。当你受到伤害时，你可以防止此伤害，然后令伤害来源从牌堆中获得一张装备牌。
```js
dclonggong: {
		audio: 2,
		trigger: { player: "damageBegin4" },
...
### dcsitian 名字:司天
描述: `出牌阶段，你可以弃置两张颜色不同的手牌。系统从所有${get.poptip({
		id: "sitian_tianqi",
		name: "天气",
		type: "character",
...
## taoshen 名字:涛神 势力:qun

### dcnutao 名字:怒涛
描述: 锁定技。①当你使用锦囊牌指定第一个目标时，若目标角色包含其他角色，你随机对其中一名其他目标角色造成1点雷电伤害。②当你于出牌阶段造成雷电伤害后，你于此阶段使用【杀】的次数上限+1。
```js
dcnutao: {
		audio: 4,
		trigger: { player: "useCardToPlayer" },
...
### clbshuijian 名字:水箭
描述: 摸牌阶段开始时，你可以令额定摸牌数+X（X为你装备区内牌数的一半+1，且向下取整）。
```js
clbshuijian: {
		trigger: { player: "phaseDrawBegin2" },
		frequent: true,
...
## yeshiwen 名字:叶诗文 势力:wu

### clbjisu 名字:急速
描述: 判定阶段开始前，你可以跳过本回合的判定阶段和摸牌阶段，视为对一名其他角色使用一张【杀】。
```js
clbjisu: {
...
## sp_jiben 名字:SP吉本 势力:qun

### spduanzhi 名字:断指
描述: 当你成为其他角色使用的牌的目标后，你可以弃置其至多两张牌，然后失去1点体力。
```js
spduanzhi: {
		trigger: { target: "useCardToTargeted" },
		logTarget: "player",
...
### spduyi 名字:毒医
描述: 出牌阶段限一次，你可以亮出牌堆顶的一张牌并交给一名角色，若此牌为黑色，该角色不能使用或打出手牌，直到回到结束。
```js
spduyi: {
		enable: "phaseUse",
		usable: 1,
...
### spmixin 名字:密信
描述: 出牌阶段限一次，你可以将一张手牌交给一名其他角色，该角色须对你选择的另一名角色使用一张无距离限制的【杀】，否则你选择的角色观看其手牌并获得其中一张。
```js
spmixin: {
		audio: "qiuyuan",
		enable: "phaseUse",
...
## sp_fuwan 名字:SP伏完 势力:qun

### spfengyin 名字:奉印
描述: 其他角色的回合开始时，若其体力值不少于你，你可以交给其一张【杀】，令其跳过出牌阶段和弃牌阶段。
```js
spfengyin: {
		audio: "moukui",
...
### spchizhong 名字:持重
描述: 锁定技，你的手牌上限等于体力上限；其他角色死亡时，你加1点体力上限。
```js
spchizhong: {
		mod: {
...
## old_lingju 名字:SP灵雎 势力:qun

### jieyuan
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### fenxin_old 名字:焚心
描述: 限定技，当你杀死一名非主公角色时，你可以与其交换未翻开的身份牌。（你的身份为主公时不能发动此技能）
```js
fenxin_old: {
		mode: ["identity"],
...
## sp_mushun 名字:SP穆顺 势力:qun

### moukui
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

## dc_wuyi 名字:经典吴懿 势力:shu

### dcbenxi 名字:奔袭
描述: 转换技，锁定技。当你失去手牌后，阳：系统随机检索出一句转换为拼音后包含“wu,yi”的技能台词，然后你念出此台词。阴：你获得上次所念出的台词对应的技能直到你的下个回合开始；若你已拥有该技能，则改为对其他角色各造成1点伤害。
```js
dcbenxi: {
		trigger: {
			player: ["loseAfter"],
...
### dcqiusuo 名字:求索
描述: 当你造成或受到伤害后，你可以从牌堆或弃牌堆中获得一张【铁索连环】。
```js
dcqiusuo: {
		audio: 2,
		trigger: {
...
### dclisao 名字:离骚
描述: 出牌阶段限一次，你可以选择至多两名角色，这些角色须同时回答《离骚》的句段填空（抢答题，一名角色回答正确后结束答题）。回答正确的角色展示所有手牌，其余角色本回合不能响应你使用的牌且受到的伤害翻倍。
```js
dclisao: {
		audio: 2,
		enable: "phaseUse",
...
## xin_sunquan 名字:会玩孙权 势力:wu

### dchuiwan 名字:会玩
描述: 每回合每种牌名限一次，当你摸牌时，你可以选择至多等同于摸牌数的基本牌或普通锦囊牌的牌名并从牌堆中获得，然后你少摸等量张牌。
```js
dchuiwan: {
		audio: 2,
		trigger: { player: "drawBegin" },
...
### dchuanli 名字:唤理
描述: `结束阶段，若你本回合：使用牌至少指定三次自己为目标，则你可以令一名其他角色的所有技能失效，然后其获得${get.poptip("zhijian")}和${get.poptip("guzheng")}，直到其下回合结束；使用牌至少指定三次一名其他角色为目标，则你可以令其所有技能失效，然后其获得${get.poptip("reyingzi")}和${get.poptip("refanjian")}，直到其下回合结束。然后若你两项均执行，你获得${get.poptip("rezhiheng")}直到你的下个回合结束。`
```js
dchuanli: {
		audio: 2,
...
## wuhujiang 名字:五虎 势力:shu

### olhuyi 名字:虎翼
描述: ①游戏开始时，你从随机三个五虎将技能中选择一个获得。②当你使用或打出一张基本牌后，若你因此技能获得的技能数小于5，你随机获得一个技能描述中包含此牌名的五虎将技能。③回合结束时，你可以失去一个以此法获得的技能，然后你观看牌堆顶三张牌并获得其中一张。
```js
olhuyi: {
		audio: 5,
		getList() {
...
## dc_noname 名字:无名 势力:qun

### dcchushan 名字:出山
描述: 锁定技，游戏开始时，你获得两个武将的各一个技能，并将你的武将名改为这两个武将的名字组合。
```js
dcchushan: {
		trigger: {
			global: "phaseBefore",
...
## ol_jsrg_caocao 名字:汉曹操 势力:qun

### oldingxi 名字:定西
描述: 当你不因〖定西〗使用伤害牌进入弃牌堆后，你可以对上家使用其中一张伤害牌，然后将使用的牌置于武将牌上。结束阶段，你摸X张牌（X为你的“定西”牌数）。
```js
oldingxi: {
		audio: 2,
...
### olnengchen 名字:能臣
描述: 锁定技，当你受到牌造成的伤害后，若你拥有与此牌牌名相同的“定西”牌，则你随机获得其中一张。
```js
olnengchen: {
		audio: 2,
		trigger: { player: "damageEnd" },
...
### olhuojie 名字:祸结
描述: 锁定技，出牌阶段开始时，若X大于游戏人数，则你进行X次【闪电】判定直到以此法受到伤害（X为你的“定西”牌数）；然后若你因此受到了伤害，则你获得所有“定西”牌。
```js
olhuojie: {
		audio: 2,
...
## jiangziya 名字:姜子牙 势力:qun

### xingzhou 名字:兴周
描述: 每回合限一次，手牌数最少的角色受到伤害后，你可以弃置两张手牌，视为对伤害来源使用一张【杀】；若其因此死亡，你令〖列神〗视为未发动过。
```js
xingzhou: {
		usable: 1,
		trigger: {
...
### lieshen 名字:列神
描述: 限定技，出牌阶段，你可以令一名角色将体力值和手牌数调整至游戏开始时。
```js
lieshen: {
		init(player) {
...
## shengongbao 名字:申公豹 势力:qun

### zhuzhou 名字:助纣
描述: 每回合限一次，手牌数最多的角色造成伤害后，你可以令其获得受伤角色的一张手牌。
```js
zhuzhou: {
		usable: 1,
		trigger: {
...
### yaoxian 名字:邀仙
描述: 出牌阶段限一次，你可以令一名角色摸两张牌，然后其须对你指定的另一名其他角色使用【杀】，否则其失去1点体力。
```js
yaoxian: {
		enable: "phaseUse",
		usable: 1,
...
## nanjixianweng 名字:南极仙翁 势力:qun

### xwshoufa 名字:授法
描述: `出牌阶段，你可以展示并将所有♠/♥/♣/♦花色的手牌交给一名其他角色，令其获得${get.poptip("tiandu")}/${get.poptip("retianxiang")}/${get.poptip("reqingguo")}/${get.poptip("new_rewusheng")}直到你的下个回合开始。`
```js
xwshoufa: {
		enable: "phaseUse",
		filter(event, player) {
...
### fuzhao 名字:福照
描述: 一名角色进入濒死状态时，你可以令其进行一次判定，若结果为♥，其回复1点体力。
```js
fuzhao: {
		trigger: {
			global: "dying",
...
### quanjia 名字:劝架
描述: 其他角色于其回合内对另一名角色造成伤害后，你可以随机观看伤害来源两张手牌。若其中有【杀】，你对其使用之；若没有【杀】，则下次发动〖劝架〗时观看牌数+1。
```js
quanjia: {
		audio: 2,
		intro: {
...
## xiangjiaoduanwu 名字:香蕉端午 势力:qun

### xuyuan 名字:许愿
描述: 出牌阶段开始时，你可选择一个花色和一个装备的副类别，然后从牌堆中随机将一张满足要求的装备牌置入你的装备区。
```js
xuyuan: {
		trigger: { player: "phaseUseBegin" },
		async cost(event, trigger, player) {
...
### xiaomian 名字:笑面
描述: 当你使用牌时，可以摸X张牌(X为你装备区里与此牌花色相同的牌数）。若如此做，你弃置装备区里的一张牌。
```js
xiaomian: {
		trigger: { player: "useCard" },
		filter(event, player) {
