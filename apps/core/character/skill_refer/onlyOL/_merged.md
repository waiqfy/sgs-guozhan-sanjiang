# onlyOL merged reference

## ol_zhonghui 名字:OL界钟会 势力:wei

### olquanji 名字:权计
描述: 当你造成或受到1点伤害后，你可以摸一张牌，然后令当前回合角色将一张手牌置于你的武将牌上，称为“权”。你每有一张“权”，你的手牌上限便+1。
```js
olquanji: {
		audio: 2,
		trigger: {
...
### olzili 名字:自立
描述: `觉醒技，每个准备阶段，若你“权”的数里大于等于3，你回复1点体力或摸两张牌，然后减少1点体力上限，获得${get.poptip("olpaiyi")}。`
```js
olzili: {
		skillAnimation: true,
...
## ol_shen_huangzhong 名字:OL神黄忠 势力:shen

### shenyu 名字:神御
描述: `出牌阶段开始时，你可视为使用一张无距离限制的【杀】，然后将${get.poptip("chixueren")}置入唯一目标角色的装备区（替换原装备）。每个结束阶段，若你本回合造成过伤害或使用过至少两张伤害牌，且${get.poptip("chixueren")}在其他角色装备区，你可将其移动至你的装备区（替换原装备），获得其和其与你之间角色各随机一张牌。`
```js
shenyu: {
		audio: 2,
		trigger: {
...
### huaren 名字:化刃
描述: 限定技，准备阶段，你可将场上或你的一张武器牌永久移出游戏，摸等同于其攻击范围数里的牌，视为拥有其技能。
```js
huaren: {
		audio: 2,
		limited: true,
...
## ol_sb_zhurong 名字:OL谋祝融 势力:shu

### olsbrenche 名字:刃掣
描述: 出牌阶段限X次，你可弃置X张牌并令至多X名其s他角色各弃置一张牌（X为本回合弃牌堆中【杀】的数量且至少为1）。每有角色因此弃置牌不为【杀】，你便摸一张牌。
```js
olsbrenche: {
		audio: 2,
		enable: "phaseUse",
...
### olsbyalian 名字:牙镰
描述: 每阶段结束时，若你此阶段不因使用失去过【杀】，你可以视为对任意名手牌数小于等于X的角色使用一张火【杀】（X为本回合弃牌堆中【杀】的数量且至少为1）。
```js
olsbyalian: {
		audio: 2,
...
## ol_liufeng 名字:OL界刘封 势力:shu

### olxiansi 名字:陷嗣
描述: 准备阶段，你可以将至多两名角色共计至多两张牌置于你的武将牌上，称为“逆”。其他角色可以移去两张“逆”视为对你使用一张【杀】。
```js
olxiansi: {
		audio: 4,
		logAudio: () => 2,
...
### olqinling 名字:侵凌
描述: 每回合限一次，你可以弃置一张牌，视为使用一张【杀】。若此【杀】造成了伤害，你摸一张牌，否则你将你弃置的牌置为“逆”。
```js
olqinling: {
		audio: 2,
		enable: "chooseToUse",
...
## ol_bulianshi 名字:OL界步练师 势力:wu

### olanxu 名字:安恤
描述: 出牌阶段限一次，你可以获得两名角色各一张牌。然后你展示一张牌并交给其中手牌较少的角色，若不为黑桃，你摸一张牌。
```js
olanxu: {
		audio: 2,
		enable: "phaseUse",
...
### olzhuiyi 名字:追忆
描述: 限定技，结束阶段，你可令一名其他角色摸三张牌并回复1点体力。然后你死亡时，可以对其再次发动此技能。
```js
olzhuiyi: {
		audio: 2,
		limited: true,
...
## ol_quancong 名字:OL界全琮 势力:wu

### olyaoming 名字:邀名
描述: `每回合限一次，当你造成或受到伤害后，你可以选择一名角色，若其手牌数: 大于等于你，你弃置其一张牌； 小于等于你，其摸一张牌。然后其回合内，你视为拥有技能${get.poptip("olzhenshan")}。`
```js
olyaoming: {
		audio: 2,
...
## ol_caoxiu 名字:OL界曹休 势力:wei

### olqianju 名字:千驹
描述: 锁定技，你每损失1点体力，你计算与其他角色的距离便-1。若全场角色均在你的攻击范围内，你出牌阶段使用【杀】的次数+1。
```js
olqianju: {
		audio: 2,
		mod: {
...
### olqingxi 名字:倾袭
描述: 当你使用杀对目标角色造成伤害时，你可令其选择一项：1.弃置你攻击范围张牌；2.此【杀】对其伤害+1。若你的装备区有武器牌，则改为其依次执行这两项，然后弃置此牌。
```js
olqingxi: {
		audio: 2,
		trigger: {
...
## ol_guanping 名字:OL界关平 势力:shu

### longyin
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### oljieyong 名字:竭勇
描述: 你可将手牌中最后一张红色牌当【杀】使用或打出
```js
oljieyong: {
		audio: 2,
...
## dm_zhangfei 名字:魔张飞 势力:shu

### olzhuohun 名字:灼魂
描述: 锁定技，你的回合内：你的【闪】均视为【杀】；一名角色的勾玉首次变为一个颜色后，你摸一张牌。
```js
olzhuohun: {
		audio: 6,
		forced: true,
...
### olchenshi 名字:瞋视
描述: `锁定技，若其他角色的勾玉颜色与你：不同，你使用黑色【杀】能额外指定其为目标；相同，你对其，或其对你使用的黑色【杀】改为【决斗】。你因【杀】和【决斗】首次造成了或受到了至少3点伤害后，你回复1点体力，${get.poptip({
...
## ol_xiahoushi 名字:OL界夏侯氏 势力:shu

### olqiaoshi 名字:樵拾
描述: 每个结束阶段，你可以与当前回合角色各摸一张牌。然后若其与你手牌数不相等，此技能本轮失效。
```js
olqiaoshi: {
		audio: 2,
		trigger: {
...
### olyanyu 名字:燕语
描述: 出牌阶段，你可以重铸【杀】。出牌阶段结束时，若你本阶段失去过至少两张【杀】，你可以令一名男性角色摸两张牌。
```js
olyanyu: {
		audio: 2,
		enable: "phaseUse",
...
## junk_zhangjiao 名字:OL神张角 势力:shen
...
## shen_dianwei 名字:神典韦 势力:shen

### juanjia 名字:捐甲
描述: 锁定技。游戏开始时，你废除一个防具栏，然后获得一个额外的武器栏。
```js
juanjia: {
		audio: 2,
		trigger: {
...
### qiexie 名字:挈挟
描述: `锁定技。准备阶段，你在剩余武将牌堆中随机观看五张牌，选择其中的任意张，将其转化为${get.poptip({
		id: "qiexie_equip1",
		name: "武器牌",
		type: "character",
...
### cuijue 名字:摧决
描述: 每回合每名角色限一次。出牌阶段，你可以弃置一张牌，然后对攻击范围内距离最远的一名其他角色造成1点伤害（没有则不选）。
```js
cuijue: {
		audio: 2,
		enable: "phaseUse",
...
## ol_zhangliao 名字:OL神张辽 势力:shen

### olduorui 名字:夺锐
描述: 当你于出牌阶段内对一名角色造成伤害后，你可以选择该角色武将牌上的一个技能。若如此做，你结束出牌阶段，且你令此技能于其下个回合结束之前无效。
```js
olduorui: {
		audio: "drlt_duorui",
...
### olzhiti 名字:止啼
描述: 锁定技，你攻击范围内已受伤角色的手牌上限-1。若场上已受伤的角色数：不小于1，你的手牌上限+1；不小于3，你于摸牌阶段开始时令额定摸牌数+1；不小于5，回合结束时，你废除一名角色的一个随机装备栏。
```js
olzhiti: {
		audio: "drlt_zhiti",
		global: "olzhiti2",
...
## shen_caopi 名字:神曹丕 势力:shen

### chuyuan 名字:储元
描述: 一名角色受到伤害后，若你武将牌上「储」的数量小于体力上限，你可以令其摸一张牌。然后其将一张手牌置于你的武将牌上，称为「储」。
```js
chuyuan: {
		audio: 2,
...
### dengji 名字:登极
描述: `觉醒技，准备阶段，若你武将牌上的「储」数不小于3，则你减1点体力上限并获得所有「储」，然后获得技能${get.poptip("tianxing")}和${get.poptip("new_rejianxiong")}。`
```js
dengji: {
		audio: 2,
...
## shen_zhenji 名字:神甄宓 势力:shen

### shenfu 名字:神赋
描述: 回合结束时，若你的手牌数为：奇数，你可对一名其他角色造成1点雷属性伤害。若其因此进入过濒死状态，你可重复此流程（不能选择本次已选择过的角色）。偶数，你可选择一名角色，你令其摸一张牌或弃置一张手牌。若其手牌数等于体力值，你可重复此流程（不能选择本次已选择过的角色）。
```js
shenfu: {
		audio: 2,
...
## ol_zhangsong 名字:OL界张松 势力:shu

### olqiangzhi 名字:强识
描述: 出牌阶段开始时，你可观看并展示一名其他角色的一张手牌，然后你本阶段使用此类别的牌后可摸一张牌。
```js
olqiangzhi: {
		audio: 2,
		trigger: {
...
### olxiantu 名字:献图
描述: 其他角色出牌阶段开始时，你可以摸至多两张牌然后交给其等量牌。此阶段结束时，若其造成伤害小于你以此法交给其的牌数，你失去1点体力。
```js
olxiantu: {
		audio: 2,
		trigger: {
...
## ol_jsrg_zhanghe 名字:闪张郃 势力:qun

### olqiongtu 名字:穷途
描述: 群势力技，每回合限一次，你可视为使用一张【无懈可击】并摸一张牌，然后你须将一张非基本牌置于你的武将牌上。若你未因此放置牌，你获得武将牌上的所有牌，并变更势力为魏。
```js
olqiongtu: {
		audio: "jsrgqiongtu",
		enable: "chooseToUse",
...
### olxianzhu 名字:先著
描述: 魏势力技，出牌阶段各限一次，你可以弃置一张锦囊牌或装备牌，视为使用一张无次数限制的【杀】。若此【杀】造成伤害，你可使用本次弃置的牌。
```js
olxianzhu: {
		audio: "jsrgxianzhu",
		enable: "phaseUse",
...
## ol_caojie 名字:OL界曹节 势力:qun

### olshouxi 名字:守玺
描述: 每回合限一次，当你成为伤害牌的目标后，你可弃置至少一张伤害牌，对使用者造成1点伤害。然后若此牌未对你造成伤害，你摸牌至手牌上限。
```js
olshouxi: {
		audio: 2,
		trigger: {
...
### olhuimin 名字:惠民
描述: 出牌阶段限一次，你可令任意手牌数小于等于体力值的角色摸一张牌，然后你的手牌上限+X至你的下个回合开始（X为这些角色中手牌数大于体力值的角色数）。
```js
olhuimin: {
		audio: 2,
		enable: "phaseUse",
...
## ol_re_xinxianying 名字:OL界辛宪英 势力:wei

### olcaishi 名字:才识
描述: 当你每轮首次使用一种类型的牌后，本轮你的手牌上限+1，若你的手牌上限不为全场最大，你可以令一名角色回复1点体力，然后本回合此技能失效。
```js
olcaishi: {
		audio: 2,
		forced: true,
...
### olzhongjian 名字:忠鉴
描述: 出牌阶段限一次，你可展示一名其他角色X张手牌（X为其体力值），然后你展示一张手牌。若展示其的牌包含与你展示牌：颜色相同的牌，你摸一张牌或弃置其一张牌；牌名相同的牌，本阶段此技能改为限两次。
```js
olzhongjian: {
		audio: 2,
		enable: "phaseUse",
...
## ol_sb_dongzhao 名字:OL谋董昭 势力:wei

### olsbshunji 名字:顺机
描述: 每轮每名角色限一次，当一名角色受到伤害后，你可摸至多两张牌，然后交给其等量张牌。若你因此失去了一个牌名的所有牌（每个牌名限一次），你对其造成1点伤害。
```js
olsbshunji: {
		audio: 3,
		trigger: {
...
### olsbyishi 名字:移势
描述: 出牌阶段限一次，你可保留手牌中每个花色各一张牌并展示，将其余牌置入弃牌堆，然后移动场上的一张牌。若你未因此失去手牌，直到你的下个回合开始，你不因使用而失去与移动牌花色相同的牌后，摸一张牌。
```js
olsbyishi: {
		audio: 2,
		enable: "phaseUse",
...
## ol_sb_chengyu 名字:OL谋程昱 势力:wei

### olsbliduan 名字:戾断
描述: 你使用【杀】后，令你使用的下一张锦囊牌可以多指定一个目标；你使用锦囊牌后，令你使用的下一张【杀】可以多指定一个目标。
```js
olsbliduan: {
		audio: 2,
		forced: true,
...
### olsbdanchi 名字:胆持
描述: 每回合限一次，与你距离1以内的角色受到伤害后，你可令受到伤害的角色选择一个类型。然后伤害来源本回合使用下一张牌后，你视为使用一张【无中生有】，若与”胆持”选择的类型不同，你可额外视为使用一张【杀】。
```js
olsbdanchi: {
		audio: 2,
		usable: 1,
...
## dm_caocao 名字:OL魔曹操 势力:wei

### olbachao 名字:霸朝
描述: `出牌阶段开始时，你可以令所有其他角色同时选择是否交给你一张牌，然后你可对一名未以此法交给你非基本牌的角色造成1点伤害；若该角色为你，本阶段你使用当前手牌无距离次数限制并${get.poptip("rule_rumo")}。`
```js
olbachao: {
		audio: 5,
		trigger: {
...
### olfuzai 名字:覆载
描述: 锁定技，①你的装备牌只能当【借刀杀人】或【无中生有】使用。②若你的装备区没有牌，你视为装备着随机武器和防具各一件（优先装备攻击距离/牌名字数与你的体力值相同的武器/防具）。
```js
olfuzai: {
		audio: 5,
		enable: "chooseToUse",
...
## ol_sb_xizhicai 名字:OL谋戏志才 势力:wei

### olsbxinchuan 名字:薪传
描述: 你使用的锦囊牌进入弃牌堆后，你可以令一名角色依次执行X次摸一张牌和X次弃置一张牌（X为本回合弃牌堆缺少的花色数），然后若其手牌数变为体力值，令其下家执行剩余流程。
```js
olsbxinchuan: {
		audio: 2,
		trigger: {
...
### olsbjinjin 名字:金烬
描述: 你需要使用普通锦囊牌时，可以将本回合进入弃牌堆的任意张花色各不相同的牌移出游戏并失去〖金烬〗，视为使用之；然后你于下个受到过伤害的回合结束时令一名角色获得移出游戏的牌和〖金烬〗。
```js
olsbjinjin: {
		audio: 2,
		enable: "chooseToUse",
...
## dm_sunquan 名字:魔孙权 势力:wu

### olquanyu 名字:权御
描述: `锁定技，每轮开始时，你令所有角色同时选择一项其本局未选择过的“${get.poptip({
		id: "quanyu_effect",
		name: "权御",
		type: "character",
...
### oltianen 名字:天恩
描述: `锁定技，每回合各限一次，你使用牌指定唯一目标后：若本轮你与其选择的“权御”效果不同，你随机弃置其一张牌，对其发动一次${get.poptip("olquanyu")}；若与你相同，你从牌堆获得一张不计入手牌上限的【杀】。`
```js
oltianen: {
		audio: 3,
...
### olqiangang 名字:乾纲
描述: `出牌阶段，你可${get.poptip("rule_rumo")}，失去${get.poptip("oltianen")}，然后本局你使用指定唯一目标的【杀】均执行目标所选择过的所有“权御”效果。`
```js
olqiangang: {
		audio: 3,
		enable: "phaseUse",
...
## ol_sb_guojia 名字:OL谋郭嘉 势力:wei

### olsbdinglun 名字:定论
描述: `出牌阶段限一次，你可选择至多半数角色（向上取整），若这些角色的手牌数之和大于其他角色的手牌数之和，这些角色获得${get.poptip("olsbquxi")}至你下个准备阶段并摸一张牌。`
```js
olsbdinglun: {
		audio: 2,
		derivation: ["olsbquxi"],
...
### olsbjieli 名字:解罹
描述: 每轮限一次，当你成为一张牌的唯一目标时，你可以选择一项：1.由你重新选择此牌的目标；2观看一名其他角色的所有手牌，获得其中与此牌花色相同的牌。
```js
olsbjieli: {
		audio: 2,
		round: 1,
...
## ol_re_yujin 名字:OL界于禁 势力:wei

### ol_zhenjun 名字:镇军
描述: 准备阶段，你可弃置一名角色X张牌（X为其手牌数减体力值且至少为1），然后你选择一项：1.你弃置与其中非装备牌数等量的牌：2.结束阶段，其摸与其中非装备牌数等量的牌。
```js
ol_zhenjun: {
		audio: 2,
		trigger: { player: "phaseZhunbeiBegin" },
...
### ol_yizhong 名字:毅重
描述: 锁定技，体力值大于等于你的角色对你使用的黑色【杀】无效，手牌数小于等于你的角色无法响应你使用的黑色【杀】。
```js
ol_yizhong: {
		audio: 2,
		trigger: {
...
## ol_jsrg_zhujun 名字:闪朱儁 势力:qun

### olfendi 名字:分敌
描述: 你使用【杀】指定唯一目标后，可以展示其至多X张手牌（X为你的体力上限）。若如此做，直到此【杀】结算结束，其只能使用或打出对应实体牌全为这些牌的牌，且此【杀】对其造成伤害后，你获得其手牌或弃牌堆中的这些牌。
```js
olfendi: {
		audio: 2,
		trigger: { player: "useCardToPlayered" },
...
### oljuxiang 名字:拒降
描述: 每回合限一次，你于摸牌阶段外获得牌后，可以弃置这些牌，令当前回合角色本回合出【杀】次数+X（X为你以此法弃置的牌数）。
```js
oljuxiang: {
		audio: 2,
		trigger: {
...
## ol_jsrg_sunjian 名字:闪孙坚 势力:qun

### olpingtao 名字:平讨
描述: 出牌阶段限一次，你可令一名其他角色选择一项：1.交给你一张牌，你本回合获得一个“讨”标记；2.你视为对其使用一张有次数限制的【杀】。你使用【杀】可弃置一枚“讨”标记并额外指定一个目标。
```js
olpingtao: {
		audio: 2,
		enable: "phaseUse",
...
### oljuelie 名字:绝烈
描述: 你使用【杀】指定第一个目标后，可以弃置至多X张牌（X为你的体力值），然后令目标角色共弃置等量的牌；然后若你的手牌数或体力值为全场最小，此【杀】基础伤害+1。你因此【杀】杀死一名角色后，将手牌摸至体力上限。
```js
oljuelie: {
		audio: 2,
		trigger: { player: "useCardToPlayered" },
...
## ol_jsrg_zhangliao 名字:闪张辽 势力:qun

### olzhengbing 名字:整兵
描述: 群势力技。出牌阶段限三次。你可以重铸一张牌，若此牌为：【杀】，你的手牌上限+1；【闪】，你摸一张牌；【桃】/【酒】，你于此回合结束时执行一个出牌阶段。然后若你以此法触发过所有选项，变更势力为魏。
```js
olzhengbing: {
		audio: "jsrgzhengbing",
		enable: "phaseUse",
...
## ol_sb_xuyou 名字:OL谋许攸 势力:qun

### olsbqianfu 名字:迁附
描述: 转换技，出牌阶段，阳：你可以将一张黑色牌当【过河拆桥】使用：阴：你可以将一张红色牌当【火攻】使用。结算后，你可将因此弃置的牌置于牌堆顶。
```js
olsbqianfu: {
		audio: 2,
		zhuanhuanji: true,
...
### olsbyushi 名字:驭势
描述: `你造成或受到伤害后，你可令一名：有转换技的角色切换其一个转换技的状态：无转换技的角色获得出牌阶段各限一次的${get.poptip("olsbqianfu")}，直到其失去最后的手牌。每轮前X次，有转换技切换状态后，你摸一张牌（X为你的体力上限）。`
```js
olsbyushi: {
		audio: 2,
		trigger: {
...
### olsbfenchao 名字:焚巢
描述: 限定技，结束阶段，你可以令一名角色获得弃牌堆中的伤害牌（不超过存活人数）且令这些牌造成伤害时改为火焰伤害。
```js
olsbfenchao: {
		audio: 2,
		limited: true,
...
## ol_sb_lusu 名字:OL谋鲁肃 势力:wu

### olsbduduan 名字:独断
描述: 回合开始时，你可以摸一张牌，跳过本回合判定、摸牌、出牌和弃牌阶段中的一个阶段（每阶段限一次），并交换其中另外两个阶段。
```js
olsbduduan: {
		audio: 2,
...
### olsbyinglve 名字:英略
描述: 出牌阶段各限一次，你可以令一名角色：1.失去1点体力，下个摸牌阶段摸牌数+2；2.摸两张牌，下个弃牌阶段手牌上限-2。
```js
olsbyinglve: {
		audio: 2,
		enable: "phaseUse",
...
### olsbmengshi 名字:盟势
描述: 限定技，结束阶段，你可以选择两名其他角色并失去X点体力（X为这两名角色体力值之差且不能为0），令这两名角色交换体力值。然后直到其中一名角色死亡，每回合结束时，你摸一张牌并回复1点体力。
```js
olsbmengshi: {
		audio: 2,
		trigger: { player: "phaseJieshuBegin" },
...
## ol_sb_zhugeliang 名字:OL谋诸葛亮 势力:shu

### olsbzhitian 名字:知天
描述: 出牌阶段，牌堆顶的七张牌对你可见。你的【火攻】结算中，你可观看并选择牌堆顶七张牌中的一张牌代替【火攻】弃牌。
```js
olsbzhitian: {
		audio: 2,
...
### olsbwujing 名字:武靖
描述: 锁定技，你的回合内，有牌不因使用而进入弃牌堆后，你使用的下一张牌无次数限制；每回合首次有不为手牌的牌进入弃牌堆后，你摸一张牌。
```js
olsbwujing: {
		audio: 2,
		trigger: {
...
### olsbzhijue 名字:智绝
描述: 转换技，阳：出牌阶段，你可将牌堆顶的一张牌当【火攻】使用；阴：将一种颜色的手牌置入弃牌堆（每种颜色每回合限一次），然后可视为使用其中一张基本牌或普通锦囊牌。若你以此法未造成伤害，你令〖知天〗可见牌与观看牌数-1（至多减至1），然后你摸两张牌。
```js
olsbzhijue: {
		audio: 5,
		zhuanhuanji: true,
...
## ol_sunluban 名字:OL界孙鲁班 势力:wu

### olzenhui 名字:谮毁
描述: 出牌阶段各限一次，你使用【杀】或普通锦囊牌（【借刀杀人】等带有指向性目标的牌除外）指定目标时，可以令一名能成为此牌目标的角色选择一项：1.交给你一张牌并代替你成为此牌使用者（不计次数）；2.成为此牌的额外目标。
```js
olzenhui: {
		audio: 2,
		trigger: { player: "useCardToPlayer" },
...
### oljiaojin 名字:骄矜
描述: 你受到伤害时，可以弃置一张非伤害锦囊牌或装备牌并防止此伤害，然后获得造成伤害的牌。
```js
oljiaojin: {
		audio: 2,
...
## dm_lvbu 名字:魔吕布 势力:qun

### olduoqi 名字:夺炁
描述: 锁定技。①所有角色的初始手牌称为“炁”。②一号位首个回合开始前，你执行一个不能使用延时锦囊牌的出牌阶段。③你每回合对一名角色首次造成伤害后，你从弃牌堆、牌堆、场上角色的区域内中获得其一张“炁”。
```js
olduoqi: {
		audio: 2,
...
### olkuangmo 名字:狂魔
描述: `出牌阶段，你可以${get.poptip("rule_rumo")}并选择一名其他角色，你与其成为“狂”角色，每回合对彼此首次造成的伤害+1；杀死对方后，获得对方所有“炁”。“狂”角色死亡后，你重新指定。`
```js
olkuangmo: {
...
### olgangquan 名字:罡拳
描述: 若你使用的上一张牌为“炁”，你可以将一张装备牌当指定相邻角色为目标的火【杀】使用；否则，你可以将一张锦囊牌当【决斗】使用。
```js
olgangquan: {
		audio: 2,
		init(player, skill) {
...
## dm_diaochan 名字:魔貂蝉 势力:qun

### olhuanhuo 名字:幻惑
描述: 每轮开始时，你摸两张牌，然后弃置至多两张牌并选择等量其他角色。其下回合出牌阶段获得以下效果直到其本阶段使用了两张牌：①若其有可用的手牌，其不能使用主动技能且当其需要使用牌时，其强制使用其中随机的一张牌。②其使用一张牌后随机弃一张牌。
```js
olhuanhuo: {
		audio: 2,
...
### olqingshi 名字:倾世
描述: `准备阶段，你可${get.poptip("rule_rumo")}，令所有角色获得一张单目标伤害牌。其他角色使用此牌指定唯一目标时，你可弃置一张牌，重新指定牌的目标（无距离限制)。这些牌：造成伤害后，你摸一张牌；未因使用进入弃牌堆后，你获得之。（准备阶段，若这些牌均离开其手牌区，你再令所有角色获得牌。）`
```js
olqingshi: {
		audio: 2,
		trigger: { player: "phaseZhunbeiBegin" },
...
## ol_guohuanghou 名字:OL界郭皇后 势力:wei

### oljiaozhao 名字:矫诏
描述: 出牌阶段限一次，你可将一张牌当本轮未有角色使用过的基本或普通锦囊牌使用。
```js
oljiaozhao: {
		audio: 2,
...
### oldanxin 名字:殚心
描述: `当你受到伤害后，你可摸X张牌，然后修改${get.poptip("oljiaozhao")}（X为你修改${get.poptip("oljiaozhao")}的次数）。`
```js
oldanxin: {
		audio: 2,
		trigger: {
...
## ol_sb_xiaoqiao 名字:OL谋小乔 势力:wu

### olmiluo 名字:迷落
描述: 出牌阶段限一次，你可展示并交给至多两名其他角色各一张牌，这些牌本轮称为“迷落”牌。每轮结束时，你可令其中一名手牌中没有/有“迷落”牌的角色失去/回复1点体力。
```js
olmiluo: {
		audio: 2,
		enable: "phaseUse",
...
### oljueyan 名字:绝颜
描述: 每轮各限一次，当你需要使用手牌中一张普通锦囊牌或红色基本牌时，可改为展示并视为使用之；你每轮首次展示一种花色的手牌后摸一张牌。
```js
oljueyan: {
		audio: 2,
		enable: "chooseToUse",
...
## dm_simayi 名字:魔司马懿 势力:wei

### olguifu 名字:诡伏
描述: ①每轮开始时或当你的体力值变化后，你可以随机从牌堆或弃牌堆获得一张不计入手牌上限的【闪】。②本局游戏中牌或技能直接造成伤害后，你记录此牌名或技能。③每回合每种牌名限一次，你可以将因此技能获得的【闪】当做不计入次数限制的〖诡伏②〗记录过的牌名使用。
```js
olguifu: {
		audio: 2,
		trigger: {
...
### olmoubian 名字:谋变
描述: `准备阶段，若你记录的牌名和技能之和大于等于3，你可以${get.poptip("rule_rumo")}，获得记录的全部技能，然后获得${get.poptip("olzhouxi")}。`
```js
olmoubian: {
		audio: 4,
		derivation: ["olzhouxi"],
...
## ol_madai 名字:OL界马岱 势力:shu

### mashu
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### olqianxi 名字:潜袭
描述: 出牌阶段开始时，你可以展示一张牌，本回合距离为1的其他角色不能使用或打出与此牌颜色相同的手牌，且你本回合使用此牌造成的伤害+1。
```js
olqianxi: {
		audio: 2,
		trigger: { player: "phaseUseBegin" },
...
## ol_sb_yl_luzhi 名字:OL谋卢植 势力:qun

### olsibing 名字:司兵
描述: 每回合限一次。①当你使用伤害牌指定唯一目标时，你可以弃置任意张红色牌令目标弃置等量红色手牌，否则不能响应该牌；②以你为目标的伤害牌结算完成后，若未对你造成伤害，你可以弃置一张黑色牌并视为使用一张【杀】。
```js
olsibing: {
		audio: 2,
		trigger: {
...
### olliance 名字:敛策
描述: 每轮限一次，当你的手牌数变化后，若为全场最少，你可将手牌摸至体力上限，然后本回合下一次有角色造成伤害时，此伤害+1。
```js
olliance: {
		audio: 2,
		trigger: {
...
## ol_guanzhang 名字:OL界关兴张苞 势力:shu

### olfuhun 名字:父魂
描述: `你可将两张牌当【杀】使用或打出。你使用的转化【杀】目标角色只能使用颜色相同的手牌响应；你于出牌阶段使用【杀】造成伤害后，你本回合获得${get.poptip("new_rewusheng")}和${get.poptip("olpaoxiao")}。`
```js
olfuhun: {
		inherit: "fuhun",
		position: "hes",
...
## ol_jsrg_liuhong 名字:闪刘宏 势力:qun

### olchaozheng 名字:朝争
描述: ①出牌阶段开始时，你可以与所有其他角色议事。若结果为：红色，意见为红色的角色各回复1点体力；黑色，意见为红色的其他角色各失去1点体力。然后你摸X张牌（X为此次议事意见与你相同的角色数且至多为2）。②当你参与的议事展示意见时，你的意见权重+1。
```js
olchaozheng: {
		audio: "jsrgchaozheng",
...
### olshenchong 名字:甚宠
描述: `限定技。准备阶段，你可以令一名其他角色获得${get.poptip("olrefeiyang")}和${get.poptip("jsrgbahu")}。若如此做，当你死亡时，杀死你的角色弃置所有手牌，其失去所有技能。`
```js
olshenchong: {
		limited: true,
		audio: "jsrgshenchong",
...
### oljulian 名字:聚敛
描述: 主公技。①其他群势力角色每回合限一次。当其不于摸牌阶段且不因〖聚敛〗摸牌后，其可以摸一张牌。②结束阶段，你可以获得所有其他群势力角色各一张手牌。
```js
oljulian: {
		audio: "jsrgjulian",
		inherit: "jsrgjulian",
...
## ol_jsrg_zhaoyun 名字:闪赵云 势力:shu

### ollonglin 名字:龙临
描述: 其他角色于其出牌阶段内首次使用【杀】指定目标后，你可以弃置一张牌令此【杀】无效，然后其可以视为对你使用一张【决斗】，你以此法造成伤害后，其本阶段使用的下一张牌不能指定其他角色为目标。
```js
ollonglin: {
		audio: "jsrglonglin",
		inherit: "jsrglonglin",
...
### olzhendan 名字:镇胆
描述: ①你可以将一张非基本手牌当作你本轮未使用过的任意基本牌使用或打出。②当你受到伤害后或每轮结束时，你摸X张牌并令该技能本轮失效（X为本轮所有角色执行过的回合数且至多为5）。
```js
olzhendan: {
		audio: "jsrgzhendan",
		enable: ["chooseToUse", "chooseToRespond"],
...
## ol_sb_jiaxu 名字:OL谋贾诩 势力:qun

### olsbwance 名字:完策
描述: 出牌阶段限一次，你可以选择一名有手牌的角色并选择一个单目标普通锦囊牌牌名，其依次将X张手牌当作此普通锦囊牌使用（X为游戏轮数且至多为3）。其以此法使用的牌指定目标时，你可以弃置一张牌并修改此牌的目标。
```js
olsbwance: {
		audio: 2,
		enable: "phaseUse",
...
### olsbchenzhi 名字:沉智
描述: ①你每轮受到第X次以后的伤害时，你弃置一张牌防止之（X为游戏轮数且至多为3)。②每局游戏限一次，每轮结束时，若你本轮未发动过〖沉智①〗，则你可以复原一名角色的一个限定技。
```js
olsbchenzhi: {
		audio: 2,
		//加个标记
...
### olsbluanchao 名字:乱朝
描述: 限定技，每轮开始时，你可以令所有角色依次选择从牌堆中获得一张【杀】或【闪】。选择获得【杀】的角色本轮首次造成的伤害+1。
```js
olsbluanchao: {
		limited: true,
		audio: 2,
...
## ol_sb_wenchou 名字:OL谋文丑 势力:qun

### olsblunzhan 名字:轮战
描述: 出牌阶段，你可将任意张牌当作【决斗】使用（至多五张，且选择的转化牌数不得为你本回合发动〖轮战〗已经转化过的牌数）。若你因此对唯一目标造成了伤害，你可以摸X张牌，然后你本回合不能再对其发动此技能（X为本回合你使用牌指定其为目标的次数）。
```js
olsblunzhan: {
		audio: 2,
		enable: "phaseUse",
...
### olsbjuejue 名字:决绝
描述: 锁定技，当你于回合内首次仅使用所有手牌指定仅一名其他角色为目标时，你令其弃置X张牌（X为本回合你使用牌指定其为目标的次数）。
```js
olsbjuejue: {
		audio: 2,
		trigger: { player: "useCardToPlayer" },
...
## ol_sb_zhangrang 名字:OL谋张让 势力:qun

### olsblucun 名字:赂存
描述: ①每轮每项各限一次，你可以视为使用一张未以此法使用过的有目标的基本牌或普通锦囊牌，然后目标中手牌最多的随机一名角色将一张手牌置于你的武将牌上，称为“赂”。②每个回合结束时，你随机移去一张“赂”，然后摸一张牌。
```js
olsblucun: {
		audio: 6,
		enable: "chooseToUse",
...
### olsbtuisheng 名字:蜕生
描述: `限定技，准备阶段或当你进入濒死状态时，你可以重置本局${get.poptip("olsblucun")}使用过的牌名，然后你选择一项并回复1点体力：①将所有手牌置于你的武将牌上，称为“赂”；②你随机从弃牌堆中获得游戏轮数张重置前“赂存”使用牌名的牌。`
```js
olsbtuisheng: {
		limited: true,
		audio: 6,
...
## ol_fuhuanghou 名字:OL界伏寿 势力:qun

### rezhuikong
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### olqiuyuan 名字:求援
描述: 当你成为【杀】或伤害锦囊牌的目标时，你可以令另一名其他角色选择一项：1.交给你一张与此牌同类型且不同牌名的牌；2.成为此牌的额外目标。
```js
olqiuyuan: {
		inherit: "qiuyuan",
		filter(event, player) {
...
## ol_guohuai 名字:OL界郭淮 势力:wei

### oljingce 名字:精策
描述: 当你于回合内首次使用某种花色的手牌时，你的手牌上限+1。一名角色的出牌阶段结束时，你可以摸X张牌（X为你本阶段内使用过的牌的类型数）。
```js
oljingce: {
		audio: 2,
		inherit: "rejingce",
...
## ol_sb_zhangxiu 名字:OL谋张绣 势力:qun

### olsbchoulie 名字:仇猎
描述: 限定技，回合开始时，你可选择一名其他角色。本回合你的每个阶段开始时，你可弃置一张牌视为对其使用一张【杀】。当其成为此【杀】的目标后，其可弃置一张基本牌或武器牌令此【杀】无效。
```js
olsbchoulie: {
		audio: 2,
		trigger: { player: "phaseBegin" },
...
### olsbzhuijiao 名字:追剿
描述: 锁定技。你使用【杀】时，若你使用的上一张牌未造成伤害，则你摸一张牌并令此【杀】伤害+1。此【杀】结算后，若仍未造成伤害，你弃置一张牌。
```js
olsbzhuijiao: {
		audio: 2,
...
## ol_sb_zhaoyun 名字:OL谋赵云 势力:shu

### olsbnilan 名字:逆澜
描述: 当你不因此技能造成伤害后，你可以选择一项：1.弃置所有手牌，若其中有【杀】，你可对一名其他角色造成1点伤害；2.摸两张牌。若如此做，你下次受到伤害后可以执行另一项。
```js
olsbnilan: {
		audio: 2,
		trigger: { source: "damageSource" },
...
### olsbjueya 名字:绝崖
描述: 当你需要使用一张你未以此法使用过的基本牌时，若你没有手牌，你可以视为使用之。
```js
olsbjueya: {
		audio: 2,
...
## ol_sb_zhangfei 名字:OL谋张飞 势力:shu

### olsbjingxian 名字:敬贤
描述: 出牌阶段每名角色限一次，你可以交给其至多两张非基本牌，然后其选择等量项：1.其与你各摸一张牌；2.令你从牌堆中获得一张【杀】。
```js
olsbjingxian: {
		audio: 2,
		enable: "phaseUse",
...
### olsbxieyong 名字:狭勇
描述: 出牌阶段限一次，你可以选择一名其他角色，直至其下个回合结束，你处于【酒】的状态。其于此期间使用一张牌后（目标不为其自己），你可以对其使用一张无视防具的【杀】。
```js
olsbxieyong: {
		audio: 2,
		enable: "phaseUse",
...
## ol_sb_huangyueying 名字:OL谋黄月英 势力:shu

### olsblixian 名字:理贤
描述: 当你受到伤害后，你可将一张牌当【无中生有】使用。当你以此法使用过三种不同牌名的锦囊牌后，此技能于你的结束阶段也可发动。
```js
olsblixian: {
		audio: 2,
		trigger: {
...
### olsbbingcai 名字:并才
描述: 每回合第一张基本牌被使用时，你可重铸一张牌。若两张牌均为伤害类或非伤害类，则此牌额外结算一次。然后若重铸的牌为锦囊牌，你为〖理贤〗添加一个牌名：【顺手牵羊】；【过河拆桥】；【铁索连环】。以上牌名均添加后，〖理贤〗于你的准备阶段也可发动。
```js
olsbbingcai: {
		audio: 2,
		trigger: {
...
## ol_sb_jushou 名字:OL谋沮授 势力:qun

### olsbguliang 名字:固粮
描述: 每回合限一次，其他角色对你使用牌时，你可令此牌对你无效，若如此做，你无法响应其对你使用的牌直到回合结束。
```js
olsbguliang: {
		audio: 2,
...
### olsbxutu 名字:徐图
描述: 锁定技，游戏开始时，你将牌堆顶三张牌置于你的武将牌上，称为“资”。每个结束阶段，你将本回合弃牌堆的一张牌与一张“资”交换，令一名角色获得三张花色或点数相同的“资”，然后将“资”补至三张。
```js
olsbxutu: {
		audio: 2,
		trigger: {
...
## ol_sb_gongsunzan 名字:OL谋公孙瓒 势力:qun

### olsbjiaodi 名字:剿狄
描述: 锁定技。①你的攻击范围视为你的体力值。②当你使用【杀】指定唯一目标时，若其攻击范围：小于等于你，你令此牌造成的伤害+1，获得其一张手牌；大于等于你，你弃置其区域一张牌，选择一名角色作为此牌的额外目标。
```js
olsbjiaodi: {
		audio: 3,
		trigger: { player: "useCardToPlayer" },
...
### olsbbaojing 名字:保京
描述: 出牌阶段限一次，你可以令一名其他角色的攻击范围+1或-1（至多减至1）直到你的下个出牌阶段开始。
```js
olsbbaojing: {
		audio: 3,
		enable: "phaseUse",
...
## ol_sb_dengai 名字:OL谋邓艾 势力:wei

### olsbjigu 名字:积谷
描述: 锁定技。一名角色于其出牌阶段外使用的牌进入弃牌堆后，若“谷”的数量小于你的体力上限，你将其中的非♥牌置于你的武将牌上，称为“谷”。体力上限与你相同的角色的回合开始时，你可以用任意张手牌替换等量“谷”。
```js
olsbjigu: {
		audio: 2,
...
### olsbjiewan 名字:解腕
描述: ①每个准备阶段，你可以减少1点体力上限或移去两张“谷”，然后将一张手牌当距离限制为X的【顺手牵羊】使用（X为你的“谷”数且至少为1）。②每个结束阶段，若你的手牌数与“谷”的数量相同且你的体力上限不为全场唯一最高，你增加1点体力上限。
```js
olsbjiewan: {
		audio: 2,
...
## ol_sb_dongzhuo 名字:OL谋董卓 势力:qun

### olguanbian 名字:观变
描述: 锁定技。①游戏开始时，你的手牌上限、其他角色计算与你的距离、你计算与其他角色的距离+X（X为游戏人数）。②首轮结束时，或当你发动〖凶逆〗或〖封赏〗后，你失去〖观变〗。
```js
olguanbian: {
		audio: 2,
		trigger: {
...
### olxiongni 名字:凶逆
描述: 出牌阶段开始时，你可以弃置一张牌，然后所有其他角色选择一项：1.弃置一张与此牌花色相同的牌；2.受到你的1点伤害。
```js
olxiongni: {
		audio: 6,
		trigger: {
...
### olfengshang 名字:封赏
描述: 出牌阶段限一次或有角色进入濒死状态时（每回合限一次），你可以将两张本回合进入弃牌堆中的本轮未以此法选择过的相同花色的牌分配给等量角色。若你未以此法获得牌，你视为使用一张不计入次数的【酒】。
```js
olfengshang: {
		audio: 6,
		getCards(player) {
...
### olzhibin 名字:执柄
描述: `主公技，锁定技。准备阶段，若其他群势力角色累计使用黑色牌的次数达到：三张，你增加1点体力上限并回复1点体力；六张，你获得${get.poptip("dcfencheng")}；九张：你获得${get.poptip("benghuai")}。`
```js
olzhibin: {
		audio: 6,
		getNum(player) {
...
## ol_sb_huaxiong 名字:OL谋华雄 势力:qun

### olsbbojue 名字:搏决
描述: 出牌阶段限两次，你可以选择一名其他角色，然后同时与其选择摸一张牌或弃置一张牌，然后若你与其的手牌数之和的变化值为：0，你与其依次弃置对方一张牌；2，你与其依次视为对对方使用一张【杀】。
```js
olsbbojue: {
		audio: 2,
		enable: "phaseUse",
...
### olsbyangwei 名字:扬威
描述: 锁定技。当你于摸牌/弃牌阶段外摸/弃置了本回合的第二张牌后，你下一次造成/受到的伤害+1。
```js
olsbyangwei: {
		audio: 2,
...
## ol_sb_yuanshu 名字:OL谋袁术 势力:qun

### olsbjinming 名字:矜名
描述: 锁定技，回合开始时，你选择一项：1.回复过1点体力；2.弃置过两张牌；3.使用过三种类型的牌；4.造成过4点伤害。然后本回合结束时你摸X张牌，若未满足选择的条件，则删除此选项（X为你最后一次发动〖矜名〗选择的选项序号）。
```js
olsbjinming: {
		audio: 2,
		trigger: { player: "phaseBegin" },
...
### olsbxiaoshi 名字:枭噬
描述: `出牌阶段限一次，你使用牌可以额外指定一个目标（无距离限制）。此牌结算完毕后，若此牌未造成过伤害，你失去1点体力或令其中一个目标摸X张牌（X为你最后一次发动${get.poptip("olsbjinming")}选择的选项序号）。`
```js
olsbxiaoshi: {
		audio: 2,
...
### olsbyanliang 名字:厌粱
描述: 主公技，其他群势力角色的出牌阶段限一次，其可以交给你一张装备牌，视为使用一张【酒】。
```js
olsbyanliang: {
		audio: 2,
		zhuSkill: true,
...
## ol_sb_sunjian 名字:OL谋孙坚 势力:wu

### olsbhulie 名字:虎烈
描述: 每回合各限一次，当你使用【杀】或【决斗】指定唯一目标后，你可以令此牌伤害值基数+1。若此牌未造成伤害，你可以令目标角色视为对你使用一张【杀】。
```js
olsbhulie: {
		audio: 3,
		trigger: {
...
### olsbyipo 名字:毅魄
描述: 你的体力值变化后，若你体力值大于0且为你首次达到，你可以选择一名角色并选择一项：1.令其摸X张牌，然后弃置一张牌；2.令其摸一张牌，然后其弃置X张牌（X为你已损失体力值且至少为1）。
```js
olsbyipo: {
		audio: 3,
...
## ol_sb_jiangwei 名字:OL谋姜维 势力:shu

### olsbzhuri 名字:逐日
描述: 你的阶段结束时，若你本阶段失去过手牌或得到过牌，则你可以与一名角色拼点。若你赢，你可以使用其中一张拼点牌；若你没赢，你失去1点体力或令此技能于本回合无效。
```js
olsbzhuri: {
		audio: 2,
		trigger: {
...
### olsbranji 名字:燃己
描述: `限定技，结束阶段。若你本回合使用过牌的阶段数大于等于/小于等于体力值，你可以获得技能${get.poptip("kunfenx")}/${get.poptip("zhaxiang")}（同时满足则都获得，以此法获得的〖困奋〗直接修改为非锁定技）。若如此做，你将手牌数调整至手牌上限或将体力值回复至体力上限，然后你不能回复体力直到你杀死角色。`
```js
olsbranji: {
		audio: 2,
		trigger: { player: "phaseJieshuBegin" },
...
## ol_caozhang 名字:OL界曹彰 势力:wei

### oljiangchi 名字:将驰
描述: 摸牌阶段结束时，你可以选择一项：①摸一张牌，本回合使用【杀】的次数上限-1，且【杀】不计入手牌上限。②重铸一张牌，本回合使用【杀】无距离限制，且使用【杀】的次数上限+1。
```js
oljiangchi: {
		audio: "rejiangchi",
...
## ol_jianyong 名字:OL界简雍 势力:shu

### olqiaoshui 名字:巧说
描述: 出牌阶段，你可与一名其他角色拼点。若你赢，你本回合使用下一张基本牌或普通锦囊牌时，可以为此牌增加或减少一个目标；若你没赢，此技能于本回合失效且本回合你不能使用锦囊牌。
```js
olqiaoshui: {
		audio: "reqiaoshui",
		inherit: "reqiaoshui",
...
## ol_lingtong 名字:OL界凌统 势力:wu

### olxuanfeng 名字:旋风
描述: 当你一次性失去至少两张牌后，或失去装备区的牌后，你可以依次弃置一至两名其他角色的共计两张牌。
```js
olxuanfeng: {
		audio: "xuanfeng",
...
## ol_sb_guanyu 名字:OL谋关羽 势力:shu

### olsbweilin 名字:威临
描述: 每回合限一次，你可以将一张牌当作任意【杀】或【酒】使用，且目标角色本回合与此牌颜色相同的手牌均视为【杀】。
```js
olsbweilin: {
		audio: 2,
		enable: "chooseToUse",
...
### olsbduoshou 名字:夺首
描述: 锁定技。①你每回合使用的第一张红色牌无距离限制。②你每回合使用的第一张基本牌不计入使用次数。③你每回合第一次造成伤害后，你摸一张牌。
```js
olsbduoshou: {
		init(player) {
...
## ol_sb_taishici 名字:OL谋太史慈 势力:wu

### olsbdulie 名字:笃烈
描述: 每回合限一次，当你成为其他角色使用基本牌或普通锦囊牌的唯一目标时，你可以令此牌额外结算一次。若如此做，此牌结算完毕后，你摸X张牌（X为你的攻击范围且至多为5）。
```js
olsbdulie: {
		audio: 2,
		trigger: { target: "useCardToTarget" },
...
### olsbdouchan 名字:斗缠
描述: 锁定技，准备阶段，你从牌堆中获得一张【决斗】，若牌堆没有【决斗】，则你的攻击范围和出牌阶段使用【杀】的次数上限+1（增加次数不超过本局游戏人数）。
```js
olsbdouchan: {
		audio: 2,
		trigger: { player: "phaseZhunbeiBegin" },
...
## ol_gaoshun 名字:OL界高顺 势力:qun

### olxianzhen 名字:陷阵
描述: 出牌阶段限一次，你可以与一名角色拼点。若你赢，本回合你无视该角色的防具且对其使用牌没有次数和距离限制，且当你使用【杀】或普通锦囊牌指定目标时，可以令该角色也成为此牌的目标；若你没赢，本回合你不能对其使用【杀】且你的【杀】不计入手牌上限。
```js
olxianzhen: {
		audio: "rexianzhen",
		inherit: "xianzhen",
...
## ol_sb_yuanshao 名字:OL谋袁绍 势力:qun

### olsbhetao 名字:合讨
描述: 其他角色使用牌指定第一个目标后，若此牌指定的目标数大于1，则你可以弃置一张与此牌颜色相同的牌并令此牌改为对其中一名目标角色结算两次。
```js
olsbhetao: {
		audio: 6,
...
### olsbshenli 名字:神离
描述: 出牌阶段限一次，当你使用【杀】指定目标后，你可以令所有可成为此牌目标的其他角色均成为此牌目标，此牌结算完毕后，若你因此牌造成的伤害值X：大于你的手牌数，你摸X张牌（至多摸五张）；大于你的体力值，你再次对所有目标角色中可以成为此牌目标的角色使用此牌。
```js
olsbshenli: {
		audio: 6,
		trigger: { player: "useCardToPlayered" },
...
### olsbyufeng 名字:玉锋
描述: `游戏开始时，你将${get.poptip("sizhaojian")}置入装备区。`
```js
olsbyufeng: {
		audio: 3,
		trigger: {
...
### olsbshishou 名字:士首
描述: `主公技，其他群势力角色失去装备区的牌后，若你的装备区中没有武器牌，其可将${get.poptip("sizhaojian")}置入你的装备区。`
```js
olsbshishou: {
		audio: 6,
		trigger: {
...
## ol_yufan 名字:OL界虞翻 势力:wu

### olzongxuan 名字:纵玄
描述: 当你弃置而失去牌后，或你的上家每回合因弃置首次失去牌后，你可以将位于弃牌堆的这些牌中的任意牌以任意顺序置于牌堆顶。
```js
olzongxuan: {
		audio: 2,
...
### olzhiyan 名字:直言
描述: 你或你的上家的结束阶段，你可以令一名角色正面朝上摸一张牌，然后若此牌：为装备牌，则其使用此牌并回复1点体力；不为装备牌且其体力值不等于你，则其失去1点体力。
```js
olzhiyan: {
		audio: 2,
		trigger: { global: "phaseJieshuBegin" },
...
## ol_chengpu 名字:OL界程普 势力:wu

### dclihuo 名字:疠火
描述: ①你使用的非火【杀】可以改为火【杀】，若如此做，此牌结算完毕后，若此牌造成过伤害，则你弃置一张牌或失去1点体力。②你使用火【杀】可以额外指定一个目标。
```js
dclihuo: {
		audio: "relihuo",
...
### olchunlao 名字:醇醪
描述: ①当你或你的上下家的【杀】因弃置进入弃牌堆后，你将位于弃牌堆的这些牌称为“醇”置于武将牌上（最多为9）。②一名角色处于濒死状态时，你可以将X张“醇”置入弃牌堆，然后令其视为使用一张【酒】（X为你本轮以此法使用【酒】的次数）。
```js
olchunlao: {
		audio: "chunlao",
...
## ol_wangyi 名字:OL界王异 势力:wei

### olzhenlie 名字:贞烈
描述: `当你成为其他角色使用【杀】或普通锦囊牌的目标后，你可以失去1点体力并令此牌对你无效，然后你选择一项：①获得使用者的一张牌；②于本回合的结束阶段发动一次${get.poptip("olmiji")}（每回合限一次）。`
```js
olzhenlie: {
		audio: 2,
		inherit: "zhenlie",
...
### olmiji 名字:秘计
描述: 结束阶段，若你已受伤，则你可以摸X张牌，然后你可以将至多X张牌任意分配给其他角色（X为你已损失的体力值）。
```js
olmiji: {
		audio: 2,
		trigger: { player: "phaseJieshuBegin" },
...
## ol_sb_pangtong 名字:OL谋庞统 势力:shu

### olsbhongtu 名字:鸿图
描述: `一名角色的阶段结束时，若你于此阶段得到过至少两张牌，你可以摸三张牌，展示三张手牌，令一名其他角色选择是否使用其中一张牌并令你随机弃置其中另一张牌。若使用牌的点数于三张牌中满足以下条件，其获得如下技能或效果直到其下一个回合的回合结束：唯一最大，其获得${get.poptip("nzry_feijun")}；不为最大且不为最小，其获得${get.poptip("qianxi")}；唯一最小，其手牌上限+2。若其未以此法使用牌，你对其与你各造成1点火焰伤害。`
```js
olsbhongtu: {
		audio: 6,
		trigger: {
...
### olsbqiwu 名字:栖梧
描述: 当你每回合首次受到伤害时，若伤害来源为你或在你的攻击范围内，你可以弃置一张红色牌，防止此伤害。
```js
olsbqiwu: {
		audio: 6,
		trigger: { player: "damageBegin4" },
...
## ol_fazheng 名字:OL界法正 势力:shu

### olxuanhuo 名字:眩惑
描述: 摸牌阶段结束时，你可以交给一名其他角色两张牌，然后其选择一项：1.对你选择的另一名其他角色使用一张【杀】，2.令你观看并获得其两张牌。
```js
olxuanhuo: {
		audio: 2,
		trigger: { player: "phaseDrawEnd" },
...
### olenyuan 名字:恩怨
描述: ①当你一次性获得一名其他角色超过一张牌后，你可以令其摸一张牌。②当你受到1点伤害后，你可以令伤害来源选择一项：1.将一张红色手牌交给你；2.失去1点体力。
```js
olenyuan: {
		audio: 2,
...
## ol_caifuren 名字:OL界蔡夫人 势力:qun

### olqieting 名字:窃听
描述: 其他角色的回合结束后，若其本回合未对其他角色造成伤害/使用过牌，你可以选择一项/至多两项：1.将其装备区的一张牌置入你的装备区；2.摸一张牌。
```js
olqieting: {
		audio: 2,
		trigger: {
...
## ol_liru 名字:OL界李儒 势力:qun

### oljuece 名字:绝策
描述: 结束阶段，你可以对一名手牌数不大于你的其他角色造成1点伤害。
```js
oljuece: {
		audio: 2,
		trigger: { player: "phaseJieshuBegin" },
...
### olmieji 名字:灭计
描述: 出牌阶段限一次，你可以将一张锦囊牌置于牌堆顶，然后令一名有手牌的其他角色弃置一张锦囊牌或依次弃置两张牌。
```js
olmieji: {
		audio: 2,
		inherit: "xinmieji",
...
## ol_liubiao 名字:OL界刘表 势力:qun

### olzishou 名字:自守
描述: 摸牌阶段，你可以多摸X张牌，你以此法摸牌的结束阶段，若你本回合使用过伤害牌，你弃置X张牌（X为全场势力数）。
```js
olzishou: {
		audio: 2,
...
### olzongshi 名字:宗室
描述: 锁定技。①你的手牌上限+X（X为全场势力数）。②每种势力限一次，当其他角色对你造成伤害时，你防止此伤害并令其获得你区域内一张牌。
```js
olzongshi: {
		mod: {
			maxHandcard(player, num) {
...
## ol_wuguotai 名字:OL界吴国太 势力:wu

### olganlu 名字:甘露
描述: 出牌阶段限一次，你可以交换两名角色装备区内的牌，然后若你的已损失体力值小于X，你弃置X张手牌（X为交换前两者装备区牌数之差）。
```js
olganlu: {
		enable: "phaseUse",
		usable: 1,
...
### olbuyi 名字:补益
描述: 一名角色进入濒死状态时，你可展示其一张牌。若此牌不为基本牌，则其弃置此牌并回复1点体力。
```js
olbuyi: {
		audio: 2,
		trigger: {
...
## ol_sb_kongrong 名字:OL谋孔融 势力:qun
...
### olsbzhengyi 名字:争义
描述: 当一名有“贤”标记的角色受到非属性伤害时，其以外所有有“贤”标记的角色同时选择是否取消此伤害，所有角色选择完毕后，若有角色选择是，则取消此伤害，然后选择是的角色中体力值最大的一名角色依次失去X点体力（X为伤害值）。
```js
olsbzhengyi: {
		audio: 2,
...
## ol_zhangchunhua 名字:OL界张春华 势力:wei
...
### oljianmie 名字:翦灭
描述: 出牌阶段限一次，你可以选择一名其他角色，你与其同时选择一个颜色，然后弃置各自选择颜色的手牌，若你/其以此法弃置了更多手牌，则你/其视为对其/你使用一张【决斗】。
```js
oljianmie: {
		audio: 2,
		enable: "phaseUse",
...
## ol_caochong 名字:OL界曹冲 势力:wei

### olchengxiang 名字:称象
描述: 当你受到1点伤害后，你可以亮出牌堆顶的四张牌，然后获得其中任意张点数之和不大于13的牌；若获得的牌点数之和刚好为13，下次发动〖称象〗额外亮出一张牌。
```js
olchengxiang: {
...
### olrenxin 名字:仁心
描述: 当一名其他角色进入濒死状态时，你可以翻面并弃置一张装备牌，然后令该角色回复体力至1点。
```js
olrenxin: {
		audio: 2,
		trigger: {
...
## ol_caozhi 名字:OL界曹植 势力:wei

### reluoying
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### oljiushi 名字:酒诗
描述: ①当你需要使用【酒】时，若你的武将牌正面向上，你可以翻面，视为使用一张【酒】。②当你受到伤害后，若你的武将牌于受到伤害时背面向上，或当你于回合外累计因〖落英〗获取至少X张牌（X为你的体力上限）后，若你的武将牌背面向上，你可以翻面。③若你的武将牌背面向上，你使用因〖落英〗获得的牌无距离限制且不可被响应。
```js
oljiushi: {
		audio: 2,
		trigger: {
...
## ol_liaohua 名字:OL界廖化 势力:shu

### oldangxian 名字:当先
描述: 回合开始时，你执行一个额外的出牌阶段。此阶段开始时，你可以从牌堆或弃牌堆获得一张无距离限制的【杀】，若如此做，此阶段结束时，若你未于此阶段造成过伤害，则你对自己造成1点伤害。
```js
oldangxian: {
		audio: 2,
...
### olfuli 名字:伏枥
描述: 限定技，当你处于濒死状态时，你可将体力回复至X点，将手牌摸至X张。若你本局游戏造成的伤害数小于X，则你将武将牌翻面（X为存活势力数）。
```js
olfuli: {
		limited: true,
		audio: 2,
