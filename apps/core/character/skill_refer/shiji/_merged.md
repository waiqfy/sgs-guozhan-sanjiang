# shiji merged reference

## shen_lusu 名字:神鲁肃 势力:shen

### tamo 名字:榻谟
描述: 游戏开始时，你可以重新分配除主公外所有角色的座次。
```js
tamo: {
		available(mode) {
...
### dingzhou 名字:定州
描述: 出牌阶段限一次。你可以将X张牌交给一名场上有牌的角色，然后你获得其场上的所有牌（X为其场上的牌数）。
```js
dingzhou: {
		audio: 2,
		enable: "phaseUse",
...
### zhimeng 名字:智盟
描述: 回合结束后，你可以选择一名其他角色。若如此做，你与其将各自所有手牌置于处理区，然后你随机获得这些牌中的一半（向上取整），其获得剩余的牌。
```js
zhimeng: {
		audio: 2,
...
## shen_huatuo 名字:手杀神华佗 势力:shen

### wuling 名字:五灵
描述: `①出牌阶段限两次。你可以选择一名没有“${get.poptip({
		id: "wl_wuqinxi",
		name: "五禽戏",
		type: "character",
...
### youyi 名字:游医
描述: ①弃牌阶段结束时，你可以将所有于此阶段弃置的牌置入仁区。②出牌阶段限一次。你可以将仁区的所有牌置入弃牌堆，令所有角色各回复1点体力。
```js
youyi: {
		init(player) {
...
## shen_sunce 名字:神孙策 势力:shen

### yingba 名字:英霸
描述: ①出牌阶段限一次，你可令一名体力上限大于1的其他角色，令其失去1点体力、减少1点体力上限并获得1枚“平定”标记，然后你减少1点体力上限。②你对拥有“平定”标记的角色使用牌没有次数/距离限制。拥有“平定”标记的角色每回合前X张牌不能指定你为目标（X为其拥有的“平定”标记数）。
```js
yingba: {
		audio: 2,
		mod: {
...
### scfuhai 名字:覆海
描述: 锁定技。①当你使用牌指定目标后，若目标角色有“平定”标记，则其不可响应此牌。若你本回合内以此法得到的牌数小于2，则你摸一张牌。②拥有“平定”标记的角色死亡时，你增加X点体力上限并摸X张牌（X为其拥有的“平定”标记数）。
```js
scfuhai: {
		audio: 2,
...
### pinghe 名字:冯河
描述: `锁定技。①你的手牌上限基数等于体力上限。②当你受到其他角色造成的伤害时，若你有牌且你的体力上限大于1，则你防止此伤害，减1点体力上限并选择一项：1.将1张牌交给一名其他角色；2.重铸1张牌。然后若你拥有技能${get.poptip("yingba")}，令伤害来源获得1个“平定”标记。`
```js
pinghe: {
		derivation: "yingba",
		audio: 2,
...
## shen_taishici 名字:神太史慈 势力:shen

### dulie 名字:笃烈
描述: 锁定技。当你成为【杀】的目标时，若使用者的体力值大于你，则你进行判定。若结果为红桃，则取消此目标。
```js
dulie: {
		audio: 2,
...
### tspowei 名字:破围
描述: `使命技。①游戏开始时，你令所有其他角色获得一个“围”。②一名角色受到伤害后，若其有“围”，则其移去“围”。③回合开始时，你选择所有有“围”的角色。这些角色失去“围”，然后这些角色的第一个不为你的下家获得等量的“围”。④一名其他角色的回合开始时，若其有“围”，则你可以选择一项：⒈弃置一张手牌并对其造成1点伤害。⒉若其体力值不大于你，则你获得其一张手牌。选择完成后，你视为在其攻击范围内直到回合结束。⑤使命：回合开始时，若场上没有“围”，则你获得技能${get.poptip("shenzhu")}。⑥失败：当你进入濒死状态时，你将体力值回复至1点，然后弃置装备区的所有牌。`
```js
tspowei: {
		audio: 3,
		dutySkill: true,
...
## shen_xunyu 名字:神荀彧 势力:shen

### tianzuo 名字:天佐
描述: `锁定技。①游戏开始时，你将八张${get.poptip("qizhengxiangsheng")}加入牌堆。②${get.poptip("qizhengxiangsheng")}对你无效。`
```js
tianzuo: {
		audio: 2,
		trigger: {
...
### lingce 名字:灵策
描述: `锁定技。当有${get.poptip("qizhengxiangsheng")}或智囊或〖定汉①〗记录过的锦囊牌被使用时，若此牌不为转化牌且对应实体牌数量为1，则你摸一张牌。`
```js
lingce: {
		audio: 2,
		init: player => {
...
### dinghan 名字:定汉
描述: ①当你成为未记录过的普通锦囊牌的目标时，或有未记录过的延时锦囊牌进入你的判定区时，你记录此牌名并取消之。②回合开始时，你可在〖定汉①〗的记录中添加或减少一种锦囊牌的牌名。
```js
dinghan: {
		audio: 2,
		trigger: {
...
## shen_guojia 名字:神郭嘉 势力:shen

### reshuishi 名字:慧识
描述: 出牌阶段限一次。若你的体力上限小于10，你可进行判定牌不置入弃牌堆的判定。若判定结果与本次发动技能时的其他判定结果的花色均不相同且你的体力上限小于10，则你加1点体力上限，且可以重复此流程。然后你将所有位于处理区的判定牌交给一名角色。若其手牌数为全场最多，则你减1点体力上限。
```js
reshuishi: {
		audio: "shuishi",
		enable: "phaseUse",
...
### stianyi 名字:天翊
描述: `觉醒技，准备阶段，若场上的所有存活角色均于本局游戏内受到过伤害，则你加2点体力上限并回复1点体力，然后令一名角色获得技能${get.poptip("zuoxing")}。`
```js
stianyi: {
		audio: 2,
...
### resghuishi 名字:辉逝
描述: 限定技，出牌阶段，你可选择一名角色。若你的体力上限不小于存活人数且其有未发动的觉醒技，则你令其中一个技能无视发动条件；否则其摸四张牌。然后你减2点体力上限。
```js
resghuishi: {
		onChooseToUse(event) {
...
## mb_shen_machao 名字:手杀神马超 势力:shen

### yuli 名字:驭雳
描述: 锁定技。①你造成的伤害改为雷电伤害，已是雷电伤害则伤害+1；②你受到雷电伤害时，防止之并摸等量牌。
```js
yuli: {
		audio: 6,
		trigger: {
...
### tingwei 名字:霆威
描述: 你使用【杀】指定目标后，可获得4枚“霆”标记并选择一名目标角色，其选择任意项并令你弃等量的“霆”标记：1.非锁定技失效至其下个回合结束；2.交给你一张装备牌；3.此牌对其造成伤害+1；4.随机弃两张牌。若其均未选择，其横置。
```js
tingwei: {
		audio: 4,
		trigger: { player: "useCardToPlayered" },
...
### jimie 名字:寂灭
描述: `限定技，出牌阶段结束时，你可以弃6枚“霆”标记，对一名角色造成等于其体力上限的伤害。然后当你${get.poptip("yuli")}的两项均执行后，此技能视为未发动过。`
```js
jimie: {
		audio: 4,
...
## mb_shen_jiangwei 名字:手杀神姜维 势力:shen

### mbtiantao 名字:天涛
描述: 锁定技，结束阶段，你选择一个区域并弃置其中所有牌，然后依次弃置任意名其他角色相同区域各一张牌，因此弃置牌且未弃置【杀】的角色失去1点体力。
```js
mbtiantao: {
		audio: 2,
		trigger: {
...
### mbxinghun 名字:星魂
描述: 出牌阶段限一次，你可以观看牌堆顶五张牌，用任意张手牌与其中的等量张牌进行交换并任意排序，然后你令一名其他角色展示你的手牌与牌堆顶的共计五张牌，你对其依次使用其中的【杀】。
```js
mbxinghun: {
		audio: 2,
		enable: "phaseUse",
...
### mbshenpei 名字:神霈
描述: `限定技，当你进入濒死状态时，你可以回复X点体力(X为你本局游戏进入过濒死状态的次数)，然后对一名角色造成等量点雷电伤害并获得${get.poptip("mbhuitian")}。`
```js
mbshenpei: {
		audio: 2,
		limited: true,
...
## liuba 名字:手杀刘巴 势力:shu

### duanbi 名字:锻币
描述: 限定技。出牌阶段，若场上所有角色的手牌数之和大于角色数之和的二倍，则你可以令所有其他角色各弃置X张手牌（X为该角色手牌数的一半且向下取整且至多为3）。然后你可选择一名角色，令其随机获得三张以此法被弃置的牌。
```js
duanbi: {
		limited: true,
		audio: 2,
...
### tongduo 名字:统度
描述: 每回合限一次。当你成为其他角色使用牌的唯一目标后，你可令一名角色重铸一张牌。
```js
tongduo: {
		audio: 2,
...
## sp_zhujun 名字:手杀朱儁 势力:qun

### xinyangjie 名字:佯解
描述: 出牌阶段限一次，你可以与一名其他角色A拼点。若你没赢，则你可以令另一名其他角色B视为对A使用一张火【杀】。
```js
xinyangjie: {
		audio: "yangjie",
		enable: "phaseUse",
...
### xinjuxiang 名字:拒降
描述: 限定技，一名其他角色脱离濒死状态时，你可以对其造成1点伤害。
```js
xinjuxiang: {
		audio: "zjjuxiang",
		inherit: "zjjuxiang",
...
### houfeng 名字:厚俸
描述: `每轮限一次。一名其他角色的出牌阶段开始时，若其在你的攻击范围内，则你可以令其进行“${get.poptip("rule_zhengsu")}”。其因此获得整肃奖励后，你获得相同的整肃奖励。`
```js
houfeng: {
		audio: 3,
...
## sp_huangfusong 名字:手杀皇甫嵩 势力:qun

### spzhengjun 名字:整军
描述: `出牌阶段开始时，你可进行“${get.poptip("rule_zhengsu")}”。你因此获得整肃奖励后，你可令一名其他角色获得整肃奖励。`
```js
spzhengjun: {
		audio: 3,
...
### spshiji 名字:势击
描述: 当你对其他角色造成属性伤害时，若你的手牌数不为全场唯一最多，则你可以观看其手牌。你令其弃置其中的所有红色牌，然后摸等量的牌。
```js
spshiji: {
		audio: 2,
...
### sptaoluan 名字:讨乱
描述: 每回合限一次。一名角色的判定结果确定时，若结果的花色为♠，则你可以终止导致此判定发生的上级事件。然后选择一项：①获得判定牌对应的实体牌。②视为对判定角色使用一张火【杀】（无距离和次数限制）。
```js
sptaoluan: {
		audio: 2,
...
## sp_lvfan 名字:手杀吕范 势力:wu

### mbdiaodu 名字:调度
描述: 准备阶段，你可以移动一名角色装备区内的一张牌（不能移动给自己），然后其摸一张牌。
```js
mbdiaodu: {
		audio: "spdiaodu",
		trigger: { player: "phaseZhunbeiBegin" },
...
### mbdiancai 名字:典财
描述: 其他角色的出牌阶段阶段结束后，若你本阶段失去的牌数不小于你的体力值，则你可将手牌数补至体力上限。
```js
mbdiancai: {
		audio: "spdiancai",
...
### spyanji 名字:严纪
描述: `出牌阶段开始时，你可以进行“${get.poptip("rule_zhengsu")}”。`
```js
spyanji: {
		audio: 3,
		trigger: { player: "phaseUseBegin" },
...
## sp_jiangqing 名字:蒋钦 势力:wu

### spjianyi 名字:俭衣
描述: 锁定技。其他角色的回合结束时，若弃牌堆中有于本回合内因弃置而进入弃牌堆的防具牌，则你获得其中一张。
```js
spjianyi: {
		audio: 2,
...
### spshangyi 名字:尚义
描述: 出牌阶段限一次。你可以弃置一张牌并选择一名有手牌的其他角色。其观看你的手牌，然后你观看其手牌并获得其中的一张。
```js
spshangyi: {
		audio: 2,
		enable: "phaseUse",
...
## sp_jiangwan 名字:手杀蒋琬 势力:shu

### spzhenting 名字:镇庭
描述: `每回合限一次。当你或你攻击范围内的角色成为【杀】或延时锦囊的目标时，若你不是此牌的使用者，你可选择一项：①弃置使用者的一张手牌；②摸一张牌；③${get.poptip("rule_beishui")}：代替其成为此牌目标。`
```js
spzhenting: {
		audio: 4,
...
### spjincui 名字:尽瘁
描述: 限定技。出牌阶段，你可以和一名其他角色交换位置，然后失去X点体力（X为你的体力值-你发动过〖镇庭〗的次数）；若你未以此法失去体力，你增加1点体力上限并失去〖镇庭〗的背水选项。
```js
spjincui: {
		audio: 4,
		enable: "phaseUse",
...
## sp_zhangchangpu 名字:手杀张昌蒲 势力:wei

### spdifei 名字:抵诽
描述: 锁定技。每回合限一次，当你受到伤害后，你摸一张牌或弃置一张手牌，然后展示所有手牌。若此伤害的渠道为没有花色的牌或你的手牌中没有与此牌花色相同的牌，则你回复1点体力。
```js
spdifei: {
		audio: 2,
...
### spyanjiao 名字:严教
描述: 出牌阶段限一次。你可以将手牌中一种花色的所有牌交给一名其他角色，对其造成1点伤害。然后你于自己的下回合开始时摸等量的牌。
```js
spyanjiao: {
		audio: 2,
		enable: "phaseUse",
...
## sp_cuiyan 名字:手杀崔琰 势力:wei

### spyajun 名字:雅俊
描述: ①摸牌阶段，你令额定摸牌数+1。②出牌阶段开始时，你可以用一张本回合得到的牌与其他角色拼点。若你赢，则你可将其中一张拼点牌置于牌堆顶。若你没赢，你本回合的手牌上限-1。
```js
spyajun: {
		audio: 2,
		trigger: { player: "phaseUseBegin" },
...
### spzundi 名字:尊嫡
描述: 出牌阶段限一次，你可以弃置一张手牌并选择一名角色，然后你进行判定。若结果为：黑色，其摸三张牌；红色，其可以移动场上的一张牌。
```js
spzundi: {
		audio: 2,
		enable: "phaseUse",
...
## sp_huaman 名字:手杀花鬘 势力:shu

### spxiangzhen 名字:象阵
描述: 锁定技。①【南蛮入侵】对你无效。②当有角色使用的【南蛮入侵】结算结束后，若有角色因此牌受到过伤害，则你与伤害来源各摸一张牌。
```js
spxiangzhen: {
		trigger: { target: "useCardToBefore" },
		forced: true,
...
### spfangzong 名字:芳踪
描述: 锁定技。①你不能于出牌阶段使用具有伤害标签的牌指定攻击范围内的角色为目标。②攻击范围内包含你的角色不能使用具有伤害标签的牌指定你为目标。③结束阶段，你将手牌摸至X张（X为场上存活人数且至多为8）。
```js
spfangzong: {
		audio: 2,
...
### spxizhan 名字:嬉战
描述: 其他角色的回合开始时，你须选择一项：①失去1点体力。②弃置一张牌并令〖芳踪〗于本回合失效，然后若此牌的花色为：♠，其视为使用一张【酒】；♥，你视为使用一张【无中生有】；♣，你视为对其使用【铁索连环】；♦：你视为对其使用火【杀】（无距离限制）。
```js
spxizhan: {
		audio: 5,
		trigger: { global: "phaseBegin" },
...
## sp_gaolan 名字:手杀高览 势力:qun

### spjungong 名字:峻攻
描述: 出牌阶段，你可失去X+1点体力或弃置X+1张牌，视为对一名其他角色使用【杀】（不计入次数和距离限制，X为你本回合内发动过〖峻攻〗的次数）。若你因此【杀】造成了伤害，则你令此技能失效直到回合结束。
```js
spjungong: {
		audio: 2,
		enable: "phaseUse",
...
### spdengli 名字:等力
描述: 当你使用【杀】指定目标后，或成为【杀】的目标后，若使用者和目标的体力值相等，则你摸一张牌。
```js
spdengli: {
		audio: 2,
		trigger: {
...
### zaoli 名字:躁厉
描述: 锁定技。①你不能于出牌阶段使用或打出你手牌中不为本回合得到的牌。②当你使用或打出手牌时，你获得一个“厉”（至多4个）。③回合开始时，若你有“厉”，则你移去所有“厉”并弃置任意张牌，然后摸X+Y张牌。若X大于2，你失去1点体力（X为你移去的标记数，Y为你弃置的牌数）。
```js
zaoli: {
		trigger: { player: "phaseBegin" },
		audio: 2,
...
## sp_wangshuang 名字:手杀王双 势力:wei

### yiyong 名字:异勇
描述: 当你受到其他角色造成的渠道为【杀】的伤害后，若你的装备区内有武器牌，则你可以获得此【杀】对应的所有实体牌，然后将这些牌当做【杀】对伤害来源使用（无距离限制）。若其装备区内没有武器牌，则此伤害+1。
```js
yiyong: {
		audio: 2,
		trigger: { player: "damageEnd" },
...
### shanxie 名字:擅械
描述: ①出牌阶段限一次，你可从牌堆中获得一张武器牌。若牌堆中没有武器牌，则你改为随机获得一名角色装备区内的一张武器牌。②当其他角色使用【闪】响应你使用的【杀】时，若此【闪】没有点数或点数不大于你攻击范围的二倍，则你令此【闪】无效。
```js
shanxie: {
		audio: 2,
		enable: "phaseUse",
...
## sp_zongyu 名字:手杀宗预 势力:shu

### zhibian 名字:直辩
描述: `准备阶段，你可以和一名其他角色拼点。若你赢，你可选择：①将其装备区/判定区内的一张牌移动到你的对应区域。②回复1点体力。③${get.poptip("rule_beishui")}：跳过下个摸牌阶段；若你没赢，你失去1点体力。`
```js
zhibian: {
		audio: 2,
		trigger: { player: "phaseZhunbeiBegin" },
...
### yuyan 名字:御严
描述: 锁定技。当你成为非转换的【杀】的目标时，若使用者的体力值大于你且此【杀】有点数，则你令使用者选择一项：①交给你一张点数大于此【杀】的牌。②取消此目标。
```js
yuyan: {
		audio: 2,
		trigger: { target: "useCardToTarget" },
...
## yuanhuan 名字:袁涣 势力:wei

### qingjue 名字:请决
描述: 每轮限一次。当有其他角色A使用牌指定另一名体力值小于A且不处于濒死状态的其他角色B为唯一目标时，你可以摸一张牌，然后与A拼点。若你赢，你取消此目标。若你没赢，你将此牌的目标改为自己。
```js
qingjue: {
		audio: 2,
...
### fengjie 名字:奉节
描述: 锁定技，准备阶段开始时，你选择一名其他角色并获得如下效果直到你下回合开始：一名角色的结束阶段开始时，你将手牌摸至（至多摸至四张）或弃置至与其体力值相等。
```js
fengjie: {
		audio: 2,
		trigger: { player: "phaseZhunbeiBegin" },
...
## sp_chendong 名字:手杀陈武董袭 势力:wu

### spyilie 名字:毅烈
描述: `出牌阶段开始时，你可选择：①本阶段内使用【杀】的次数上限+1。②本回合内使用【杀】被【闪】抵消时，摸一张牌。③${get.poptip("rule_beishui")}：失去1点体力。`
```js
spyilie: {
		audio: 2,
		trigger: { player: "phaseUseBegin" },
...
### spfenming 名字:奋命
描述: 出牌阶段限一次，你可以选择一名体力值不大于你的角色。若其：未横置，其横置；已横置，你获得其一张牌。
```js
spfenming: {
		audio: 2,
		enable: "phaseUse",
...
## db_wenyang 名字:手杀文鸯 势力:wei

### dbquedi 名字:却敌
描述: `每回合限一次。当你使用【杀】或【决斗】指定唯一目标后，你可选择：①获得目标角色的一张手牌。②弃置一张基本牌，并令此牌的伤害值基数+1。③${get.poptip("rule_beishui")}：减1点体力上限。`
```js
dbquedi: {
		audio: 2,
		trigger: { player: "useCardToPlayered" },
...
### dbzhuifeng 名字:椎锋
描述: 魏势力技。每回合限两次，你可以失去1点体力并视为使用一张【决斗】（你死亡后仍然结算）。当你因此【决斗】而受到伤害时，你防止此伤害并令此技能失效直到出牌阶段结束。
```js
dbzhuifeng: {
		audio: 2,
		groupSkill: "wei",
...
### dbchongjian 名字:冲坚
描述: 吴势力技。你可以将一张装备牌当做一种【杀】（无距离限制且无视防具）或【酒】使用。当你以此法使用【杀】造成伤害后，你获得目标角色装备区内的X张牌（X为伤害值）。
```js
dbchongjian: {
		audio: 2,
		groupSkill: "wu",
...
### dbchoujue 名字:仇决
描述: 锁定技。当你杀死其他角色后，你加1点体力上限并摸两张牌，然后你本回合发动〖却敌〗的次数上限+1。
```js
dbchoujue: {
		audio: 2,
...
## sp_yanghu 名字:手杀羊祜 势力:qun

### mingfa 名字:明伐
描述: ①结束阶段，你可展示一张牌并记录为“明伐”。②出牌阶段开始时，若“明伐”牌在你的手牌区或装备区，则你可以使用“明伐”牌与一名其他角色拼点。若你赢：你获得对方一张牌并从牌堆中获得一张点数等于“明伐”牌牌面点数-1的牌。若你没赢：你本回合不能使用牌指定其他角色为目标。③你的拼点牌亮出后，你令此牌的点数+2。
```js
mingfa: {
		audio: 2,
		trigger: { player: "phaseUseBegin" },
...
### rongbei 名字:戎备
描述: 限定技。出牌阶段，你可选择一名有空装备栏的角色。系统为该角色的每个空装备栏选择一张装备牌，然后该角色使用之。
```js
rongbei: {
		audio: 2,
		enable: "phaseUse",
...
## qiaogong 名字:桥公 势力:wu

### yizhu 名字:遗珠
描述: ①结束阶段，你摸两张牌，然后将两张牌随机插入牌堆前2X张牌的位置中（X为角色数，选择牌的牌名对其他角色可见）。②其他角色使用“遗珠”牌指定唯一目标时，你可以取消此目标，然后你清除对应的“遗珠”标记。③当“遗珠”牌不因使用而进入弃牌堆后，你清除对应的“遗珠”标记。
```js
yizhu: {
		audio: 2,
		trigger: { player: "phaseJieshuBegin" },
...
### luanchou 名字:鸾俦
描述: 出牌阶段限一次，你可令两名角色获得“姻”标记并清除原有标记。拥有“姻”标记的角色视为拥有技能〖共患〗。
```js
luanchou: {
		audio: 2,
		enable: "phaseUse",
...
## liuzhang 名字:刘璋 势力:qun

### jutu 名字:据土
描述: `锁定技，准备阶段，你获得所有你武将牌上的“生”，然后摸X+1张牌，然后将X张牌置于你的武将牌上，称为“生”（X为你因${get.poptip("yaohu")}选择势力的角色数量)。`
```js
jutu: {
		audio: "xiusheng",
		trigger: { player: "phaseZhunbeiBegin" },
...
### yaohu 名字:邀虎
描述: 每轮限一次，你的回合开始时，你须选择场上一个势力。该势力其他角色的出牌阶段开始时，其获得你的一张“生”，然后其须选择一项：①对你指定的另一名的其他角色使用一张【杀】（无距离和次数限制）；②直到本阶段结束，其使用伤害牌指定你为目标时，除非交给你两张牌，否则取消之。
```js
yaohu: {
		audio: "yinlang",
		trigger: { player: "phaseBegin" },
...
### rehuaibi 名字:怀璧
描述: `主公技，锁定技，你的手牌上限+X（X为你因${get.poptip("yaohu")}选择势力的角色数量)。`
```js
rehuaibi: {
		audio: "huaibi",
		zhuSkill: true,
...
## sp_zhangwen 名字:手杀张温 势力:wu

### gebo 名字:戈帛
描述: 锁定技，当有角色回复体力后，你将牌堆顶的一张牌置入仁库。
```js
gebo: {
		audio: 2,
...
### spsongshu 名字:颂蜀
描述: 其他角色的摸牌阶段开始时，若其体力值大于你且仁库内有牌，则你可以令其放弃摸牌。其改为获得X张仁（X为你的体力值且至多为5），且本回合内不能使用牌指定其他角色为目标。
```js
spsongshu: {
		audio: 2,
...
## zhangzhongjing 名字:张机 势力:qun

### jishi 名字:济世
描述: 锁定技。①当你使用的牌结算完成后，若你未因此牌造成过伤害，则你将此牌对应的所有实体牌置于仁库中。②当有牌不因溢出而离开仁库时，你摸一张牌。
```js
jishi: {
		audio: 2,
		trigger: { player: "useCardAfter" },
...
### xinliaoyi 名字:疗疫
描述: 其他角色的回合开始时，你可选择一项：①令其从仁库中获得一张牌。②若其手牌数大于体力值，则令其将X张手牌置入仁库（X为其手牌数与体力值之差）。
```js
xinliaoyi: {
		audio: "liaoyi",
		trigger: { global: "phaseBegin" },
...
### binglun 名字:病论
描述: 出牌阶段限一次，你可以将仁库中的一张牌置于弃牌堆并选择一名角色。该角色选择一项：①摸一张牌。②于其下回合结束时回复1点体力。
```js
binglun: {
		audio: 2,
		enable: "phaseUse",
...
## sp_xujing 名字:手杀许靖 势力:shu

### boming 名字:博名
描述: 出牌阶段限两次，你可以将一张牌交给一名其他角色。结束阶段，若你本回合以此法失去了至少两张牌，则你摸一张牌。
```js
boming: {
		audio: 2,
		enable: "phaseUse",
...
### ejian 名字:恶荐
描述: 锁定技，每名角色限一次。当有其他角色因〖博名〗而得到了你的牌后，若其拥有与此牌类型相同的其他牌，则你令其选择一项：①受到1点伤害。②展示所有手牌，并弃置所有与此牌类别相同的牌。
```js
ejian: {
		audio: 2,
...
## sp_huaxin 名字:手杀华歆 势力:wei

### yuanqing 名字:渊清
描述: 锁定技，出牌阶段结束时，你随机将弃牌堆中你本回合使用过的牌类型的各一张牌置于仁库中。
```js
yuanqing: {
		audio: 2,
		trigger: { player: "phaseUseEnd" },
...
### shuchen 名字:疏陈
描述: 锁定技，当有角色进入濒死状态时，若仁库中的牌数大于三，则你获得仁库中的所有牌，然后其回复1点体力。
```js
shuchen: {
		audio: 2,
		init(player) {
...
## xiangchong 名字:向宠 势力:shu

### guying 名字:固营
描述: 锁定技。每回合限一次，当你于回合外失去牌后，若牌数为1，则你获得1枚“固”并令当前回合角色选择一项：①随机交给你一张牌。②令你获得本次失去的牌，若为装备牌，则你使用之。准备阶段开始时，你移去所有“固”并弃置等量的牌。
```js
guying: {
		audio: 2,
		trigger: {
...
### muzhen 名字:睦阵
描述: 出牌阶段各限一次。①你可以将至多两张牌交给一名装备区内有牌的其他角色，然后获得其装备区内的一张牌。②你可以将一张装备牌置于其他角色的装备区内，然后获得其一张手牌。③你可以选择任意名其他角色，这些角色手牌数和装备区牌数每有一项与你相同，其摸一张牌，若这些角色均摸了两张牌，你摸选择角色数张牌。
```js
muzhen: {
		audio: 2,
		enable: "phaseUse",
...
## caizhenji 名字:蔡贞姬 势力:wei

### sheyi 名字:舍裔
描述: 每轮限一次。当有体力值小于你的其他角色受到伤害时，你可以交给其至少X张牌并防止此伤害（X为你的体力值）。
```js
sheyi: {
		audio: 2,
...
### tianyin 名字:天音
描述: 锁定技，结束阶段开始时，你从牌堆中获得每种本回合未使用过的类型的牌各一张。
```js
tianyin: {
		audio: 2,
		trigger: { player: "phaseJieshuBegin" },
...
## sp_kongrong 名字:手杀孔融 势力:qun

### xinlirang 名字:礼让
描述: ①其他角色的摸牌阶段开始时，若你没有“谦”标记，则你可以获得一枚“谦”标记。若如此做，其额定摸牌数+2，且本回合的弃牌阶段结束时，你可以获得其弃置的至多两张牌。②准备阶段，若你有“谦”标记，则你跳过下个摸牌阶段并移除“谦”标记。
```js
xinlirang: {
		audio: "splirang",
...
### xinmingshi 名字:名仕
描述: 锁定技，当你受到伤害后，若你有“谦”标记，则伤害来源弃置其区域内的一张牌。若此牌为：黑色：你获得之。红色，你回复1点体力。
```js
xinmingshi: {
		audio: "spmingshi",
		trigger: { player: "damageEnd" },
...
## zhouchu 名字:手杀周处 势力:wu

### xianghai 名字:乡害
描述: 锁定技，其他角色的手牌上限-1。你手牌区的装备牌均视为【酒】。
```js
xianghai: {
		audio: 2,
		global: "xianghai_g",
...
### rechuhai 名字:除害
描述: `使命技。①出牌阶段限一次，你可以摸一张牌，然后和一名其他角色拼点。若你赢，则你观看其手牌，并从牌堆/弃牌堆中获得其手牌中包含的类型的牌各一张，且当你于此阶段内对其造成伤害后，你将牌堆/弃牌堆中的一张装备牌置于你的一个空置装备栏内。②当你因发动〖除害①〗而展示拼点牌时，你令此牌的点数+X（X=(4-你装备区的牌数)）。③使命：当有装备牌进入你的装备区后，若你的装备区内有至少三张牌，则你将体力值回复至上限，失去〖乡害〗并获得${get.poptip("zhangming")}。④失败：当你因发动〖除害①〗发起的拼点没赢时，若你的最终点数不大于6，则你触发使命失败分支。`
```js
rechuhai: {
		audio: "chuhai",
		dutySkill: true,
...
## wangfuzhaolei 名字:王甫赵累 势力:shu

### xunyi 名字:殉义
描述: ①游戏开始时，你令一名其他角色获得“义”；拥有“义”效果的角色死亡后，你可以令另一名其他角色获得“义”。②当你或“义”角色造成/受到1点伤害后，若受伤角色/伤害来源不为另一方，令另一方摸一张牌/弃置一张牌。
```js
xunyi: {
		audio: 2,
		trigger: {
...
## wangling 名字:王凌 势力:wei

### xingqi 名字:星启
描述: ①当你使用牌时，若此牌不为延时锦囊牌且你没有同名的“备”，则你获得一枚与此牌名称相同的“备”。②结束阶段，你可移去一枚“备”，然后从牌堆中获得一张与此“备”名称相同的牌。
```js
xingqi: {
		audio: 2,
...
### xinzifu 名字:自缚
描述: 锁定技。出牌阶段结束时，若你本阶段内未使用牌，则你移去所有“备”且本回合的手牌上限-1。
```js
xinzifu: {
		audio: "zifu",
		trigger: { player: "phaseUseEnd" },
...
### mibei 名字:秘备
描述: `使命技。①使命：当你使用的牌结算完成后，若你的“备”中包含的基本牌，锦囊牌，装备牌数量均大于1，则你从牌堆中获得这三种类型的牌各一张并获得技能${get.poptip("xinmouli")}。②失败：结束阶段开始时，若你没有“备”，且你于本回合的准备阶段开始时也没有“备”，则你减1点体力上限。`
```js
mibei: {
		audio: 2,
		trigger: { player: "useCardAfter" },
...
### heji 名字:合击
描述: 当有角色使用的【决斗】或红色【杀】结算完成后，若此牌对应的目标数为1，则你可以对相同的目标使用一张【杀】或【决斗】（无距离和次数限制）。若你以此法使用的牌不为转化牌，则你从牌堆中随机获得一张红色牌。
```js
heji: {
		audio: 2,
		trigger: { global: "useCardAfter" },
...
### liubing 名字:流兵
描述: 锁定技。①你于出牌阶段使用的第一张非虚拟【杀】的花色视为♦。②其他角色于其出牌阶段内使用的非转化黑色【杀】结算结束后，若此【杀】未造成伤害，则你获得之。
```js
liubing: {
		audio: 2,
		trigger: { player: "useCard1" },
...
### qingyu 名字:清玉
描述: `使命技，锁定技。①当你受到伤害时，你弃置两张牌，然后防止此伤害。②使命：准备阶段，若你的体力值等于体力上限且你的手牌数小于游戏轮数，则你获得${get.poptip("xuancun")}。③失败：当你进入濒死状态时，你减1点体力上限，令一名其他角色获得${get.poptip("mbyongjue")}。`
```js
qingyu: {
		audio: 3,
		dutySkill: true,
...
## sp_xinpi 名字:手杀辛毗 势力:wei

### spyinju 名字:引裾
描述: 出牌阶段限一次，你可令一名其他角色选择一项：①对你使用一张【杀】（无距离限制）。②其下个回合的准备阶段开始时，跳过出牌阶段和弃牌阶段。
```js
spyinju: {
		audio: 2,
		enable: "phaseUse",
...
### spchijie 名字:持节
描述: 每回合限一次。当你成为其他角色使用牌的唯一目标时，你可判定。若结果大于6，则你取消此牌的所有目标。
```js
spchijie: {
		audio: 2,
		trigger: { target: "useCardToTarget" },
...
## feiyi 名字:手杀费祎 势力:shu

### mjshengxi 名字:生息
描述: 准备阶段，你可以获得一张【调剂盐梅】；结束阶段，若你本回合使用过牌且未造成伤害，则你可以获得一张智囊或摸一张牌。
```js
mjshengxi: {
		audio: "shengxi",
		audioname: ["feiyi"],
...
### fyjianyu 名字:谏喻
描述: 每轮限一次。出牌阶段，你可选择两名角色，令这些角色获得“喻”直到你的下回合开始。当一名有“喻”的角色A于其出牌阶段内使用牌指定另一名有“喻”的角色B为目标时，你令B摸一张牌。
```js
fyjianyu: {
		initSkill(skill) {
			if (!lib.skill[skill]) {
...
## sp_bianfuren 名字:手杀卞夫人 势力:wei

### spwanwei 名字:挽危
描述: 每轮累计限一次。①出牌阶段，你可选择一名其他角色。②当有其他角色处于濒死状态时。你可令该角色回复X+1点体力（至少回复至1），然后你失去X点体力。（X为你的体力值）
```js
spwanwei: {
		audio: 2,
		enable: "chooseToUse",
...
### spyuejian 名字:约俭
描述: 你的手牌上限基数等于你的体力上限。当你进入濒死状态时，你可以弃置两张牌，然后回复1点体力。
```js
spyuejian: {
		mod: {
			maxHandcardBase(player) {
...
## sp_duyu 名字:手杀杜预 势力:qun

### spwuku 名字:武库
描述: 锁定技，当有角色使用装备牌时，若你的“武库”数小于3，则你获得一个“武库”。
```js
spwuku: {
		audio: 2,
		trigger: { global: "useCard" },
...
### spsanchen 名字:三陈
描述: `觉醒技，结束阶段，若你的“武库”数大于2，则你加1点体力上限并回复1点体力，然后获得${get.poptip("spmiewu")}。`
```js
spsanchen: {
		audio: 2,
		trigger: { player: "phaseJieshuBegin" },
...
## luotong 名字:手杀骆统 势力:wu

### qinzheng 名字:勤政
描述: 锁定技，当你使用或打出牌时，若你本局游戏内使用或打出过的牌数和：为3的倍数，你从牌堆中获得一张【杀】或【闪】；为5的倍数，你从牌堆中获得一张【桃】或【酒】；为8的倍数，你从牌堆中获得一张【决斗】或【无中生有】（可获得对应的衍生替换牌）。
```js
qinzheng: {
		audio: 2,
		trigger: { player: ["useCard", "respond"] },
...
## sp_wangcan 名字:手杀王粲 势力:wei

### spqiai 名字:七哀
描述: 出牌阶段限一次，你可以将一张非基本牌交给一名其他角色。然后其选择一项：①你回复1点体力。②你摸两张牌。
```js
spqiai: {
		audio: 2,
		enable: "phaseUse",
...
### spshanxi 名字:善檄
描述: 出牌阶段开始时，你可令一名其他角色获得“檄”标记并清除场上已有的其他“檄”标记（若有）。有“檄”标记的角色回复体力时，若其体力值大于0，则其需选择一项：①交给你两张牌。②失去1点体力。
```js
spshanxi: {
		audio: 2,
		init(player) {
...
## sp_chenzhen 名字:陈震 势力:shu

### shameng 名字:歃盟
描述: 出牌阶段限一次，你可弃置两张颜色相同的手牌并选择一名其他角色。其摸两张牌，然后你摸三张牌。
```js
shameng: {
		audio: 2,
		enable: "phaseUse",
...
## sp_sunshao 名字:手杀孙邵 势力:wu

### mbdingyi 名字:定仪
描述: 锁定技，游戏开始时，你选择一项并令任意名角色获得对应效果：1.摸牌阶段摸牌数+1；2.手牌上限+2；3.攻击范围+1；4.脱离濒死状态时回复1点体力。
```js
mbdingyi: {
		audio: "mjdingyi",
		forced: true,
...
### mbzuici 名字:罪辞
描述: `每回合限一次，你受到伤害后，可令一名有“定仪”效果的角色从牌堆获得一个你指定的${get.poptip("rule_zhinang")}，然后移去其所有“定仪”效果。`
```js
mbzuici: {
		audio: "zuici",
		usable: 1,
...
### mbfubi 名字:辅弼
描述: 出牌阶段限一次，你可选择一名角色并选择一项：1.令其获得一个其未拥有的“定仪”效果；2.令其全部“定仪”效果翻倍至你的下个出牌阶段开始。
```js
mbfubi: {
		audio: "fubi",
		enable: "phaseUse",
...
## sp_xunchen 名字:手杀荀谌 势力:qun

### mjweipo 名字:危迫
描述: 每回合限一次，出牌阶段，你可以选择一个智囊或【兵临城下】，令一名没有〖危迫〗效果的角色获得如下一次性效果直到你下回合开始：其可于出牌阶段弃置一张【杀】，并获得一张你选择的牌。
```js
mjweipo: {
		audio: 2,
		enable: "phaseUse",
...
### mjchenshi 名字:陈势
描述: 一名其他角色使用【兵临城下】指定第一个目标后，其可交给你一张牌，并将牌堆顶三张牌中所有不为【杀】的牌置入弃牌堆；一名其他角色成为【兵临城下】的目标后，其可交给你一张牌，然后将牌堆顶三张牌中所有的【杀】置入弃牌堆。
```js
mjchenshi: {
		audio: 2,
...
### mjmouzhi 名字:谋识
描述: 锁定技，当你受到伤害时，若伤害渠道对应的牌和你上次受到的伤害花色相同，则你防止此伤害。
```js
mjmouzhi: {
		audio: 2,
		trigger: { player: "damageBegin2" },
