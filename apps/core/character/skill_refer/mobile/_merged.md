# mobile merged reference

## mb_shen_caocao 名字:手杀神曹操 势力:shen
...
### potjiezhu 名字:竭逐
描述: 每回合限一次，你可将手牌弃置至最接近的唯一值，视为使用一张指定至多X名角色为目标，无距离限制的普通【杀】（X为弃置牌数）。此【杀】结算后，若此【杀】目标数为X且对所有目标造成伤害，你将手牌摸至最接近的唯一值。
```js
potjiezhu: {
		audio: 2,
...
### pothuanshi 名字:还施
描述: `使命技，你每回合使用首张【杀】伤害+1；你于非濒死状态时无法使用但可重铸【酒】。成功：你造成或受到伤害后，若本次伤害值等于你体力值，你获得${get.poptip("potjianlv")}。`
```js
pothuanshi: {
		audio: 3,
		dutySkill: true,
...
## mb_cheliji 名字:手杀彻里吉 势力:qun

### mbqucheng 名字:驱乘
描述: 锁定技，你使用【杀】无距离限制，且不因此技能使用的【杀】只能指定上家/下家为目标；当你使用指定唯一目标的【杀】结算后，若此【杀】目标角色的上家/下家不是你且未造成伤害，此【杀】不计入次数，然后你视为对其的上家/下家使用一张普通【杀】。
```js
mbqucheng: {
		audio: 2,
		forced: true,
...
### mbqiangyong 名字:羌勇
描述: 锁定技，你使用【杀】造成伤害时，弃置受伤角色的X张牌（X为本回合你使用【杀】的次数），然后若其没有手牌，此【杀】伤害+1。
```js
mbqiangyong: {
		audio: 4,
...
## mb_dingshangwan 名字:手杀丁尚涴 势力:wei

### mbzhaofu 名字:照抚
描述: 每回合限一次，你可弃置所有手牌视为使用一张：1.【杀】；2.【闪】；3.【酒】；4.【桃】。若你因此恰好弃置了序号数张牌，你可令一名其他角色获得一个对应效果的【照抚】，且其发动后或其回合结束时失去对应效果的【照抚】。
```js
mbzhaofu: {
		audio: 2,
		enable: "chooseToUse",
...
### mbqiliu 名字:泣留
描述: `锁定技，当你每轮首次因${get.poptip("mbzhaofu")}失去所有手牌后，你摸X张牌（X为你已执行过的回合数）；当你于回合外对其他角色使用【桃】后，防止其本回合受到的下一次伤害。若其因此防止了伤害，此技能本轮失效。`
```js
mbqiliu: {
		audio: 2,
		forced: true,
...
## re_wangji 名字:手杀界王基 势力:wei

### reqizhi 名字:奇制
描述: 蓄力技（1/2），出牌阶段每名角色限一次，你可以消耗1点蓄力点令一名角色摸两张牌，然后本阶段你使用基本牌或普通锦囊牌指定目标后，你依次观看本阶段因此摸牌的角色的手牌并弃置其一张牌。
```js
reqizhi: {
		audio: 4,
		logAudio: () => 2,
...
### rejinqu 名字:进趋
描述: 锁定技，你每于回合内使用两张基本牌或普通锦囊牌后，你令一名拥有蓄力技的角色获得1点蓄力点；结束阶段，你摸X张牌（X为本回合你蓄力点点数存在过的最高值）。
```js
rejinqu: {
		audio: 4,
...
## re_zhoufei 名字:手杀界周妃 势力:wu

### reliangyin 名字:良姻
描述: ①当有牌被置于武将牌上时，你可令一名角色摸一张牌。②当有角色获得一名角色武将牌上的牌时，你可令一名角色弃置一张牌。③每轮结束时，你可令本轮唯一成为此技能目标的角色回复1点体力，或令本轮唯一未成为此技能目标的角色失去1点体力。
```js
reliangyin: {
		audio: 4,
		logAudio: () => 2,
...
### rekongsheng 名字:箜声
描述: 准备阶段、当你失去或回复体力后，你可以将当前回合角色的任意张牌扣置于其武将牌上。若如此做，该角色的结束阶段，你可以使用其中一张牌，然后其获得其余的牌。当你死亡时，所有角色收回其“箜声”牌。
```js
rekongsheng: {
		audio: 2,
...
## re_yanyan 名字:手杀界严颜 势力:shu

### rejuzhan 名字:拒战
描述: `转换技，游戏开始时，若你的势力为蜀，此技能的初始状态改为阴；阳：当你或者此状态下第三次有角色成为【杀】的目标后，你可以与此【杀】的使用者各摸一张牌，然后其本回合不能对你使用牌；阴：当你或者此状态下第三次有角色使用【杀】指定目标后，你可以获得此【杀】的目标角色一张牌，然后你本回合不能对其使用牌。${get.poptip("rule_chengshi")}：你使用【杀】的次数上限+1，且此【杀】结算完毕后，你获得之。`
```js
rejuzhan: {
		audio: 4,
		addtip(player, skill) {
...
## mb_gaoxiang 名字:手杀高翔 势力:shu

### mbgxchiyuan 名字:驰援
描述: 出牌阶段限一次，你可以交给一名体力值小于你的角色一张牌，然后视为对攻击范围内包含其的一名角色使用一张【杀】。此【杀】结算后，若造成过伤害，其回复1点体力值；若未造成过伤害，目标角色依次视为对你使用一张【杀】。
```js
mbgxchiyuan: {
		audio: 2,
		enable: "phaseUse",
...
### mberdi 名字:饵敌
描述: 结束阶段，你可以指定一名手牌数大于你的角色，你失去1点体力值并摸一张牌，然后你与其同时弃置任意张手牌。此法弃置牌后，若你弃置的牌数：大于其，你回复1点体力值并对其造成1点伤害；小于其，你摸等同于其弃置的牌数。
```js
mberdi: {
		audio: 2,
...
## mb_cuimao 名字:手杀崔琰毛玠 势力:wei

### mbzhengbi 名字:征辟
描述: 准备阶段，你可以选择至多一名其他角色，其选择令你获得其区域里的一至三张牌，然后你交给其等量张牌。每名角色限一次，若其选择三，此技能可选择的角色数+1。
```js
mbzhengbi: {
		audio: 4,
...
### mbfengying 名字:奉迎
描述: 限定技，出牌阶段，你可以选择一名角色并令其攻击范围内的所有其他角色各将一张手牌当【无中生有】对其使用。若该角色体力上限为全场最大，此回合结束时你获得一个额外的回合。
```js
mbfengying: {
		audio: 4,
		logAudio: () => 2,
...
## mb_zhongyao 名字:手杀钟繇 势力:wei

### rehuomo
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### mbzuoding 名字:佐定
描述: 当其他角色于其出牌阶段内使用♠️牌指定目标后，若你或者目标角色于此阶段内没有受到过伤害，你可以令其中一名目标角色摸一张牌。
```js
mbzuoding: {
		audio: "zuoding",
...
## mb_wutugu 名字:手杀兀突骨 势力:qun

### mbranshang 名字:燃殇
描述: 锁定技，①当你受到1点火焰伤害后，你获得1枚“燃”标记。②结束阶段，你失去X点体力（X为你的“燃”标记数）；然后若你的“燃”数量大于1，你减少2点体力上限并摸两张牌。
```js
mbranshang: {
		audio: "ranshang",
...
### mbhanyong 名字:悍勇
描述: 当你使用牌指定的目标数大于1时，若你已受伤，你可以选择一项：1.摸两张牌；2.令此牌造成的伤害+1。背水：获得一个“燃”。
```js
mbhanyong: {
		audio: "hanyong",
		trigger: {
...
## mb_chengyu 名字:手杀程昱 势力:wei

### mbshefu 名字:设伏
描述: ①结束阶段，你可以记录一个未被记录的非装备牌名并扣置一张牌，称为“伏兵”。②你的回合外，当其他角色使用牌时，你可以移去一张记录名称相同的“伏兵”，令此牌无效并摸一张牌。
```js
mbshefu: {
		audio: "shefu",
		onremove(player, skill) {
...
### mbbenyu 名字:贲育
描述: 当你受到伤害后，若你的手牌数：小于X，你可以将手牌摸至X张（至多摸五张）；大于X，你可以弃置至少X+1张手牌，对伤害来源造成1点伤害（X为伤害来源的手牌数）。
```js
mbbenyu: {
		audio: "benyu",
		trigger: { player: "damageEnd" },
...
## mb_dongbai 名字:手杀董白 势力:qun

### mblianzhu 名字:连诛
描述: ①出牌阶段限一次，你可以展示一张牌并将此牌交给一名其他角色。若如此做，你对持有此牌的角色获得以下效果直至出牌阶段结束：1.你对其使用牌无距离次数限制；2.当你对其造成伤害后，你可以获得此牌。②一名其他角色于此阶段内因此技能获得此牌后，其须选择一项：1.交给你一张除此牌以外的其他牌，然后将此牌交给其上家角色；2.令你摸两张牌。
```js
mblianzhu: {
		audio: "lianzhu",
		enable: "phaseUse",
...
### mbxiahui 名字:黠慧
描述: 锁定技，①黑色牌不计入你的手牌上限；②当其他角色获得“连诛”牌或者你的黑色牌时，其不能使用、打出或弃置这些牌直到其体力值减少为止。
```js
mbxiahui: {
		audio: "xiahui",
		mod: {
...
## mb_zhugeguo 名字:手杀诸葛果 势力:shu

### mbqirang 名字:祈禳
描述: 当有装备牌进入你的装备区时，你可从牌堆中获得一张锦囊牌，你使用此锦囊牌时无距离限制并摸一张牌。
```js
mbqirang: {
		audio: "qirang",
...
### mbyuhua 名字:羽化
描述: 锁定技，你的非基本牌不计入手牌上限；结束阶段，若你的手牌数大于体力值，你观看牌堆顶X张牌（X为你手牌类别数），将其中任意数量的牌置于牌堆顶，其余的牌置于牌堆底。
```js
mbyuhua: {
		audio: "yuhua",
		trigger: { player: "phaseJieshuBegin" },
...
## mb_caochun 名字:手杀曹纯 势力:wei

### mbshanjia 名字:缮甲
描述: 出牌阶段限一次，你可以摸三张牌，然后弃置X张牌（X为3-你本局失去过的装备区里的牌数）。若你未因此弃置：基本牌，你可视为使用一张无任何次数限制的【杀】；锦囊牌，你本回合使用牌无距离限制。
```js
mbshanjia: {
		audio: "shanjia",
		enable: "phaseUse",
...
## mb_xiahoumao 名字:手杀夏侯楙 势力:wei

### mbtongwei 名字:统围
描述: `出牌阶段限一次，你可重铸手牌中任意张基本牌。然后你选择一项：①令所有手牌中有同名牌的角色弃置一张基本牌。②获得本回合因弃置进入弃牌堆的同名基本牌。③${get.poptip("rule_beishui")}：移除一个可触发${get.poptip("mbcuguo")}的牌名。`
```js
mbtongwei: {
		audio: 2,
		enable: "phaseUse",
...
### mbcuguo 名字:蹙国
描述: 锁定技，当你使用伤害牌指定目标后，弃置你与目标角色点数最大的手牌。若弃置了点数小于此牌的牌，此牌对所有目标额外结算一次。
```js
mbcuguo: {
		audio: 2,
...
## o_tadun 名字:缘蹋顿 势力:qun

### youlve 名字:游掠
描述: 锁定技，当你于回合内不以此法获得一张牌时，你摸一张牌；当你于回合外不以此法失去一张牌时，你弃置一张牌。
```js
youlve: {
		audio: 2,
		trigger: {
...
### lianxi 名字:连袭
描述: 【杀】不计入你的手牌上限；当你的【杀】因弃置进入弃牌堆时，你可以视为使用一张无距离和任何次数限制的【杀】。
```js
lianxi: {
		audio: 2,
		mod: {
...
## o_chenlan 名字:缘陈兰 势力:qun

### jujun 名字:据峻
描述: 锁定技，你成为过牌的目标且体力值变化过的阶段结束时，你视为使用一张【万箭齐发】。
```js
jujun: {
		audio: 1,
...
## o_meicheng 名字:缘梅成 势力:qun

### bixian 名字:壁险
描述: 锁定技，你成为过牌的目标且体力值未变化过的阶段结束时，你视为使用一张【决斗】。
```js
bixian: {
		audio: 1,
		trigger: { global: "phaseAnyEnd" },
...
## o_guanyu 名字:缘关羽 势力:shu

### mbwusheng 名字:武圣
描述: 你可以将任意张牌当无距离限制的任意【杀】使用或打出。此【杀】结算结束后，若你本回合首次以此数量转化牌，则你摸两张牌且此【杀】不计入次数限制。
```js
mbwusheng: {
		locked: false,
		mod: {
...
### mbyijue 名字:义绝
描述: 锁定技，当你对一名角色造成到大于等于其体力值的伤害时，其选择交给你任意张牌，然后你选择一项：1.本回合其不能使用或打出牌，且你弃置所有与这些牌花色相同的手牌；2.你防止此伤害，且本轮你与其使用牌指定对方为目标时，取消之。
```js
mbyijue: {
		audio: 4,
...
## o_sunquan 名字:缘孙权 势力:wu

### mbshizhong 名字:势众
描述: 锁定技，结束阶段，你摸一张牌并展示所有手牌；准备阶段，你摸X张牌（X为你手牌中剩余以此法展示过的牌数），若X与你上次以此法展示牌数相等，本回合你造成伤害时，将伤害值改为十万！
```js
mbshizhong: {
		audio: 3,
		logAudio: () => 2,
...
### mbcaowei 名字:操微
描述: 锁定技，当你受到伤害后，你重铸至少一个类别的所有牌并摸一张牌。
```js
mbcaowei: {
		audio: 2,
		trigger: { player: "damageEnd" },
...
## mb_tw_zhangji 名字:手杀张既 势力:wei

### mbdingzhen 名字:定镇
描述: 每轮开始时，你可令与你距离X以内的至多X名其他角色依次选择一项（X为你的体力值）：1.弃一张【杀】；2.本轮其于回合内不能指定你为锦囊牌的目标。
```js
mbdingzhen: {
		audio: "twdingzhen",
...
### mbyouye 名字:攸业
描述: 锁定技。①其他角色的结束阶段，若其本回合未对你造成过伤害且“蓄”数小于5，你将牌堆顶的牌置于武将牌上，称为“蓄”。②当你造成或受到伤害后，若你有“蓄”，你将所有“蓄”分配给任意角色。
```js
mbyouye: {
		audio: "twyouye",
...
## xin_simayi 名字:手杀神司马懿 势力:shen

### xinrenjie 名字:忍戒
描述: 锁定技。①当你需要响应一张延时锦囊牌或其他角色使用的牌时，若你未响应此牌，你获得1枚“忍”标记（你每轮以此法至多获得4枚“忍”标记）。②游戏开始时，若你为神势力且你未执行神武将势力选择，则你可以变更势力。
```js
xinrenjie: {
		audio: 2,
		trigger: {
...
### xinbaiyin 名字:拜印
描述: `觉醒技，准备阶段，若你的“忍”标记数不小于4，你减少1点体力上限，然后获得${get.poptip("xinjilve")}。`
```js
xinbaiyin: {
		audio: 2,
		inherit: "sbaiyin",
...
### xinlianpo 名字:连破
描述: 当你杀死一名角色后，你可以选择一项：1.于此回合结束后获得一个额外回合（每回合限一次）；2.若你拥有〖极略〗，你获得一个你未拥有的〖极略〗技能。
```js
xinlianpo: {
		audio: "lianpo",
...
## o_tanshihuai 名字:缘檀石槐 势力:qun

### mblianzhan 名字:连斩
描述: 你使用指定唯一目标的有点数的【杀】结算结束后，你可以对其使用一张点数更大的【杀】（无距离次数限制）并摸一张牌。
```js
mblianzhan: {
		audio: 2,
		trigger: {
...
### mbkoulve 名字:寇掠
描述: 锁定技，你于摸牌阶段外获得的手牌不计入手牌上限且只能当做【杀】使用或打出。
```js
mbkoulve: {
		audio: 2,
		forced: true,
...
## o_lvbu 名字:缘吕布 势力:qun

### mblvezhen 名字:掠阵
描述: 出牌阶段开始时，你可以令所有其他角色依次选择一项：1.展示X张手牌（X为其选择此选项的次数+1）；2.令你视为对其使用一张无距离和次数限制的【杀】。
```js
mblvezhen: {
		audio: 2,
		trigger: {
...
### mbhengwei 名字:横威
描述: 锁定技，当你对一名其他角色造成伤害时，除非其展示一张手牌并交给你，否则你令此伤害+1。你的回合内，其他角色展示牌后，其不能使用与展示牌相同颜色的手牌直到本回合结束。
```js
mbhengwei: {
		audio: 2,
		forced: true,
...
## o_gaoshun 名字:缘高顺 势力:qun

### mbjiren 名字:激刃
描述: 限定技，出牌阶段你可以发动此技能，直到本局游戏结束，所有角色使用非武器牌不能指定自己为目标。
```js
mbjiren: {
		audio: 2,
		limited: true,
...
### mbjuezhi 名字:决止
描述: 出牌阶段每项限一次：1.你可以摸两张牌，视为使用一张【决斗】，然后弃置手牌中所有非伤害牌；2.你可以摸两张牌，然后回复1点体力并弃置手牌中所有伤害牌。
```js
mbjuezhi: {
		audio: 4,
		enable: "phaseUse",
...
## hanxuan 名字:韩玄 势力:qun

### mbweizhan 名字:诿战
描述: 其他角色出牌阶段开始时，若你计算与其距离为1，你可以摸一张牌并交给其至多三张牌，本回合结束时若其本回合造成伤害值：不小于X，你摸两张牌；小于X，你获得其区域里两张牌（X为你交给其的牌数）。
```js
mbweizhan: {
		audio: 2,
		trigger: {
...
### mbyilv 名字:遗履
描述: 每回合限一次，一名角色使用【杀】指定你为目标后，你可与其谋奕。若你赢且你选择的选项为：“金蝉脱壳”，随机弃置其一张手牌，若为基本牌你获得之；“弃履狂奔”，此【杀】伤害-1。
```js
mbyilv: {
		audio: 4,
		usable: 1,
...
## hefei_zhangliao 名字:骥张辽 势力:wei

### hefeichonglei 名字:冲垒
描述: 锁定技。①你的出牌阶段内，所有其他角色的非基本手牌均只能作为【闪】使用或打出。②出牌阶段限X次（X为其他角色数），当你使用的牌被其他角色响应后，或当你响应其他角色使用的牌后，你获得该角色一张手牌。
```js
hefeichonglei: {
		audio: 2,
		forced: true,
...
### hefeidangshi 名字:荡势
描述: 当你使用伤害牌结算结束后，你可令一名其他目标角色选择一项：1.对你使用一张非转化、非虚拟且与此牌牌名相同的伤害牌；2.弃置X张牌（X为所有角色本轮选择此项的次数且至少为1）；3.受到你造成的1点伤害。若其选择了本阶段未被执行过的选项，你摸一张牌且本阶段出【杀】次数+1。
```js
hefeidangshi: {
		audio: 4,
		trigger: { player: "useCardAfter" },
...
## hefei_lidian 名字:骥李典 势力:wei

### hefeiduanjin 名字:断津
描述: 你使用基本牌结算结束后，可以弃置一名其他角色一张牌。
```js
hefeiduanjin: {
		audio: 2,
		trigger: {
...
### hefeigaigong 名字:慨公
描述: 每回合限一次，你对其他角色造成伤害后，或受到其他角色造成的伤害后，你可以展示你或其的至多两张手牌，然后交换这些牌与牌堆底等量张牌。若此交换流程中的牌包含至少两种花色，你可使用其中一张牌（无次数与距离限制）。
```js
hefeigaigong: {
		audio: 4,
		trigger: {
...
### hefeiheyulidian 名字:合御
描述: `锁定技，①若${get.poptip("characterx_hefei_yuejin")}在场且与你阵营相同，你获得因${get.poptip("hefeiduanjin")}弃置的牌。②若${get.poptip({
...
## hefei_yuejin 名字:骥乐进 势力:wei

### hefeixianjian 名字:陷坚
描述: 你使用非转化且非虚拟的【杀】指定唯一目标后，可以选择一项：1.摸一张牌，其弃置X张牌（X为目标场上牌数且至少为1）；2.此【杀】结算结束后，你将此【杀】置入其一个空置装备栏，称为“陷坚”牌。
```js
hefeixianjian: {
		audio: 2,
		trigger: { player: "useCardToPlayered" },
...
### hefeizherui 名字:折锐
描述: `①装备区有“陷坚”牌的角色使用非转化且非虚拟的【杀】指定目标后，你可以对其发动一次其选择选项的${get.poptip("hefeixianjian")}。②有角色失去装备栏里的一张“陷坚”牌后，你对其造成1点伤害。`
```js
hefeizherui: {
...
### hefeiheyuyuejin 名字:合御
描述: `锁定技，①若${get.poptip("characterx_hefei_zhangliao")}在场且与你阵营相同，你发动过${get.poptip("hefeixianjian")}的【杀】对一名角色造成伤害后，你可对其执行未选择的一项。②若${get.poptip("characterx_hefei_lidian")}在场且与你阵营相同，每回合首次有角色失去“陷坚”牌后，你摸一张牌。`
```js
hefeiheyuyuejin: {
		audio: 2,
		trigger: {
...
## mb_liuhui 名字:数刘徽 势力:qun

### mbgeyuan 名字:割圆
描述: 当你使用点数为X的牌时（X为圆周率中小数点后第一位的值），你可以摸Y张牌并令X改为后移一位的值（Y为你本回合本技能发动的次数+1，且至多为π)。
```js
mbgeyuan: {
		audio: 2,
		init(player, skill) {
...
### mbchongcha 名字:重差
描述: ①你的点数大于等于10的牌不计入手牌上限，且使用时视为满足〖割圆〗中的点数0。②出牌阶段限一次，你可以弃置一张牌，令〖割圆〗中的X改为后移一位的值。
```js
mbchongcha: {
		audio: 2,
		mod: {
...
## mb_caozhi 名字:御曹植 势力:wei

### mbchongsi 名字:冲司
描述: `出牌阶段，若你本轮未选择过最后一项，你可选择一名其他角色并执行一项，然后令其选择一项执行：1.使用一张【杀】；2.弃置两张手牌；3.对自己或装备${get.poptip("cz_liulongcanjia")}的角色造成1点伤害。`
```js
mbchongsi: {
		audio: 2,
		enable: "phaseUse",
...
### mbpeidong 名字:辔东
描述: `你可按以下方式操作${get.poptip("cz_liulongcanjia")}并视为使用对应牌：【杀】，从装备区移动至下家；【闪】，从其他角色场上获得；【桃】，从手牌或牌堆中亮出；【酒】，从游戏外置入宝物栏。`
```js
mbpeidong: {
		audio: 4,
		logAudio(event) {
...
## mb_zhouyu 名字:手杀乐周瑜 势力:wu

### mbshouyue 名字:授乐
描述: `摸牌阶段开始时，或你的体力值减少后，你可选择一项：1.摸一张牌，令一名角色获得${get.poptip("qinyin")}（已有则改为摸一张牌）；2.令一名角色复原武将牌。`
```js
mbshouyue: {
		audio: 2,
...
### mbdieyin 名字:叠音
描述: 出牌阶段限一次，你可将武将牌翻至背面，于此阶段结束后执行一个指定的额外阶段。
```js
mbdieyin: {
		audio: 2,
		enable: "phaseUse",
...
## mb_zhangzhi 名字:书张芝 势力:qun

### mbshiju 名字:势举
描述: `锁定技，你使用牌结算结束后，若此牌与上一张被使用的牌：1.类型相同，你获得牌堆顶一张牌；2.花色相同，你获得牌堆底一张牌；${get.poptip("rule_chengshi")}：若牌名也相同，你获得或升级${get.poptip("mbkubai")}。`
```js
mbshiju: {
		audio: 3,
		trigger: {
...
## mb_cuilingyi 名字:手杀崔令仪 势力:wei

### mbcaiqiu 名字:裁裘
描述: ①每轮开始时，你观看牌堆顶X张牌（X为游戏人数），然后可以获得其中任意张牌。②每轮限一次，其他角色使用牌结算结束后，若你本轮因〖裁裘〗获得过同名牌，你失去1点体力。
```js
mbcaiqiu: {
		audio: 4,
		logAudio: () => 2,
...
### mbxishang 名字:袭裳
描述: `锁定技，①游戏开始时，你选择本局形象并获得${get.poptip("mbweizhuang")}。②你不因摸牌而获得牌时，明置之。`
```js
mbxishang: {
		audio: 8,
		logAudio(event, player) {
...
## mb_luyu 名字:礼卢毓 势力:wei

### mbbingfa 名字:秉法
描述: `每轮开始时，你可选择两条${get.poptip({
		id: "bingfa_lvfa",
		name: "律法",
		type: "character",
...
### mbshuxing 名字:束刑
描述: 你的回合内每名角色限一次，一名其他角色成为【杀】的目标时，你可令此【杀】对其无效并展示其手牌，若其中有【闪】，你令其选择一项：1.失去1点体力；2.交给你其手牌中【闪】和其下次〖秉法〗的投票权。
```js
mbshuxing: {
		audio: 2,
		trigger: {
...
## mb_caocao 名字:手杀SP曹操 势力:qun

### mblingfa 名字:令法
描述: `①第一轮开始时，你可令本轮其他角色使用【杀】时，其需弃置一张牌，否则你对其造成1点伤害。②第二轮游戏开始时，你可令本轮其他角色使用【桃】结算结束后，其需交给你一张牌，否则你对其造成1点伤害。③非前两轮游戏开始时，你失去〖令法〗并获得${get.poptip("twzhian")}和${get.poptip("new_rejianxiong")}。`
```js
mblingfa: {
		audio: 4,
		trigger: { global: "roundStart" },
...
## mb_caohong 名字:手杀曹洪 势力:wei

### mbyuanhu 名字:援护
描述: `①游戏开始时，你获得${get.poptip("twjuezhu")}。②出牌阶段限一次。你可将一张装备牌置入一名角色的装备区内。若此牌为：武器牌，你弃置与其距离为1的另一名角色区域的一张牌；防具牌，其摸一张牌；坐骑牌，其回复1点体力；宝物牌，你选择基本牌或普通锦囊牌从牌堆中获得一张，其获得另一类型的一张牌。然后若其体力值或手牌数不大于你，则你可摸一张牌，且你可以于结束阶段发动一次〖援护〗。`
```js
mbyuanhu: {
		audio: "yuanhu",
		enable: "phaseUse",
...
## re_baosanniang 名字:手杀鲍三娘 势力:shu

### mbshuyong 名字:姝勇
描述: 当你使用或打出【杀】时，你可以获得一名其他角色区域内的一张牌。若本轮你以此法获得其区域内的牌数大于1，其摸一张牌。
```js
mbshuyong: {
		audio: "meiyong",
		trigger: {
...
### mbxushen 名字:许身
描述: `限定技，出牌阶段，你可以摸至多三张牌并失去等量体力，然后若你因此进入濒死状态，则当你脱离此次濒死状态后，你可以将${get.poptip("new_rewusheng")}${get.poptip("redangxian")}${get.poptip("rezhiman")}分配给本次濒死令你回复过体力的角色（若其已拥有，改为摸三张牌）。`
```js
mbxushen: {
		limited: true,
		audio: "xinfu_xushen",
...
### mbzhennan 名字:镇南
描述: 一名角色使用普通锦囊牌指定首个目标后，若此牌目标包含你且此牌目标数大于X（X为此牌使用者的体力值且至少为1），则你可以弃置一张牌并对一名角色造成1点伤害。
```js
mbzhennan: {
		audio: "xinfu_zhennan",
		trigger: { global: "useCardToPlayered" },
...
## mb_zerong 名字:手杀笮融 势力:qun

### mbfutu 名字:浮图
描述: ①一名角色的回合结束时，若你本回合造成的伤害值/令任意角色回复的体力值为全场最高，你将牌堆顶首张黑色/红色牌置于武将牌上，称为“业”。②你受到伤害时，可以弃置一张“业”并防止此伤害。
```js
mbfutu: {
		audio: 8,
...
### mbjingtu 名字:净土
描述: `限定技，出牌阶段，你可选择一项：1.获得所有黑色“业”并对一名角色造成等量伤害；2.获得所有红色“业”，令一名角色增加等量点体力上限并恢复等量体力；3.${get.poptip("rule_beishui")}，两种颜色的“业”数量相同。然后你失去〖浮图〗并获得${get.poptip("mbfozong")}。`
```js
mbjingtu: {
		audio: 6,
		enable: "phaseUse",
...
### mbjiebian 名字:劫辩
描述: 一名角色的出牌阶段结束时，若本阶段没有角色受到过伤害，你可与当前回合角色或体力值最低的角色拼点（你因此拼点时可以使用“业”进行拼点），若你赢，你选择一项：1.对没赢的角色造成1点伤害；2.令没赢的角色恢复1点体力并摸一张牌，然后获得其至多两张牌。
```js
mbjiebian: {
		audio: 2,
		trigger: {
...
## mb_mengda 名字:手杀孟达 势力:qun

### mbjili 名字:积戾
描述: 一名其他角色的回合开始时，若其在你的攻击范围内，你可以从0~2中秘密选择一个本轮未选择过的数字X。若如此做，本回合结束阶段，若其本回合使用牌指定你为目标的次数：小于X，你摸4-X张牌；等于X，你交给其X张牌；大于X，你可以视为对其使用一张无距离限制的【杀】。
```js
mbjili: {
		audio: 9,
...
### mbshishu 名字:恃术
描述: 锁定技，一名其他角色于其回合内获得你的牌后，你令其选择一项：1.弃置这些牌；2.交给你一张与这些牌类型均不同的牌。
```js
mbshishu: {
		audio: 2,
		trigger: {
...
## mb_caoxing 名字:射曹性 势力:qun

### mbjinzu 名字:劲镞
描述: 出牌阶段限一次，你可以选择一名其他角色，与其分别同时展示一/两张手牌，若你的展示牌点数在所有牌中：为最值，你弃置所有展示牌，重置此技能；不为唯一最值，你本回合使用的下一张【杀】对其造成伤害+1且其不可响应。
```js
mbjinzu: {
		audio: 2,
		enable: "phaseUse",
...
### mbanxian 名字:暗弦
描述: 你每回合首次弃置手牌时，可以获得并使用其中的一张【杀】（无距离次数限制）。
```js
mbanxian: {
		audio: 2,
		trigger: {
...
## mb_guanyinping 名字:手杀关银屏 势力:shu

### mbxuehen 名字:雪恨
描述: 当你每回合首次造成或受到伤害后，你可展示至多X张手牌（X为你已损失体力值），这些牌只能当不计入次数且无次数限制的【杀】使用或打出，直到你使用这些牌造成伤害。
```js
mbxuehen: {
		audio: "xueji",
		trigger: {
...
### mbhuxiao 名字:虎啸
描述: `出牌阶段限一次，你可选择一项：1.对一名体力值大于等于你的角色造成1点火焰伤害：2.本回合使用牌无距离限制；3.${get.poptip("rule_beishui")}：弃置一张红色牌。`
```js
mbhuxiao: {
		audio: "huxiao",
		enable: "phaseUse",
...
### mbwuji 名字:武继
描述: 限定技，出牌阶段，你可修改〖雪恨〗直到本阶段结束。此阶段结束时，若你因〖雪恨〗获得大于等于两张牌，你改为永久修改〖雪恨〗。
```js
mbwuji: {
		audio: "wuji",
		enable: "phaseUse",
...
## mb_xingdaorong 名字:手杀邢道荣 势力:qun

### mbkuangwu 名字:诳武
描述: 其他角色的出牌阶段开始时，你可将手牌调整至与其手牌数相同（至多摸至五张，弃至一张），然后视为对其使用一张【决斗】。此【决斗】结算结束后，若其未因此牌受到伤害，则你失去1点体力且此技能失效直到本轮结束。
```js
mbkuangwu: {
		audio: 4,
...
### mbanda 名字:谙达
描述: 每轮限一次，当一名角色进入濒死状态时，你可令伤害来源交给其两张颜色不同的牌，否则该角色回复1点体力。
```js
mbanda: {
		audio: 2,
...
### mbzhuguo 名字:助国
描述: 出牌阶段限一次，你可令一名角色将手牌调整至X（X为其体力上限且至多为5）。然后若其未摸牌，其回复1点体力，否则若其手牌数全场最多，你可以令其选择是否对你指定的另一名角色使用一张无任何次数和距离限制的【杀】。
```js
mbzhuguo: {
		audio: 3,
...
## mb_simazhao 名字:玄司马昭 势力:wei

### mbxiezheng 名字:挟征
描述: 你的登场势力为魏。结束阶段，你可以令一名角色将随机一张手牌置于牌堆顶，然后你视为使用一张【兵临城下】。然后若此牌若未造成伤害，你失去1点体力。
```js
mbxiezheng: {
		audio: "jsrgxiezheng",
...
### mbqiantun 名字:谦吞
描述: 魏势力技。出牌阶段限一次，你可以令一名其他角色展示至少一张手牌，然后与其拼点，其本次拼点只能从展示牌中选择。若你赢，你获得其展示的手牌；若你没赢，你获得其未展示的手牌。然后你展示手牌。
```js
mbqiantun: {
		audio: "jsrgqiantun",
...
### mbweisi 名字:威肆
描述: 群势力技。出牌阶段限一次，你可以选择一名其他角色，令其将任意张手牌移出游戏直到回合结束，然后视为对其使用一张【决斗】；此牌对其造成伤害后，你获得其所有手牌。
```js
mbweisi: {
		audio: "jsrgweisi",
...
### mbzhaoxiong 名字:昭凶
描述: `${get.poptip("rule_chihengji")}，限定技。准备阶段，若你已受伤，你可以变更势力为群，然后获得技能${get.poptip("mbdangyi")}。`
```js
mbzhaoxiong: {
		audio: "jsrgzhaoxiong",
...
## friend_cuijun 名字:友崔均 势力:qun

### friendshunyi 名字:顺逸
描述: 当你使用点数最小的手牌时，若此牌为红桃且此牌点数大于你本回合发动此技能的次数，则你可以将所有与此花色相同的手牌扣置于武将牌上直到回合结束（无此花色手牌则跳过此步骤），然后摸一张牌。
```js
friendshunyi: {
		audio: 2,
		trigger: { player: "useCard" },
...
### friendbiwei 名字:鄙位
描述: 出牌阶段限一次，你可以弃置手牌中点数最大的手牌并令一名其他角色弃置大于等于此牌点数的所有手牌，若其未因此弃牌，则你重置此技能。
```js
friendbiwei: {
		audio: 2,
		enable: "phaseUse",
...
### friendgongli_cuijun 名字:共砺
描述: 锁定技，游戏开始时，你为〖顺逸〗增加X个可触发花色（X为场上的友系列武将数）。
```js
friendgongli_cuijun: {
		audio: 2,
		trigger: {
...
## friend_shitao 名字:友石韬 势力:qun

### friendqinying 名字:钦英
描述: 出牌阶段限一次，你可以重铸任意张牌，视为使用一张【决斗】。此【决斗】结算流程中限X次（X为你以此法重铸的牌数），你或目标角色可以弃置自己区域里的一张牌，视为打出一张【杀】。
```js
friendqinying: {
		audio: 4,
		logAudio: () => 2,
...
### friendlunxiong 名字:论雄
描述: 当你造成或受到伤害后，你可以弃置手牌中点数最大的牌并摸三张牌，本局游戏你再次发动此技能弃置牌的点数须大于等于此牌的点数。
```js
friendlunxiong: {
		audio: 2,
		trigger: {
...
### friendgongli_shitao 名字:共砺
描述: 锁定技，游戏开始时，你为〖钦英〗减少X个可弃置类别（X为场上的友系列武将数）。
```js
friendgongli_shitao: {
		audio: 2,
		trigger: {
...
## mb_qinghegongzhu 名字:手杀清河公主 势力:wei

### mbzengou 名字:谮构
描述: 出牌阶段限一次，你可以观看一名其他角色的手牌，然后你选择一项：①视为使用两张其手牌中未拥有的基本牌牌名的牌（不计入次数且无次数限制且不可重复）；②将你与其手牌中共同牌名的牌替换为牌堆中等量的【杀】，你的这些牌不计入手牌上限直到你的结束阶段。然后你选择一个基本牌牌名，令其获得1枚此牌名对应的“诬”标记（每回合使用的前三张牌结算完毕后，若拥有此牌名的“诬”标记，则失去1点体力并移去1枚此牌名的“诬”标记）。
```js
mbzengou: {
		audio: 3,
		enable: "phaseUse",
...
### mbfeili 名字:诽离
描述: 当你受到伤害时，若你拥有〖谮构〗，则你可以弃置两张牌并防止此伤害；若此伤害存在来源且伤害来源拥有“诬”标记，则你可以改为移去其“诬”标记并防止此伤害，你摸四张牌且你不能再对其发动〖谮构〗。
```js
mbfeili: {
		audio: 2,
...
## friend_pangtong 名字:友庞统 势力:qun

### friendmanjuan 名字:漫卷
描述: 每轮限五次。当你不因本技能一次性获得至少两张牌后，你可以将这些牌中的任意张牌以任意顺序置于牌堆顶。若如此做，你每放置一张牌，便从弃牌堆中随机获得一张与此牌类别不同的牌（至多获得五张）。
```js
friendmanjuan: {
		audio: 2,
		trigger: {
...
### friendyangming 名字:养名
描述: 出牌阶段结束时，若你本阶段失去过至少两张手牌，则你可以将牌堆顶的四张牌置于你的武将牌上，然后使用其中任意张花色各不相同的牌（无次数限制），将剩余牌置入弃牌堆。
```js
friendyangming: {
		audio: 2,
...
### friendpangtonggongli 名字:共砺
描述: 锁定技，若友诸葛亮在场且确认与你阵营一致，你发动〖养名〗亮出牌的张数+1；若友徐庶在场且确认与你阵营一致，你发动〖养名〗后，获得一张本次亮出牌中未使用过的花色的牌。
```js
...
## friend_xushu 名字:友徐庶 势力:qun

### friendxiaxing 名字:侠行
描述: `游戏开始时，你获得并使用${get.poptip("xuanjian")}；当${get.poptip("xuanjian")}进入弃牌堆时，你可以移除2个“启诲”标记并获得之。`
```js
friendxiaxing: {
		audio: 2,
		trigger: {
...
### friendqihui 名字:启诲
描述: 锁定技。当你使用牌时，你获得1个此牌对应类别的“启诲”标记（每个类别标记上限为1），然后若你至少拥有3个“启诲”标记，你移去2或3个“启诲”标记并选择一项执行：①回复1点体力；②摸两张牌；③使用的下一张牌无任何次数限制；④令一名其他角色摸三张牌。若选择移除3个则每个类别的标记上限+1。
```js
friendqihui: {
		audio: 3,
		trigger: { player: "useCard" },
...
### friendxushugongli 名字:共砺
描述: 锁定技，若友诸葛亮在场且确认与你阵营一致，你发动〖玄剑〗改为将一张手牌当【杀】使用；若友庞统在场且确认与你阵营一致、你发动〖玄剑〗使用的【杀】无距离限制。
```js
friendxushugongli: {
		audio: 2,
		locked: true,
...
## friend_zhugeliang 名字:友诸葛亮 势力:qun

### friendyance 名字:演策
描述: `每轮限一次，首轮开始时或准备阶段，你可以选择一项：①从牌堆中随机获得一张锦囊牌；②执行${get.poptip("friendyance_minigame")}。`
```js
friendyance: {
		audio: 7,
		trigger: {
...
### friendfangqiu 名字:方遒
描述: `限定技，当你执行${get.poptip("friendyance_minigame")}后，你可以展示你的${get.poptip("friendyance_minigame")}预测。若如此做，本次${get.poptip("friendyance_minigame")}的预测全部验证后，执行“[]”中效果的值均+1，然后若${get.poptip("friendyance_minigame")}预测的牌数大于等于3且预测全部正确，该技能可以再次发动。`
```js
friendfangqiu: {
		audio: 3,
...
### friendzhugelianggongli 名字:共砺
描述: `锁定技，若友庞统在场且确认与你阵营一致，你执行${get.poptip("friendyance_minigame")}初始可预测的牌数+1；若友徐庶在场且确认与你阵营一致，你执行${get.poptip("friendyance_minigame")}预测的第一张牌的结果始终视为正确。`
```js
...
## mb_xuezong 名字:手杀薛综 势力:wu

### mbfunan 名字:复难
描述: 其他角色使用或打出牌响应你使用的牌时，你可以获得其使用或打出的牌。
```js
mbfunan: {
		audio: "funan",
...
### mbjiexun 名字:诫训
描述: `结束阶段，你可以选择一种花色，令一名其他角色摸等同于场上此花色数量的牌（至多为5），然后弃置X张牌（X为此技能发动次数）。若其以此法弃置了所有手牌，你修改${get.poptip("mbfunan")}`
```js
mbjiexun: {
		getSuitsMap() {
			const suits = {};
...
## mb_ganfuren 名字:手杀甘夫人 势力:shu

### mbzhijie 名字:智诫
描述: 每轮限一次，一名角色的出牌阶段开始时，你可以展示其一张手牌。本阶段其使用与此牌类别相同的牌后，其摸一张牌并弃置X张牌（X为其本回合使用此牌类型的次数-1）；此阶段结束时，若其阶段因此获得的牌数大于因此弃置的牌数，你与其各摸一张牌。
```js
mbzhijie: {
		audio: 2,
		trigger: { global: "phaseUseBegin" },
...
### mbshushen 名字:淑慎
描述: 每回合每项各限一次。当你回复体力时，你可以令一名其他角色摸两张牌；当你获得至少两张牌后，你可以令一名其他角色回复1点体力。
```js
mbshushen: {
		audio: 2,
		trigger: {
...
## mb_sp_zhenji 名字:手杀SP甄宓 势力:qun

### mbbojian 名字:博鉴
描述: 锁定技。出牌阶段结束时，若你本阶段使用牌数与花色数与上个出牌阶段皆不同（没有上个出牌阶段则均视为0），你摸一张牌；否则你将一张弃牌堆中本阶段你因使用而失去的牌交给一名角色。
```js
mbbojian: {
		audio: 2,
		init(player, skill) {
...
### mbjiwei 名字:济危
描述: 锁定技。①其他角色的回合结束时，你摸X张牌（X为本回合满足的项数：1.至少[1]名角色失去过牌；2.至少[1]名角色受到过伤害）。②准备阶段，若所有角色均存活且你的手牌数不小于5，你将手牌中颜色较多的牌分配给任意名其他角色。
```js
mbjiwei: {
		audio: 4,
		getNum(event, player) {
...
## mb_zhangfen 名字:手杀张奋 势力:wu

### mbquchong 名字:渠冲
描述: `①出牌阶段，你可以重铸装备牌。②一名角色的回合结束时，你将弃牌堆中的所有装备移出游戏，然后获得等量的铸造值。③出牌阶段开始时，若场上：没有【大攻车】，则你可以依次消耗0、2、5、5点铸造值，从${get.poptip("dagongche_attack")}和${get.poptip("dagongche_defend")}中选择一个令一名角色获得（你任选此牌花色和点数）并令其使用此牌；有【大攻车】，则你可以将场上的一张【大攻车】交给不为此牌拥有者的一名角色并令其使用此牌。`
```js
mbquchong: {
		audio: 4,
...
### mbxunjie 名字:逊节
描述: 锁定技，当你受到有来源的伤害时，若其体力值大于你且场上没有【大攻车】，则你进行一次判定，若结果为红色，则此伤害-1。
```js
mbxunjie: {
		audio: 2,
		trigger: { player: "damageBegin4" },
...
## mb_jiachong 名字:手杀贾充 势力:qun

### mbbeini 名字:悖逆
描述: 出牌阶段限一次，你可以选择一名体力值不小于你的其他角色，令你或其摸两张牌，然后未摸牌的角色选择一项：1.视为对摸牌的角色使用一张【杀】；2.获得摸牌的角色场上的一张牌。
```js
mbbeini: {
		audio: "beini",
		inherit: "beini",
...
### mbdingfa 名字:定法
描述: 弃牌阶段结束时，若本回合你失去的牌数不小于4，你可以选择一项：1、回复1点体力；2、弃置一名角色至多两张牌。
```js
mbdingfa: {
		audio: "dingfa",
...
## mb_simazhou 名字:手杀司马伷 势力:wei

### mbbifeng 名字:避锋
描述: 你成为一张基本牌或普通锦囊牌的目标时，若此牌目标数不大于4，你可取消之。若如此做，此牌结算完成后若没有其他角色响应此牌，你失去1点体力，否则你摸两张牌。
```js
mbbifeng: {
		audio: 3,
...
### mbsuwang 名字:宿望
描述: 一名角色的回合结束时，若其此回合对你使用过牌且你未受到过伤害，则你将牌堆顶的一张牌置于武将牌上。摸牌阶段开始时，你可以改为获得武将牌上的“宿望”牌，然后你可以令一名其他角色摸两张牌。
```js
mbsuwang: {
		audio: 2,
		trigger: { global: "phaseEnd" },
...
## mb_wenqin 名字:玄文钦 势力:wei

### mbbeiming 名字:孛明
描述: 游戏开始时，你可令至多两名角色获得牌堆中一张攻击范围为X的武器牌（X为其手牌中的花色数）。
```js
mbbeiming: {
		audio: 2,
		trigger: {
...
### mbchoumang 名字:仇铓
描述: `每回合限一次，你使用【杀】指定唯一目标或成为【杀】的唯一目标后，可以选择一项：①令此【杀】伤害+1；②若此【杀】被【闪】抵消，你可以获得你距离1以内的一名其他角色区域里的一张牌；③${get.poptip("rule_beishui")}：弃置你与其装备栏里的武器牌。`
```js
mbchoumang: {
		audio: 2,
		trigger: {
...
## mb_simafu 名字:手杀司马孚 势力:wei

### mbpanxiang 名字:蹒襄
描述: 当一名角色受到伤害时，你可以选择一项（不能与上次你因其发动此技能时选择的选项相同）：⒈令此伤害-1，伤害来源摸两张牌；⒉令此伤害+1，其摸三张牌。
```js
mbpanxiang: {
		initSkill(skill) {
			if (!lib.skill[skill]) {
...
### mbchenjie 名字:臣节
描述: 锁定技。当一名角色死亡后，若你有〖蹒襄〗且其成为过你〖蹒襄〗的目标，你弃置区域里的所有牌，摸四张牌。
```js
mbchenjie: {
		audio: 2,
...
## mb_sp_guanqiujian 名字:玄毌丘俭 势力:wei

### mbcuizhen 名字:摧阵
描述: ①游戏开始时，你可以废除至多三名其他角色的武器栏。②当你于出牌阶段使用伤害类牌指定其他角色为目标后，若目标角色的手牌数不小于体力值，你可以废除其武器栏。③摸牌阶段，你令额定摸牌数+X（X为所有角色被废除的武器栏数之和+1，至多为3）。
```js
mbcuizhen: {
		audio: 2,
		trigger: {
...
### mbkuili 名字:溃离
描述: 锁定技。当你受到伤害后，若来源有被废除的武器栏，你令其恢复武器栏。
```js
mbkuili: {
		audio: 2,
		trigger: {
...
## mb_caomao 名字:手杀曹髦 势力:wei

### mbqianlong 名字:潜龙
描述: `${get.poptip("rule_chihengji")}。①游戏开始时，你获得20枚“道心”标记。②当你得到牌后/受到1点伤害后/造成1点伤害后，你获得5/10/15枚“道心”（上限为99枚）。③若你的“道心”数不小于25/50/75/99，你视为拥有${get.poptip("mbcmqingzheng")}/${get.poptip("mbcmjiushi")}/${get.poptip("mbcmfangzhu")}/${get.poptip("mbjuejin")}。`
```js
mbqianlong: {
		audio: 6,
		persevereSkill: true,
...
### mbweitong 名字:卫统
描述: `${get.poptip("rule_chihengji")}，主公技。游戏开始时，若你有〖潜龙〗且场上有其他魏势力角色，你因“潜龙”于游戏开始时获得的“道心”标记数修改为60枚。`
```js
mbweitong: {
		audio: 1,
		persevereSkill: true,
...
## chengji 名字:成济 势力:wei

### mbkuangli 名字:狂戾
描述: 锁定技。①出牌阶段开始时，你随机令场上任意名其他角色获得“狂戾”标记。②出牌阶段限两次，当你使用牌指定有“狂戾”的角色为目标后，你随机弃置你与其各一张牌，然后你摸两张牌。③回合结束时，你移除所有角色的“狂戾”。
```js
mbkuangli: {
		audio: 2,
		trigger: { player: "phaseUseBegin" },
...
### mbxiongsi 名字:凶肆
描述: 限定技。出牌阶段，若你的手牌数不少于三张，你可以弃置所有手牌，然后令所有其他角色依次失去1点体力。
```js
mbxiongsi: {
		audio: 2,
		enable: "phaseUse",
...
## lizhaojiaobo 名字:李昭焦伯 势力:wei

### mbzuoyou 名字:佐佑
描述: 转换技。出牌阶段限一次，阳：你可以令一名角色摸三张牌，然后其弃置两张手牌；阴：你可以令一名角色获得1点护甲。
```js
mbzuoyou: {
		audio: 2,
		enable: "phaseUse",
...
### mbshishou 名字:侍守
描述: 锁定技。当你发动〖佐佑〗后，若目标角色不为你，你执行〖佐佑〗中目标角色未执行的一项。
```js
mbshishou: {
		audio: 2,
...
## yangfeng 名字:杨奉 势力:qun

### mbxuetu 名字:血途
描述: 转换技。出牌阶段限一次，阳：你可以令一名角色回复1点体力；阴：你可以令一名角色摸两张牌。
```js
mbxuetu: {
		audio: 4,
		enable: "phaseUse",
...
### mbweiming 名字:威命
描述: 使命技，锁定技。①出牌阶段开始时，你记录一名未以此法记录过的其他角色。②成功：当你杀死一名未被〖威命①〗记录过的角色后，修改〖血途〗为成功版本。③失败：当一名被〖威命①〗记录过的角色死亡后，你修改〖血途〗为失败版本。
```js
mbweiming: {
		audio: 3,
		dutySkill: true,
...
## xin_huojun 名字:手杀霍峻 势力:shu

### sidai 名字:伺怠
描述: 限定技，出牌阶段，你可以将手牌区内的所有基本牌当做【杀】使用。若此牌对应的实体牌中：包含【闪】，则目标角色成为此牌的目标后，需弃置一张基本牌，否则不可响应此牌；包含【桃】，则当目标角色受到此牌的伤害后，其减1点体力上限。
```js
sidai: {
...
### jieyu 名字:竭御
描述: 结束阶段，你可以从弃牌堆中获得共X张不同牌名的基本牌（X为3-你上次发动〖竭御〗至今你成为其他角色使用伤害类卡牌目标的次数，且X至少为1）。
```js
jieyu: {
...
## muludawang 名字:木鹿大王 势力:qun

### shoufa 名字:兽法
描述: ①当你于一回合首次造成伤害后，你可以选择一名距离2以内的角色。②每回合限五次，当你受到伤害后，你可以选择一名与你距离不小于2的角色。其随机执行以下一项：豹，其受到1点无来源伤害；鹰，你随机获得其一张牌；熊，你随机弃置其装备区的一张牌；兔，其摸一张牌。
```js
shoufa: {
...
### zhoulin 名字:咒鳞
描述: `限定技，出牌阶段，你可以获得2点护甲值，然后选择一个“兽法”效果，你发动${get.poptip("shoufa")}的执行效果改为你选择的效果直到你的下个回合开始。`
```js
zhoulin: {
		audio: 2,
		limited: true,
...
### yuxiang 名字:御象
描述: 锁定技，若你有护甲值，则：①你计算与其他角色的距离-1，其他角色计算与你的距离+1；②当你受到火焰伤害时，此伤害+1。
```js
yuxiang: {
		mod: {
...
## mb_chengui 名字:手杀陈珪 势力:qun

### guimou 名字:诡谋
描述: 锁定技。游戏开始时/回合结束时，你随机/须选择以下一项直到你的下个准备阶段：①记录场上期间角色使用牌数；②记录期间场上角色弃置牌数；③记录期间场上角色获得牌数。准备阶段，你可以选择一名场上对应记录数值最少的其他角色，观看其手牌并选择其中一张牌，然后你将此牌交给另一名其他角色或弃置此牌。
```js
guimou: {
		audio: 2,
		trigger: {
...
### zhouxian 名字:州贤
描述: 锁定技。当你成为其他角色使用的伤害类卡牌的目标时，你亮出牌堆顶的三张牌，然后其须选择一项：①弃置一张与亮出牌之一类别相同的牌；②取消你为此牌的目标。
```js
zhouxian: {
		audio: 2,
		trigger: { target: "useCardToTarget" },
...
## mb_huban 名字:手杀胡班 势力:wei

### mbyilie 名字:义烈
描述: 锁定技。①游戏开始时，你选择一名其他角色，然后你获得以下效果：其受到伤害时，若你没有“烈”，则你获得等同于伤害值的“烈”标记，然后防止此伤害；其对其他角色造成伤害后，你回复1点体力。②结束阶段，若你拥有“烈”标记，你摸一张牌并失去X点体力，然后移去所有“烈”（X为你拥有的“烈”标记数）。
```js
mbyilie: {
		audio: 3,
...
## mb_xianglang 名字:手杀向朗 势力:shu

### naxue 名字:纳学
描述: 你可以跳过出牌阶段。若如此做，你可以弃置任意张牌并摸等量的牌，然后你可以交给至多两名其他角色各一张手牌。
```js
naxue: {
		audio: 2,
...
### yijie 名字:遗诫
描述: 锁定技。当你死亡时，你令所有其他角色将体力回复或失去至X（X为所有其他角色的体力之和除以所有其他角色数，向下取整，且X至少为1）。
```js
yijie: {
		audio: 2,
		trigger: { player: "die" },
...
## yanxiang 名字:阎象 势力:qun

### kujian 名字:苦谏
描述: 出牌阶段限一次，你可以将至多两张手牌称为“谏”并交给一名其他角色，然后你获得以下效果：其他角色失去“谏”后，若为因使用或打出而失去，你与其各摸两张牌；否则你与其各弃置一张牌。
```js
kujian: {
		audio: "twkujian",
		inherit: "twkujian",
...
### ruilian 名字:睿敛
描述: 每轮开始时，你可以选择一名角色。其下回合结束时，若其本回合弃置过牌，你可以选择其本回合弃置过的一种类别，你与其各从弃牌堆中获得一张此类别的牌。
```js
ruilian: {
		audio: "twruilian",
		trigger: { global: "roundStart" },
...
## mb_sunluyu 名字:手杀孙鲁育 势力:wu

### mbmeibu 名字:魅步
描述: `其他角色的出牌阶段开始时，若你在其攻击范围内，你可以弃置一张牌，令该角色于本回合内获得${get.poptip("mbzhixi")}。若你以此法弃置的牌不是【杀】或黑色锦囊牌，则本回合其与你的距离视为1。`
```js
mbmeibu: {
		inherit: "new_meibu",
		derivation: ["mbzhixi"],
...
### mbmumu 名字:穆穆
描述: 出牌阶段开始时，你可以选择一项：1.弃置场上的一张装备牌；2.获得场上的一张防具牌，然后你本回合不能使用或打出【杀】。
```js
mbmumu: {
		audio: "mumu",
		inherit: "new_mumu",
...
## xin_wuban 名字:吴班 势力:shu

### xinjintao 名字:进讨
描述: 锁定技，你使用【杀】无距离限制且次数上限+1。你于出牌阶段内使用的第一张【杀】不可被响应，第二张【杀】伤害+1。
```js
xinjintao: {
		audio: "jintao",
		inherit: "jintao",
...
### mutao 名字:募讨
描述: 出牌阶段限一次。你可以选择一名角色，令其将手牌中所有的【杀】置于武将牌上，然后将这些牌依次随机交给其下家开始的每一名角色。然后其对最后一名以此法获得【杀】的角色A造成X点伤害（X为A手牌中【杀】的数量且至多为2）。
```js
mutao: {
		audio: "twmutao",
		inherit: "twmutao",
...
### yimou 名字:毅谋
描述: 当与你距离1以内的一名角色受到伤害后，你可以选择一项：1.令其从牌堆中获得一张【杀】；2.令其将一张手牌交给另一名角色，然后摸一张牌。
```js
yimou: {
...
## jiangji 名字:蒋济 势力:wei

### twjichou
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### jilun 名字:机论
描述: 当你受到伤害后，若你拥有技能〖急筹〗，你可以摸X张牌，或视为使用一张未被〖机论〗记录过的基本牌或〖急筹①〗记录过且未被〖机论〗记录过的合法目标数不大于X的普通锦囊牌并记录此牌（X为〖急筹①〗记录数且至少为1，至多为3）。
```js
jilun: {
		audio: 2,
		trigger: { player: "damageEnd" },
...
### jiaohua 名字:教化
描述: 出牌阶段限两次，你可以选择一个未被〖教化〗记录过的牌的类型，令一名角色从牌堆中获得一张此类型的牌，然后记录此类型，若基本、锦囊、装备均已被你发动〖教化〗记录，则你清空〖教化〗记录。
```js
jiaohua: {
		onremove: true,
		audio: "twjiaohua",
...
### laishou 名字:来寿
描述: 锁定技。①当你受到伤害值大于等于你的体力值的伤害时，若你的体力上限小于9，你防止此伤害并增加等量体力上限。②准备阶段，若你的体力上限不小于9，你死亡。
```js
laishou: {
		audio: 3,
...
### luanqun 名字:乱群
描述: 出牌阶段限一次，若你有手牌，则你可以令所有有手牌的角色同时展示一张手牌，然后你可以获得其中至多两张与你展示的牌颜色相同的展示牌，所有本次展示牌颜色与你展示的牌颜色不同的角色的下个出牌阶段使用的第一张【杀】只能对你使用，且你无法响应其于回合内使用的【杀】。
```js
luanqun: {
		audio: 2,
		enable: "phaseUse",
...
## xin_guozhao 名字:手杀郭照 势力:wei

### yichong 名字:易宠
描述: ①准备阶段，你可以选择一名其他角色并选择一个花色，然后你获得其所有此花色的牌，移除场上的所有“雀”标记，令其获得“雀”标记直到你的下个回合开始。②拥有“雀”标记的角色获得你最后一次发动〖易宠①〗选择的花色的牌后，你获得这些牌（你至多通过每个“雀”得到一张牌）。
```js
yichong: {
		initSkill(skill) {
			if (!lib.skill[skill]) {
...
### wufei 名字:诬诽
描述: 若场上存在拥有“雀”标记的角色A，则：①你即将由【杀】或伤害类普通锦囊牌造成的伤害的来源为A。②当你受到伤害后，若A的体力值大于3且大于你，则你可以令A受到1点无来源伤害。
```js
wufei: {
		audio: 2,
...
## xin_zhangyi 名字:手杀界张嶷 势力:shu

### xinwurong 名字:怃戎
描述: 出牌阶段限一次，你可以与一名其他角色进行谋弈：<br><li>若你选择“镇压”且其选择“反抗”，你对其造成1点伤害，然后你摸一张牌。<br><li>若你选择“安抚”且其选择“归顺”，其须交给你两张牌（若其牌数不足两张，则改为令其跳过其下个摸牌阶段）。<br><li>若你选择“镇压”且其选择“归顺”，你获得其一张牌，然后你交给其两张牌。<br><li>若你选择“安抚”且其选择“反抗”，你受到1点伤害，然后你摸一张牌。
```js
xinwurong: {
		audio: 3,
		enable: "phaseUse",
...
## xin_sunliang 名字:手杀孙亮 势力:wu

### xinzhizheng 名字:掣政
描述: 锁定技，你的出牌阶段内，攻击范围内不包含你的其他角色不能成为你使用牌的目标。出牌阶段结束时，若你本阶段内使用的牌数小于这些角色的数量，则你弃置其中一名角色的一张牌。
```js
xinzhizheng: {
		audio: "nzry_zhizheng",
		mod: {
...
### xinkuizhu 名字:溃诛
描述: 弃牌阶段结束后，你可以选择一项：1.令至多X名角色各摸一张牌。2.对任意名体力值之和为X的角色造成1点伤害，若你以此法选择的角色数不小于2，你失去1点体力。（X为你此阶段弃置的牌数）
```js
xinkuizhu: {
		audio: "nzry_kuizhu",
		trigger: { player: "phaseDiscardAfter" },
...
### xinlijun 名字:立军
描述: 主公技，其他吴势力角色于其回合内使用【杀】结算完毕后，其可以将此【杀】对应的实体牌交给你，然后你可以令其摸一张牌。
```js
xinlijun: {
		audio: "nzry_lijun1",
		trigger: { global: "useCardAfter" },
...
## shichangshi 名字:十常侍 势力:qun

### mbdanggu 名字:党锢
描述: 锁定技。①游戏开始时，你获得十张“常侍”牌，然后你进行一次结党。②当你修整结束后，你进行一次结党并摸一张牌。③若你有亮出的“常侍”牌，你视为拥有这些牌的技能。
```js
mbdanggu: {
		trigger: {
			player: "enterGame",
...
## re_zhangzhang 名字:手杀界张昭张纮 势力:wu
...
## qianzhao 名字:手杀牵招 势力:wei

### mbshihe 名字:势吓
描述: 出牌阶段限一次。你可以与一名角色拼点。若你：赢，当其于其下回合结束前对你和你的友方角色造成伤害时，取消之；没赢，你随机弃置一张牌。
```js
mbshihe: {
		audio: 2,
		enable: "phaseUse",
...
### mbzhenfu 名字:镇抚
描述: 结束阶段，若你本回合因弃置失去过牌，你可以令一名其他角色获得1点护甲。
```js
mbzhenfu: {
		audio: 2,
		trigger: { player: "phaseJieshuBegin" },
...
## re_yanwen 名字:手杀界颜良文丑 势力:qun

### reshuangxiong
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

## xin_zhoutai 名字:手杀界周泰 势力:wu
...
## xin_caoxiu 名字:手杀界曹休 势力:wei

### qianju
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### xinqingxi 名字:倾袭
描述: 每回合限一次。当你对其他角色造成伤害时，你可以令其选择一项：1.弃置X张手牌（X为4减去你至其的距离，至少为1）；2.令此伤害+1。
```js
xinqingxi: {
		audio: 2,
		usable: 1,
...
## xin_zhuhuan 名字:手杀界朱桓 势力:wu

### fenli
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### xinpingkou 名字:平寇
描述: 回合结束时，你可以对至多X名其他角色各造成1点伤害（X为你本回合跳过的阶段数）。然后你从牌堆中获得一张装备牌。
```js
xinpingkou: {
		audio: 2,
		trigger: { player: "phaseEnd" },
...
## sp_pengyang 名字:手杀彭羕 势力:shu

### spdaming 名字:达命
描述: ①游戏开始时，你获得3点“达命”值。②其他角色A的出牌阶段限一次。其可以交给你一张牌，然后你选择另一名其他角色B。若B有与此牌相同类型的牌，其将一张该类型的牌交给A，你获得1点“达命”值；否则你将此牌交给A。
```js
spdaming: {
		audio: 3,
		trigger: { global: "phaseBefore", player: "enterGame" },
...
### spxiaoni 名字:嚣逆
描述: ①出牌阶段限一次。若你的“达命”值大于0，你可以将一张牌当任意一种【杀】或伤害类锦囊牌使用。然后你减少等同于此牌指定目标数的“达命”值。②你的手牌上限基数为“达命”值和体力值中较小的一项，【杀】的次数上限为其中较大的一项。
```js
spxiaoni: {
		audio: 2,
		enable: "phaseUse",
...
## wangjun 名字:手杀王濬 势力:qun

### zhujian 名字:筑舰
描述: 出牌阶段限一次。你可以令至少两名装备区里有牌的角色各摸一张牌。
```js
zhujian: {
		audio: 2,
		enable: "phaseUse",
...
### duansuo 名字:断索
描述: 出牌阶段限一次。你可以重置任意名处于连环状态的角色，然后对这些角色各造成1点火焰伤害。
```js
duansuo: {
		audio: 2,
		enable: "phaseUse",
...
## xin_zhuzhi 名字:手杀界朱治 势力:wu

### sbanguo 名字:安国
描述: ①游戏开始时，你令一名其他角色获得1枚“安国”标记（有“安国”的角色手牌上限基数等于体力上限）。②出牌阶段开始时，你可以将一名有“安国”的角色的所有“安国”移动给一名本局游戏未获得过“安国”的其他角色。③当你受到伤害时，若有有“安国”的角色且伤害值不小于你的体力值且此伤害没有来源或来源没有“安国”，防止此伤害。④一名角色进入濒死状态时，若其有你因〖安国①〗获得的“安国”，你移去其该“安国”，令其将体力回复至1点。然后你选择一项：1.若你的体力值大于1，你失去体力至1点；2.若你的体力上限大于1，你将体力上限减至1。最后你令其获得1点护甲。
```js
sbanguo: {
		audio: 3,
		trigger: { global: "phaseBefore", player: "enterGame" },
...
## xin_wuyi 名字:手杀界吴懿 势力:shu

### sbbenxi 名字:奔袭
描述: 出牌阶段开始时，你可以弃置至少一张牌，然后你于此阶段获得如下效果：①你至其他角色距离-X；②当你使用的下一张基本牌或普通锦囊牌A选择目标后，你可以额外指定至多X名距离为1的角色为目标；③牌A结算结束后，若此牌造成过伤害，你摸五张牌（X为你以此法弃置的牌数）。
```js
sbbenxi: {
		audio: 3,
		trigger: { player: "phaseUseBegin" },
...
### jiebing 名字:借兵
描述: 锁定技。当你受到伤害后，你选择来源外的一名其他角色，随机获得其一张牌并展示。若此牌为装备牌，你使用之。
```js
jiebing: {
		audio: 2,
		trigger: {
...
### hannan 名字:扞难
描述: 出牌阶段限一次。你可以与一名角色拼点，赢的角色对没赢的角色造成1点伤害。
```js
hannan: {
		audio: 2,
		enable: "phaseUse",
...
## sp_caosong 名字:手杀曹嵩 势力:wei

### yijin 名字:亿金
描述: 锁定技。①游戏开始时，你获得“膴仕”、“金迷”、“贾凶”、“通神”、“拥蔽”、“厚任”各1枚（均称为“金”）。②出牌阶段开始时，你选择一名没有“金”的其他角色。你交给其1枚“金”，且令其获得对应效果。③一名角色的回合结束后，若其有你交给其的“金”，其移去此“金”。④当你死亡时，移去场上所有你交出的“金”。⑤回合开始时，若你没有“金”，你死亡。
```js
yijin: {
		audio: 3,
		trigger: { player: "phaseUseBegin" },
...
### guanzong 名字:惯纵
描述: 出牌阶段限一次。你可以令一名其他角色视为对另一名其他角色造成过1点伤害。
```js
guanzong: {
		audio: 2,
		enable: "phaseUse",
...
## re_dianwei 名字:手杀界典韦 势力:wei

### reqiangxi
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

## xin_mamidi 名字:手杀马日磾 势力:qun

### chengye 名字:承业
描述: `锁定技。①其他角色使用的非转化牌结算结束后，或其他角色的装备牌和延时锦囊牌进入弃牌堆后，或有延时锦囊牌因其他角色执行判定阶段的流程而进入弃牌堆后，若你的${get.poptip({
		id: "chengye_liujing",
		name: "六经",
		type: "character",
...
### buxu 名字:补续
描述: `出牌阶段，若你拥有${get.poptip("chengye")}且你的${get.poptip("chengye_liujing")}中有空缺的位置，则你可以弃置X+1张牌并选择一种空缺的${get.poptip("chengye_liujing")}（X为你本阶段内发动过〖补续〗成功填补${get.poptip("chengye_liujing")}的次数）。系统从牌堆中检索一张对应的牌，然后你将此牌置于你武将牌上，填补${get.poptip("chengye_liujing")}的对应位置。`
```js
buxu: {
		audio: 2,
		enable: "phaseUse",
...
## ruanhui 名字:阮慧 势力:wei

### mingcha 名字:明察
描述: 摸牌阶段开始时，你亮出牌堆顶的三张牌。若这三张牌中有点数小于9的牌，则你可以放弃摸牌并获得这些牌，然后你可以获得一名其他角色的随机一张牌。
```js
mingcha: {
		audio: 2,
		trigger: { player: "phaseDrawBegin1" },
...
### jingzhong 名字:敬重
描述: 弃牌阶段结束时，若你于本阶段内弃置过至少两张黑色牌，则你可以选择一名其他角色并获得如下效果直到其回合结束：每阶段限三次，其于出牌阶段内使用的牌结算结束后，你获得此牌对应的实体牌。
```js
jingzhong: {
		audio: 2,
		trigger: { player: "phaseDiscardAfter" },
...
## xin_quancong 名字:手杀界全琮 势力:wu

### sbyaoming 名字:邀名
描述: 蓄力技（2/4）。①当你受到1点伤害后，你获得1点蓄力值。②出牌阶段或当你受到伤害后，你可消耗1点蓄力值并选择一项：⒈弃置一名手牌数不小于你的其他角色的一张牌。⒉令一名手牌数不大于你的角色摸一张牌。若你上次发动〖邀名②〗时未获得过蓄力值且你选择的选项和上次不同，则你获得1点蓄力值。
```js
sbyaoming: {
		init(player, skill) {
...
## re_xunyu 名字:手杀界荀彧 势力:wei

### quhu
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### rejieming 名字:节命
描述: 当你受到1点伤害后，你可以令一名角色摸两张牌。然后若其手牌数小于体力上限，则你摸一张牌。
```js
rejieming: {
		audio: 2,
		trigger: { player: "damageEnd" },
...
## xin_jushou 名字:手杀界沮授 势力:qun

### xinjianying 名字:渐营
描述: ①当你使用与你本阶段使用的上一张牌点数或花色相同的牌时，你可以摸一张牌。②出牌阶段限一次，你可以将一张牌当做任意基本牌使用。若你于此阶段内使用的上一张牌有花色，则此牌的花色视为上一张牌的花色。
```js
xinjianying: {
		audio: 2,
		subfrequent: ["draw"],
...
## re_bulianshi 名字:手杀界步练师 势力:wu

### reanxu 名字:安恤
描述: 出牌阶段限一次，你可以选择两名其他角色，令其中一名角色获得另一名角色的一张牌。若以此法移动的牌不来自装备区，则你摸一张牌。然后你可以令二者中手牌数较少的一名角色摸一张牌。
```js
reanxu: {
		audio: 2,
		enable: "phaseUse",
...
## sp_jianggan 名字:手杀蒋干 势力:wei

### mbdaoshu 名字:盗书
描述: 出牌阶段限一次，你可以选择一名手牌数不少于两张的敌方角色，其随机获得三个牌名并将一张手牌的牌名伪装成其中一个与原牌名不同的牌名，然后你和队友观看其手牌并猜测其伪装的手牌，猜对的角色对其造成1点伤害，猜错的角色随机弃置两张手牌（手牌数不足两张则改为失去1点体力）。
```js
mbdaoshu: {
		audio: 3,
		enable: "phaseUse",
...
### spdaizui 名字:戴罪
描述: 限定技。当你受到伤害值不小于体力值的伤害时，你可防止此伤害并将此伤害渠道对应的所有实体牌置于伤害来源的武将牌上，称为“释”。本回合结束时，其获得所有“释”。
```js
spdaizui: {
		audio: 2,
		trigger: { player: "damageBegin2" },
...
## peixiu 名字:手杀裴秀 势力:qun

### xingtu 名字:行图
描述: 锁定技。你使用点数为X的倍数的牌无次数限制，你使用点数为X的约数的牌时摸一张牌（X为你本局游戏使用的上一张牌的点数）。
```js
xingtu: {
		audio: 2,
		trigger: { player: "useCard" },
...
### juezhi 名字:爵制
描述: 出牌阶段，你可以弃置至少两张牌，然后从牌堆中获得一张点数为Y的牌（Y为这些牌的点数和对13取余，余数为0时Y取13）。
```js
juezhi: {
		audio: 2,
		enable: "phaseUse",
...
## re_wuguotai 名字:手杀界吴国太 势力:wu

### reganlu 名字:甘露
描述: 出牌阶段限一次，你可以选择装备区牌数之差的绝对值不大于X的两名角色或包含你在内的两名角色，然后交换这两名角色装备区内的牌。（X为你已损失的体力值）
```js
reganlu: {
		enable: "phaseUse",
		usable: 1,
...
## xin_sunxiu 名字:手杀界孙休 势力:wu

### mobileyanzhu 名字:宴诛
描述: 出牌阶段限一次，你可以令一名有牌的其他角色选择一项：①你获得其装备区里所有的牌，然后你失去技能〖宴诛〗并修改技能〖兴学〗。②你获得其区域里的一张牌。
```js
mobileyanzhu: {
		audio: 2,
		enable: "phaseUse",
...
### mobilexingxue 名字:兴学
描述: 结束阶段开始时，你可以令至多X名角色依次摸一张牌并将一张牌置于牌堆顶（X为你的体力值）。
```js
mobilexingxue: {
		audio: 2,
		trigger: { player: "phaseJieshuBegin" },
...
## sp_maojie 名字:毛玠 势力:wei

### bingqing 名字:秉清
描述: 当你于出牌阶段内使用的牌结算完毕后，若你于本阶段内此前使用的所有牌的花色与此牌花色均不相同，则你可根据X的值执行对应效果：为2，你令一名角色摸两张牌；为3，你弃置一名角色区域内的一张牌；为4，你对一名其他角色造成1点伤害。（X为你于本阶段内此前使用的所有牌包含的花色数+1）
```js
bingqing: {
		init(player, skill) {
			player.addSkill(skill + "_mark");
...
### yingfeng 名字:迎奉
描述: 准备阶段，你可以令一名角色获得“奉”标记并移除场上所有其他的“奉”标记。有“奉”标记的角色使用牌没有距离限制。
```js
yingfeng: {
		audio: 2,
		trigger: { player: "phaseZhunbeiBegin" },
...
## re_yufan 名字:手杀界虞翻 势力:wu

### zhiyan
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### rezongxuan 名字:纵玄
描述: 当你的牌因弃置而进入弃牌堆后，你可以将其中任意张牌以任意顺序置于牌堆顶。出牌阶段限一次，你可以摸一张牌，然后将一张牌置于牌堆顶。
```js
rezongxuan: {
		inherit: "zongxuan",
...
## sunhanhua 名字:手杀孙寒华 势力:wu

### chongxu 名字:冲虚
描述: 出牌阶段限一次，你可以随机演奏一首音乐，并根据完成度来获得相应的分数（至多五分）。然后你可修改〖妙剑〗或〖莲华〗（消耗3分），并使用剩余的分数进行摸牌（每张2分）。
```js
chongxu: {
		audio: 2,
		enable: "phaseUse",
...
### miaojian 名字:妙剑
描述: 出牌阶段限一次。你可将一张【杀】当做刺【杀】使用，或将一张锦囊牌当做【无中生有】使用。
```js
miaojian: {
		audio: 2,
		enable: "phaseUse",
...
## mayuanyi 名字:马元义 势力:qun

### jibing 名字:集兵
描述: ①摸牌阶段开始时，若你的“兵”数小于势力数，则你可以改为将牌堆顶的两张牌置于你的武将牌上，称为“兵”。②你可以将一张“兵”当做【杀】或【闪】使用或打出。
```js
jibing: {
		audio: 2,
		enable: ["chooseToUse", "chooseToRespond"],
...
### wangjing 名字:往京
描述: 锁定技。当你因〖集兵〗而使用或打出牌时，若对方是场上体力值最高的角色，则你摸一张牌。
```js
wangjing: {
		audio: 2,
...
### moucuan 名字:谋篡
描述: `觉醒技。准备阶段，若你的“兵”数不小于势力数，则你减1点体力上限并获得${get.poptip("binghuo")}。`
```js
moucuan: {
		audio: 2,
		derivation: "binghuo",
...
### jueyong 名字:绝勇
描述: 锁定技。①当你不因〖绝勇〗成为唯一牌的目标时，若此牌不为转化牌且对应的实体牌牌数为1且不为【桃】或【酒】且你的“绝”数小于你的体力值，则你将此牌置于你的武将牌上，称为“绝”，且取消此牌的目标。②结束阶段开始时，若你有“绝”，则你令所有“绝”的原使用者依次对你使用所有“绝”，将无法使用的“绝”置入弃牌堆。
```js
jueyong: {
		audio: 2,
		trigger: { target: "useCardToTarget" },
...
### poxiang 名字:破降
描述: 出牌阶段限一次。你可以将一张牌交给一名其他角色。你摸三张牌（不计入本回合的手牌上限），移去所有“绝”并失去1点体力。
```js
poxiang: {
		audio: 2,
		enable: "phaseUse",
...
## xin_caozhen 名字:手杀界曹真 势力:wei

### mbsidi 名字:司敌
描述: 当你使用除延时锦囊牌外的牌结算完毕后，你可以选择一名未指定“司敌”目标的其他角色，并为其指定一名“司敌”目标角色（仅你可见）。其使用的下一张除延时锦囊牌外的牌指定目标后，清除你为其指定的“司敌”目标角色，若此时其使用此牌仅指定“司敌”目标为唯一目标，且目标：为你，你摸一张牌；不为你，你可以选择一项：⒈取消此牌目标，然后若场上没有处于濒死的角色，你对其造成1点伤害；⒉摸两张牌。
```js
mbsidi: {
		audio: "disordersidi",
		trigger: { player: "useCardAfter" },
...
## xin_sunluban 名字:手杀界孙鲁班 势力:wu

### xinzenhui 名字:谮毁
描述: 出牌阶段限一次。当你使用【杀】或黑色普通锦囊牌指定唯一目标时，你可选择另一名能成为此牌目标的其他角色并选择一项：①令其也成为此牌的目标。②获得其一张牌，然后将此牌的使用者改为该角色。
```js
xinzenhui: {
		audio: 2,
...
### xinjiaojin 名字:骄矜
描述: 当你受到男性角色造成的伤害时，你可以弃置一张装备牌并防止此伤害。
```js
xinjiaojin: {
		audio: 2,
		trigger: { player: "damageBegin4" },
...
## qiaozhou 名字:手杀谯周 势力:shu

### zhiming 名字:知命
描述: 准备阶段开始时或弃牌阶段结束时，你摸一张牌，然后可以将一张牌置于牌堆顶。
```js
zhiming: {
		audio: 2,
...
### xingbu 名字:星卜
描述: 结束阶段，你可以亮出牌堆顶的三张牌，然后你可以根据X值（X为这三张牌中红色牌的数量），令一名其他角色获得对应的效果直到其下回合结束：①三张：其摸牌阶段多摸两张牌，使用【杀】的次数上限+1。②两张：其使用【杀】的次数上限-1，跳过弃牌阶段。③小于两张：其于准备阶段开始时弃置一张手牌。
```js
xingbu: {
		audio: 2,
		trigger: { player: "phaseJieshuBegin" },
...
## xin_guyong 名字:手杀界顾雍 势力:wu

### xinshenxing 名字:慎行
描述: 出牌阶段限X次（X为你的体力值），你可以弃置两张牌，然后摸一张牌。若这两张牌颜色不同，则改为摸两张牌。
```js
xinshenxing: {
		audio: 2,
		enable: "phaseUse",
...
### xinbingyi 名字:秉壹
描述: 结束阶段，你可展示所有手牌。若这些牌的颜色或类型均相同，则你可以令至多X名角色各摸一张牌（X为你的手牌数）。
```js
xinbingyi: {
		audio: "bingyi",
...
## re_zhonghui 名字:手杀界钟会 势力:wei

### requanji 名字:权计
描述: 出牌阶段结束时，若你的手牌数大于体力值，或当你受到1点伤害后，你可以摸一张牌，然后将一张手牌置于武将牌上，称为“权”；你的手牌上限+X（X为“权”的数量）。
```js
requanji: {
		audio: 2,
...
## xin_caifuren 名字:手杀界蔡夫人 势力:qun

### xinqieting 名字:窃听
描述: 其他角色的回合结束时，若其本回合内未对其他角色造成过伤害，则你可选择一项：①摸一张牌。②观看其两张手牌并获得其中的一张。③将其装备区内的一张牌移动至你的装备区。
```js
xinqieting: {
		audio: 2,
		trigger: { global: "phaseEnd" },
...
## xin_zhoucang 名字:手杀界周仓 势力:shu

### mobilezhongyong 名字:忠勇
描述: 当你于出牌阶段内使用的【杀】结算结束后，若没有目标角色使用【闪】响应过此【杀】，则你可获得此【杀】；否则你可选择一项：①获得目标角色使用的【闪】，然后可将此【杀】交给另一名其他角色。②将目标角色使用的【闪】交给另一名其他角色，然后你本回合使用【杀】的次数上限+1且下一张【杀】的伤害值基数+1。（你不能使用本回合因执行〖忠勇〗的效果得到的牌）
```js
mobilezhongyong: {
		audio: 2,
		trigger: { player: "useCardAfter" },
...
## ol_yujin 名字:手杀界于禁 势力:wei

### rejieyue 名字:节钺
描述: 结束阶段开始时，你可以将一张牌交给一名其他角色。然后其选择一项：令你摸三张牌：或其保留一张手牌和装备区的牌，然后弃置其余的牌。
```js
rejieyue: {
		audio: 2,
		trigger: { player: "phaseJieshuBegin" },
...
## zhouqun 名字:周群 势力:shu

### tiansuan 名字:天算
描述: 出牌阶段限一次，若场上没有因你产生的“命运签”，则你可以抽取一张“命运签”，然后令一名角色获得“命运签”直到你的下回合开始。若你以此法抽取的“命运签”为：上上签，你观看其手牌并获得其区域内的一张牌；上签，你获得其一张牌。
```js
tiansuan: {
		audio: 2,
		enable: "phaseUse",
...
## re_zhurong 名字:手杀界祝融 势力:shu

### juxiang
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### relieren 名字:烈刃
描述: 当你使用【杀】指定目标后，你可以和目标角色进行拼点。若你赢，你获得其一张牌。若你没赢，你获得对方的拼点牌，其获得你的拼点牌。
```js
relieren: {
		audio: 2,
...
## re_jiangwei 名字:手杀界姜维 势力:shu

### retiaoxin 名字:挑衅
描述: 出牌阶段限一次，你可以指定一名有牌的其他角色，该角色需对你使用一张【杀】，否则你弃置其一张牌。
```js
retiaoxin: {
		audio: "tiaoxin",
...
## nanhualaoxian 名字:手杀南华老仙 势力:qun

### yufeng 名字:御风
描述: 出牌阶段限一次，你可以表演“御风飞行”。若表演失败，则你摸X张牌。若表演成功，则你可以选择至多X名其他角色获得“御风”效果，然后摸X-Y张牌（准备阶段开始时，你进行判定。若结果为：红色，你跳过摸牌阶段；黑色，你跳过出牌阶段和弃牌阶段。X为你的得分。Y为你选择的角色数）。
```js
yufeng: {
		inherit: "yufeng_old",
		manualConfirm: true,
...
### tianshu 名字:天书
描述: 出牌阶段开始时，若场上没有【太平要术】，则你可以弃置一张牌并选择一名角色。该角色获得并使用【太平要术】。
```js
tianshu: {
		audio: 2,
		trigger: { player: "phaseUseBegin" },
...
## xin_fuhuanghou 名字:手杀界伏寿 势力:qun

### xinzhuikong 名字:惴恐
描述: 每轮限一次，其他角色的回合开始时，若其体力值不小于你，你可与其拼点。若你赢，其本回合无法使用牌指定除其以外的角色为目标；若你没赢，你获得其拼点的牌，然后其视为对你使用一张【杀】。
```js
xinzhuikong: {
		audio: 2,
...
### xinqiuyuan 名字:求援
描述: 当你成为【杀】的目标时，你可以令另一名其他角色交给你一张除【杀】以外的基本牌，否则其也成为此【杀】的目标。
```js
xinqiuyuan: {
		inherit: "qiuyuan",
		async content(event, trigger, player) {
...
## xin_panzhangmazhong 名字:手杀界潘璋马忠 势力:wu

### xinduodao 名字:夺刀
描述: 当你受到伤害后，你可以获得伤害来源装备区里的武器牌。
```js
xinduodao: {
		audio: 2,
		trigger: { player: "damageEnd" },
...
### xinanjian 名字:暗箭
描述: 锁定技，当你使用【杀】指定目标后，若你不在目标角色攻击范围内，你选择一项：1. 令其无法响应此【杀】；2. 其受到此【杀】造成的伤害+1。
```js
xinanjian: {
		audio: 2,
		trigger: { player: "useCardToPlayered" },
...
## xin_guohuai 名字:手杀界郭淮 势力:wei

### mobilejingce 名字:精策
描述: 结束阶段，若此回合因使用或打出而置入弃牌堆的牌的数量不小于你的体力值，则你可以摸两张牌。
```js
mobilejingce: {
		audio: 2,
		trigger: { player: "phaseJieshuBegin" },
...
## gongsunkang 名字:公孙康 势力:qun

### juliao 名字:据辽
描述: 锁定技，其他角色计算与你的距离始终+X（X为场上势力数-1）。
```js
juliao: {
		mod: {
			globalTo(from, to, distance) {
...
### taomie 名字:讨灭
描述: 当你受到伤害后或当你造成伤害后，你可以令伤害来源或受伤角色获得“讨灭”标记（如场上已有标记则转移给该角色）；你和拥有“讨灭”标记的角色视为在彼此的攻击范围内，且当你对该角色造成伤害时，选择一项：1. 此伤害+1；2. 你获得其区域内的一张牌并可将之交给另一名角色；3. 依次执行前两项并于伤害结算后弃置其“讨灭”标记。
```js
taomie: {
		audio: 3,
...
## xin_zhangfei 名字:手杀界张飞 势力:shu

### new_repaoxiao
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### liyong 名字:厉勇
描述: 锁定技，若你于出牌阶段内使用的【杀】被【闪】抵消，则你获得如下效果：你本回合使用的下一张【杀】不可被响应且伤害+1，指定的目标本回合非锁定技失效，当此【杀】造成伤害后，若目标角色未死亡，你失去1点体力。
```js
liyong: {
		audio: "retishen",
...
## xin_hansui 名字:手杀韩遂 势力:qun

### xinniluan 名字:逆乱
描述: 其他角色的结束阶段，若其本回合对除其以外的角色使用过牌，则你可以对其使用一张【杀】。若以此法使用的【杀】造成伤害，则你弃置其一张牌。
```js
xinniluan: {
		audio: "niluan",
		trigger: { global: "phaseJieshuBegin" },
...
## hucheer 名字:手杀胡车儿 势力:qun

### daoji 名字:盗戟
描述: 出牌阶段限一次，你可以弃置一张非基本牌并选择一名装备区里有牌的其他角色，你获得其装备区中的一张牌并使用之。若你以此法得到的牌是武器牌，则你使用此牌后对其造成1点伤害。
```js
daoji: {
		audio: 2,
		enable: "phaseUse",
...
## simashi 名字:司马师 势力:wei

### baiyi 名字:败移
描述: 限定技，出牌阶段，若你已受伤，你可以交换两名其他角色的座次。
```js
baiyi: {
		audio: 2,
		enable: "phaseUse",
...
### jinglve 名字:景略
描述: 出牌阶段限一次，若场上没有与你对应的「死士」牌，则你可以观看一名其他角色的手牌，将其中一张牌标记为「死士」。当其使用对应的实体牌中包含「死士」的牌时，你取消此牌的所有目标。当「死士」牌不因使用而进入弃牌堆，或其回合结束后，若「死士」牌仍在其区域内，则你获得此牌。
```js
jinglve: {
		audio: 2,
		enable: "phaseUse",
...
### shanli 名字:擅立
描述: `觉醒技，准备阶段，若你已发动过〖败移〗且对至少两名角色发动过〖景略〗，则你减1点体力上限并选择一名角色。系统随机选择三个不为${get.poptip("yuri_wangxi")}的主公技，然后你选择其中一个技能，令其获得之。其将交互表情中的【拖鞋】和【酒杯】替换为【枷锁】和【玉玺】。`
```js
shanli: {
		audio: 2,
		trigger: { player: "phaseZhunbeiBegin" },
...
## yanghuiyu 名字:羊徽瑜 势力:wei

### hongyi 名字:弘仪
描述: 出牌阶段限一次，你可以选择一名其他角色。你的下回合开始前，该角色造成伤害时进行判定，若结果为：黑色，此伤害-1。红色，受到伤害的角色摸一张牌。
```js
hongyi: {
		audio: 2,
		enable: "phaseUse",
...
### requanfeng 名字:劝封
描述: 限定技。①其他角色死亡时，你可失去〖弘仪〗，然后获得其武将牌上的所有非主公技，非隐匿技和非Charlotte技，加1点体力上限并回复1点体力。②当你处于濒死状态时，你可以加2点体力上限，然后回复4点体力。
```js
requanfeng: {
		audio: "quanfeng",
		enable: "chooseToUse",
...
## xin_zhuran 名字:手杀界朱然 势力:wu

### mobiledanshou 名字:胆守
描述: 一名其他角色的结束阶段开始时，若X：为0，你摸一张牌；不等于0，你可弃置X张牌并对其造成1点伤害（X为其本回合内使用的目标包含你的牌的数量）。
```js
mobiledanshou: {
		trigger: { global: "phaseJieshuBegin" },
		audio: 2,
...
## xin_gongsunzan 名字:手杀界公孙瓒 势力:qun
...
## dingyuan 名字:手杀丁原 势力:qun

### beizhu 名字:备诛
描述: 出牌阶段限一次，你可以观看一名其他角色的手牌。若其中：没有【杀】，你弃置其一张牌，然后你可令其获得牌堆中的一张【杀】；有【杀】，其依次对你使用这些【杀】，当你因此受到1点伤害后，你摸一张牌。
```js
beizhu: {
		audio: 3,
		enable: "phaseUse",
...
## xin_jianyong 名字:手杀界简雍 势力:shu

### xinqiaoshui 名字:巧说
描述: 出牌阶段限一次，你可以和一名其他角色拼点。若你赢，则你本阶段内使用下一张基本牌或普通锦囊牌时，可以为此牌增加或减少一个目标。若你没赢，你本阶段内不能使用锦囊牌。
```js
xinqiaoshui: {
		audio: 2,
		enable: "phaseUse",
...
### xinjyzongshi 名字:纵适
描述: 当你拼点后，你观看牌堆顶的一张牌，然后可以选择一项：获得此牌，或获得两张拼点牌中点数较小的一张。
```js
xinjyzongshi: {
		audio: 2,
		trigger: {
...
## xin_caozhang 名字:手杀界曹彰 势力:wei

### rejiangchi 名字:将驰
描述: 出牌阶段开始时，你可以选择一项：1、摸一张牌，若如此做，你本阶段内不能使用【杀】。 2、弃置一张牌，若如此做，此阶段你使用【杀】无距离限制且你可以额外使用一张【杀】。
```js
rejiangchi: {
		audio: 2,
		trigger: {
...
## xin_liaohua 名字:手杀界廖化 势力:shu

### redangxian 名字:当先
描述: 锁定技，回合开始时，你从弃牌堆中获得一张【杀】并进行一个额外的出牌阶段。
```js
redangxian: {
		trigger: { player: "phaseBegin" },
		forced: true,
...
### refuli 名字:伏枥
描述: 限定技，当你处于濒死状态时，你可以将体力值回复至X点（X为势力数）。然后若你的体力值为全场唯一最多，你翻面。
```js
refuli: {
		skillAnimation: true,
		animationColor: "soil",
...
## furong 名字:手杀傅肜 势力:shu

### xuewei 名字:血卫
描述: 准备阶段，你可以选择一名其他角色（仅自己可见）。若如此做，直到你的下回合开始前，当其第一次受到伤害时，你防止此伤害，改为由你受到等量的伤害并对伤害来源造成等量同属性的伤害。
```js
xuewei: {
		audio: 2,
		trigger: { player: "phaseZhunbeiBegin" },
...
## re_dongzhuo 名字:手杀界董卓 势力:qun

### rejiuchi 名字:酒池
描述: 你可以将一张黑桃手牌当做【酒】使用。当你于回合内使用带有【酒】效果的【杀】造成伤害时，你令你的〖崩坏〗失效直到回合结束。
```js
rejiuchi: {
		group: ["jiuchi"],
...
## re_xusheng 名字:手杀界徐盛 势力:wu

### repojun
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

## sp_sufei 名字:手杀苏飞 势力:qun

### zhengjian 名字:诤荐
描述: 锁定技，结束阶段，你令一名角色获得一枚“诤”。回合开始时，你令有“诤”的角色移去“诤”并摸X张牌（X为其最后一次获得“诤”后使用和打出的牌数，且至多不能超过其体力上限或5）。
```js
zhengjian: {
		audio: 2,
		trigger: { player: "phaseJieshuBegin" },
...
### gaoyuan 名字:告援
描述: 当你成为【杀】的目标时，你可弃置一张牌将此【杀】转移给一名有“诤”且不是此【杀】使用者的其他角色。
```js
gaoyuan: {
		audio: 2,
		trigger: { target: "useCardToTarget" },
...
## zhengxuan 名字:郑玄 势力:qun

### zhengjing 名字:整经
描述: 出牌阶段，你可以整理卡牌。然后，你将整理出的卡牌中的至少一张作为“经”置于一名角色的武将牌上，然后获得其余的牌。该角色的准备阶段获得这些牌，且跳过此回合的判定和摸牌阶段。
```js
zhengjing: {
		audio: 2,
		enable: "phaseUse",
...
## dengzhi 名字:手杀邓芝 势力:shu

### jimeng 名字:急盟
描述: 出牌阶段开始时，你可以获得一名其他角色的一张牌，然后交给该角色X张牌（X为你当前体力值）。
```js
jimeng: {
		audio: 2,
		trigger: { player: "phaseUseBegin" },
...
### shuaiyan 名字:率言
描述: 弃牌阶段开始时，若你的手牌数大于1，则你可以展示所有手牌，然后你令一名其他角色交给你一张牌。
```js
shuaiyan: {
		audio: 2,
		trigger: { player: "phaseDiscardBegin" },
...
## xin_chengpu 名字:手杀界程普 势力:wu

### relihuo 名字:疠火
描述: 当你使用普【杀】时，你可以将此【杀】改为火属性。若如此做，当你因执行此【杀】的效果而对横置角色造成伤害时，此伤害+1；当你使用的火【杀】结算完成后，你失去X点体力（X为你因此【杀】造成的伤害总点数的一半且向下取整）。
```js
relihuo: {
		audio: 2,
...
## yangyi 名字:手杀杨仪 势力:shu

### duoduan 名字:度断
描述: 每回合限一次，当你成为【杀】的目标后，你可以重铸一张牌。若如此做，你选择一项：①令使用者摸两张牌，且此【杀】无效。②令使用者弃置一张牌，且你不能响应此【杀】。
```js
duoduan: {
		audio: 2,
...
### gongsun 名字:共损
描述: 出牌阶段开始时，你可以弃置两张牌并指定一名其他角色。你选择一个基本牌或普通锦囊牌的牌名。直到你的下回合开始或你死亡，你与其不能使用或打出或弃置此名称的牌。
```js
gongsun: {
		audio: 2,
		trigger: { player: "phaseUseBegin" },
...
## dongcheng 名字:手杀董承 势力:qun

### chengzhao 名字:承诏
描述: 一名角色的结束阶段，若你于本回合内获得了至少两张牌，则你可以与一名角色拼点。若你赢，你视为对其使用一张无视防具的【杀】。
```js
chengzhao: {
		audio: 2,
		trigger: { global: "phaseJieshuBegin" },
...
## re_pangtong 名字:手杀界庞统 势力:shu

### xinlianhuan 名字:连环
描述: 你可以将一张♣手牌当【铁索连环】使用或重铸。你使用【铁索连环】选择目标后，可以给此牌增加一个目标。
```js
xinlianhuan: {
		audio: 2,
...
## re_guanqiujian 名字:手杀毌丘俭 势力:wei

### rezhengrong 名字:征荣
描述: 锁定技。当你于出牌阶段使用的指定了其他角色为目标的牌结算完成后，若此牌是你本局游戏内于出牌阶段使用的指定了其他角色为目标的第偶数张牌，则你将一名其他角色的随机一张牌置于你的武将牌上，称为「荣」。
```js
rezhengrong: {
		trigger: { player: "useCardAfter" },
		audio: "drlt_zhenrong",
...
### rehongju 名字:鸿举
描述: `觉醒技，准备阶段，若你武将牌上「荣」的数量不小于3且有角色死亡，则你摸等同于「荣」数量的牌。然后可以用任意数量的手牌交换等量的「荣」。你减1点体力上限并获得技能${get.poptip("reqingce")}。`
```js
rehongju: {
		trigger: { player: "phaseZhunbeiBegin" },
		audio: "drlt_hongju",
...
## chendeng 名字:手杀陈登 势力:qun

### zhouxuan 名字:周旋
描述: 出牌阶段限一次，你可以指定一名敌方角色，然后选择一个基本牌的名称或非基本牌的类型。其使用或打出下一张牌时，若此牌的名称或类型和你选择的相同，则你观看牌堆顶的三张，然后将这些牌分配给任意名角色。
```js
zhouxuan: {
		audio: 2,
		enable: "phaseUse",
...
### fengji 名字:丰积
描述: 锁定技，回合结束时，你记录你的手牌数。回合开始时，若你的手牌数不小于你记录的手牌数，则你摸三张牌且本回合手牌上限为体力上限。
```js
fengji: {
		audio: 2,
		trigger: { player: "phaseBegin" },
...
## re_heqi 名字:手杀贺齐 势力:wu

### reqizhou 名字:绮胄
描述: `锁定技，当你使用装备时，你根据使用过的装备牌名数获得以下技能：1种或以上：${get.poptip("yingzi")}；2种或以上：${get.poptip("jiang")}；3种或以上：${get.poptip("xinpojun")}；4种或以上：${get.poptip("dbchongjian")}；5种或以上：${get.poptip("fenwei")}。`
```js
reqizhou: {
		audio: 2,
		trigger: { player: "useCard" },
...
### reshanxi 名字:闪袭
描述: 出牌阶段限一次，你可以弃置一张基本牌，然后弃置一名其他角色的一张手牌。若你弃置的牌为红色，你获得其弃置的牌；若其因此弃置的牌名未因此技能弃置过，你对其造成1点伤害。
```js
reshanxi: {
		audio: "shanxi",
		enable: "phaseUse",
...
## yangbiao 名字:手杀杨彪 势力:qun

### zhaohan 名字:昭汉
描述: 锁定技，你的第1-4个准备阶段开始时，你加1点体力上限并回复1点体力，你的第5-7个准备阶段开始时，你减1点体力上限。
```js
zhaohan: {
		audio: 2,
		trigger: { player: "phaseZhunbeiBegin" },
...
### rangjie 名字:让节
描述: 当你受到1点伤害后，你可以选择一项并摸一张牌：获得牌堆或弃牌堆里你选择的类型的一张牌，或移动场上的一张牌。
```js
rangjie: {
		audio: 2,
		trigger: { player: "damageEnd" },
...
### yizheng 名字:义争
描述: 出牌阶段限一次，你可以和一名体力值不大于你的其他角色拼点。若你赢，其跳过下个摸牌阶段。若你没赢，你减1点体力上限。
```js
yizheng: {
		audio: 2,
		enable: "phaseUse",
...
## re_sp_zhugeliang 名字:手杀界卧龙 势力:shu
...
## xin_xiahoudun 名字:手杀界夏侯惇 势力:wei

### reganglie
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### xinqingjian 名字:清俭
描述: 每回合限一次。当你不因摸牌阶段的额定摸牌而得到牌后，你可以将任意张手牌扣置于武将牌上。一名角色的回合结束后，你将这些牌交给任意名其他角色。若你此次交出牌的数量大于1，你摸一张牌。
```js
xinqingjian: {
		audio: "qingjian",
		trigger: {
...
## zhangyì 名字:张翼 势力:shu

### rezhiyi 名字:执义
描述: 锁定技，一名角色的结束阶段开始时，若你本回合内使用或打出过基本牌，则你选择一项：1.摸一张牌。2.视为使用一张你本回合内使用或打出过的基本牌。
```js
rezhiyi: {
		audio: "zhiyi",
		trigger: { global: "phaseJieshuBegin" },
...
### tongqu 名字:通渠
描述: 游戏开始时，你拥有一个“渠”标记。准备阶段，你可以失去1点体力令一名没有“渠”标记的角色获得“渠”标记。有“渠”的角色摸牌阶段额外摸一张牌，然后将一张牌交给其他有“渠”的角色或弃置。若以此法给出的是装备牌则使用之。当有“渠”的角色进入濒死状态时，移除其“渠”标记。
```js
tongqu: {
		audio: 2,
		trigger: {
...
### xinwanlan 名字:挽澜
描述: 当一名角色受到伤害值不小于体力值的伤害时，你可以弃置装备区中的所有牌（至少一张） 防止此伤害。
```js
xinwanlan: {
		audio: "wanlan",
		trigger: { global: "damageBegin4" },
...
## miheng 名字:手杀祢衡 势力:qun

### kuangcai 名字:狂才
描述: 出牌阶段开始时，你可以令你此阶段内的主动出牌时间变为5秒。若如此做，你于此阶段内使用牌没距离和次数限制，且每当你于此阶段内使用牌时，你摸一张牌且主动出牌时间-1秒。若主动出牌时间减至0，则你结束出牌阶段。
```js
kuangcai: {
		audio: 2,
		trigger: { player: "phaseUseBegin" },
...
### shejian 名字:舌剑
描述: 弃牌阶段结束时，若你于此阶段弃置过至少两张牌且这些牌花色均不相同，则你可以弃置一名其他角色的一张牌。
```js
shejian: {
		audio: 2,
...
## taoqian 名字:手杀陶谦 势力:qun

### zhaohuo 名字:招祸
描述: 锁定技，当其他角色进入濒死状态时，你将体力上限调整为1点。若你的体力上限因此减少，则你摸X张牌。（X为你以此法减少的体力上限）
```js
zhaohuo: {
		audio: 2,
...
### yixiang 名字:义襄
描述: 每名角色的回合限一次，当你成为一名角色使用牌的目标后，若该角色的体力值大于你的体力值，你可以随机获得牌堆里的一张你没有的基本牌。
```js
yixiang: {
		audio: 2,
		audioname: ["re_taoqian"],
...
### yirang 名字:揖让
描述: 出牌阶段开始时，你可以将所有非基本牌交给一名体力上限大于你的其他角色，然后调整体力上限至与该角色相同并回复X点体力（X为你以此法交给其的牌的类别数）。
```js
yirang: {
		audio: 2,
		audioname: ["re_taoqian"],
...
## liuzan 名字:手杀留赞 势力:wu

### fenyin 名字:奋音
描述: 你的回合内，当你使用牌时，若此牌与你于此回合内使用的上一张牌的颜色不同，则你可以摸一张牌。
```js
fenyin: {
		locked: false,
		mod: {
...
## lingcao 名字:lingcao 势力:wu

### dujin 名字:独进
描述: 摸牌阶段，你可以多摸X+1张牌（X为你装备区里牌数的一半且向上取整）。
```js
dujin: {
		audio: 2,
		trigger: { player: "phaseDrawBegin2" },
...
## lifeng 名字:lifeng 势力:shu

### tunchu 名字:屯储
描述: 摸牌阶段，若你没有「粮」，你可以多摸两张牌。若如此做，摸牌阶段结束时，你可以将任意张手牌置于你的武将上，称为「粮」，只要你的武将牌上有「粮」，你便不能使用【杀】。
```js
tunchu: {
		audio: 2,
		trigger: { player: "phaseDrawBegin2" },
...
### shuliang 名字:输粮
描述: 一名角色的结束阶段开始时，若其手牌数少于体力值，你可以移去一张「粮」，然后该角色摸两张牌。
```js
shuliang: {
		audio: 2,
		trigger: { global: "phaseJieshuBegin" },
...
## zhuling 名字:zhuling 势力:wei

### xinzhanyi 名字:战意
描述: 出牌阶段限一次，你可以弃置一张牌并失去1点体力，然后根据你弃置的牌获得对应效果直到此阶段结束：基本牌，你可以将一张基本牌当作任意基本牌使用，且你此阶段下一张使用的基本牌获得以下效果：{当你因执行你使用此牌的效果而造成伤害时，此伤害值+1；当一名角色因执行你使用此牌的效果而回复体力时，此回复值+1。}；锦囊牌，摸三张牌且你使用的牌不能被【无懈可击】响应；装备牌，你使用【杀】指定唯一目标后，其弃置两张牌，然后你获得其中的一张。
```js
xinzhanyi: {
		audio: "zhanyi",
		enable: "phaseUse",
...
## liuye 名字:手杀刘晔 势力:wei

### polu 名字:破橹
描述: `锁定技，回合开始时，若${get.poptip("ly_piliche")}未加入游戏或在牌堆/弃牌堆内，则你获得并使用之；当你受到1点伤害后，若你的装备区里没有${get.poptip("ly_piliche")}，你摸一张牌，然后你获得牌堆中的一张随机武器牌并使用之。`
```js
polu: {
		audio: 2,
		trigger: { player: "phaseZhunbeiBegin" },
...
### choulve 名字:筹略
描述: 出牌阶段开始时，你可以令一名其他角色交给你一张牌，若其如此做，视为你使用上一张对你造成过伤害且不为延时锦囊牌的牌。
```js
choulve: {
		init(player, skill) {
			player.addSkill(skill + "_mark");
...
## zhaotongzhaoguang 名字:手杀赵统赵广 势力:shu

### yizan_use 名字:翊赞
描述: 你可以将两张牌（其中至少一张为基本牌）当做任意基本牌使用或打出。
```js
yizan_use: {
...
### xinfu_longyuan 名字:龙渊
描述: 觉醒技，准备阶段，若你本局游戏内发动过〖翊赞〗的次数大于等于3，则你修改〖翊赞〗。
```js
xinfu_longyuan: {
		audio: 2,
		forced: true,
...
## majun 名字:majun 势力:wei

### xinfu_jingxie 名字:精械
描述: 出牌阶段，你可以展示一张未强化过的【诸葛连弩】或标准包/军争包/SP包中的防具牌，然后对其进行强化。当你处于濒死状态时，你可以重铸一张防具牌，然后将体力回复至1点。
```js
xinfu_jingxie: {
		audio: 2,
		video(player, info) {
...
### qiaosi 名字:巧思
描述: 出牌阶段限一次，你可以表演「大键角色图」并根据表演结果获得相应的牌。然后，你选择一项：1.弃置X张牌。2.将X张牌交给一名其他角色。（X为你以此法得到的牌数）
```js
qiaosi: {
		audio: 2,
...
## simazhao 名字:SP司马昭 势力:wei

### xinfu_daigong 名字:怠攻
描述: 每回合限一次。当你受到伤害时，你可以展示所有手牌，然后令伤害来源选择一项：交给你一张与你所有手牌花色均不相同的一张牌，或防止此伤害。
```js
xinfu_daigong: {
		usable: 1,
		audio: 2,
...
### xinfu_zhaoxin 名字:昭心
描述: 出牌阶段限一次，你可以将任意张牌置于武将牌上并摸等量的牌，称之为“望”（你至多拥有三张“望”）。你或你攻击范围内的角色的摸牌阶段结束后，其可以获得一张“望”，然后你可以对其造成1点伤害。
```js
xinfu_zhaoxin: {
		audio: 2,
		enable: "phaseUse",
...
## wangyuanji 名字:wangyuanji 势力:wei

### xinfu_qianchong 名字:谦冲
描述: `锁定技，若你的装备区内有牌且：均为红色，则你视为拥有技能${get.poptip("mingzhe")}。均为黑色，则你视为拥有技能${get.poptip("weimu")}。若均不满足，则出牌阶段开始时，你可以选择一种类别的牌，然后你本回合内使用该类别的牌时没有次数和距离限制。`
```js
xinfu_qianchong: {
		audio: 1,
		init(player, skill) {
...
### xinfu_shangjian 名字:尚俭
描述: 锁定技。一名角色的结束阶段开始时，若你于此回合内不因使用装备牌而失去了X张或更少的牌，则你摸等量的牌（X为你的体力值）。
```js
xinfu_shangjian: {
		audio: 2,
		getNum(player) {
...
## pangdegong 名字:手杀庞德公 势力:qun

### xinfu_pingcai 名字:评才
描述: 出牌阶段限一次，你可以挑选一个宝物并擦拭掉其上面的灰尘。然后，你可以根据宝物类型执行对应的效果。
```js
xinfu_pingcai: {
		subSkill: { backup: {} },
...
### xinfu_pdgyingshi 名字:隐世
描述: 锁定技，你始终跳过准备阶段，判定阶段，结束阶段。你不能被选择为延时锦囊牌的目标。
```js
xinfu_pdgyingshi: {
		mod: {
...
### mbchengxiong 名字:惩凶
描述: 你使用锦囊牌仅指定其他角色为目标后，可选择一名牌数不小于X的其他角色，弃置其一张牌（X为你此阶段使用的牌数）。若此牌颜色与你使用的锦囊牌相同，你对其造成1点伤害。
```js
mbchengxiong: {
		audio: 2,
...
### mbwangzhuang 名字:妄专
描述: 一名角色受到非牌造成的伤害后，若你是伤害来源或受伤角色，你可以摸一张牌，令当前回合角色本回合非锁定技失效。
```js
mbwangzhuang: {
		audio: 2,
...
## mb_wangjing 名字:王经 势力:wei

### mbzujin 名字:阻进
描述: 每回合每种牌名限一次，若你未受伤或体力值不为最低，你可将一张基本牌当作【杀】使用或打出；若你已受伤，你可将一张基本牌当作【闪】或【无懈可击】使用或打出。
```js
mbzujin: {
		audio: 3,
		enable: ["chooseToUse", "chooseToRespond"],
...
### mbjiejian 名字:节谏
描述: 准备阶段，你可将任意张手牌交给一名其他角色，并令这些角色获得“节谏”标记。每回合每名角色限一次，“节谏”角色成为一张非你使用的非装备牌的唯一目标时，你可将此牌转移给你，然后摸一张牌。“节谏”角色的回合结束时，移去其“节谏”标记，若其体力值不小于X（X为你交给其牌时其的体力值），你摸两张牌。
```js
mbjiejian: {
		audio: 3,
		trigger: {
...
## shenpei 名字:审配 势力:qun

### shouye 名字:守邺
描述: 每回合限一次。当你成为其他角色使用牌的唯一目标后，你可以与其进行【对策】。若你赢，则此牌对你无效，且你于此牌进入弃牌堆时获得其对应的所有实体牌。
```js
shouye: {
		audio: 2,
		trigger: { target: "useCardToTargeted" },
...
### liezhi 名字:烈直
描述: 准备阶段，你可以依次弃置至多两名其他角色区域内的各一张牌。当你受到伤害时，此技能失效直至你的下个结束阶段。
```js
liezhi: {
		audio: 2,
		trigger: { player: "phaseZhunbeiBegin" },
...
## re_wangyun 名字:手杀王允 势力:qun

### relianji 名字:连计
描述: 出牌阶段限一次，你可以选择两名其他角色A和B。第一名角色A随机使用牌堆中的一张武器牌，然后A视为对另一名角色B随机使用一张下列的牌名的牌：【决斗】、【火攻】、【南蛮入侵】、【万箭齐发】或普【杀】。然后若此牌造成伤害且A为伤害来源，你获得X枚“连计”（X为此次扣减的体力值点数）
```js
relianji: {
		audio: "wylianji",
		enable: "phaseUse",
...
### remoucheng 名字:谋逞
描述: `觉醒技，当一名角色造成伤害后，若你拥有的“连计”数大于2，你增加1点体力上限，回复1点体力，失去${get.poptip("relianji")}，获得${get.poptip("jingong")}。`
```js
remoucheng: {
...
## hujinding 名字:手杀胡金定 势力:shu

### renshi 名字:仁释
描述: 锁定技，当你受到【杀】的伤害时，若你已受伤，则你防止此伤害并获得此【杀】对应的所有实体牌，然后减1点体力上限。
```js
renshi: {
		audio: 2,
		trigger: { player: "damageBegin4" },
...
### wuyuan 名字:武缘
描述: 出牌阶段限一次，你可将一张【杀】交给一名其他角色，然后你回复1点体力，其摸一张牌。若此【杀】为：红色【杀】，其回复1点体力；属性【杀】，其改为摸两张牌。
```js
wuyuan: {
		audio: 2,
		enable: "phaseUse",
...
## re_zhanggong 名字:手杀张恭 势力:wei

### reqianxin 名字:遣信
描述: 出牌阶段限一次，你可将至多两张手牌随机交给等量的其他角色，称为「信」。这些角色的准备阶段开始时，若其手牌中有「信」，则其选择一项：令你摸两张牌，本回合手牌上限-2。
```js
reqianxin: {
		audio: "xinfu_qianxin",
		enable: "phaseUse",
...
### rezhenxing 名字:镇行
描述: 结束阶段开始时或当你受到伤害后，你可以观看牌堆顶的三张牌，然后你获得其中与其余牌花色均不相同的一张牌，将剩余牌以原顺序放回牌堆顶。
```js
rezhenxing: {
...
## re_xugong 名字:手杀许贡 势力:qun

### rebiaozhao 名字:表召
描述: 结束阶段，你可以将一张牌置于武将牌上，称为「表」。当有一张与「表」点数相同的牌进入弃牌堆后，你将「表」置入弃牌堆并失去1点体力。准备阶段，若你的武将牌上有「表」，则你移去「表」并选择一名角色，该角色回复1点体力并摸三张牌。
```js
rebiaozhao: {
		audio: "biaozhao",
		// intro: {
...
## re_weiwenzhugezhi 名字:手杀卫温诸葛直 势力:wu

### refuhai 名字:浮海
描述: 出牌阶段限一次，你可令其他角色同时在「潮起」和「潮落」中选择一项，并依次展示这些角色的选项。若从你下家开始选择了相同选项的角色数目大于1，则你摸X张牌（X为连续相同结果的数量）。
```js
refuhai: {
		audio: "xinfu_fuhai",
		enable: "phaseUse",
...
## scs_zhaozhong 名字:赵忠 势力:qun

### scschiyan 名字:鸱咽
描述: ①当你使用【杀】指定目标后，你可以将其的一张牌置于其武将牌上，然后其于当前回合结束时获得这些牌。②当你因执行【杀】的效果对一名角色造成伤害时，若该角色的手牌数和装备区内的牌数均不大于你，此伤害+1。
```js
scschiyan: {
		audio: 1,
		trigger: { player: "useCardToPlayered" },
...
## scs_sunzhang 名字:孙璋 势力:qun

### scszimou 名字:自谋
描述: 锁定技。出牌阶段，当你使用第二/四/六张牌时，你从牌堆中获得一张【酒】/【杀】/【决斗】。
```js
scszimou: {
		audio: 1,
		trigger: { player: "useCard" },
...
## scs_bilan 名字:毕岚 势力:qun

### scspicai 名字:庀材
描述: 出牌阶段限一次。你可进行判定牌不置入弃牌堆的判定。若判定结果与本次发动技能时的其他判定结果的花色均不相同，则你可以重复此流程。然后你将所有位于处理区的判定牌交给一名角色。
```js
scspicai: {
		audio: 1,
		enable: "phaseUse",
...
## scs_xiayun 名字:夏恽 势力:qun

### scsyaozhuo 名字:谣诼
描述: 出牌阶段限一次。你可以与一名角色拼点，若你赢，其跳过下一个摸牌阶段；若你没赢，你弃置两张牌。
```js
scsyaozhuo: {
		audio: 1,
		enable: "phaseUse",
...
## scs_lisong 名字:栗嵩 势力:qun

### scskuiji 名字:窥机
描述: 出牌阶段限一次。你可以观看一名其他角色的手牌，然后弃置你与其的共计四张花色各不相同的手牌。
```js
scskuiji: {
		audio: 1,
		enable: "phaseUse",
...
## scs_duangui 名字:段珪 势力:qun

### scschihe 名字:叱吓
描述: 当你使用【杀】指定唯一目标后，你可亮出牌堆顶的两张牌，令此【杀】的伤害值基数+X（X为亮出牌中花色与此【杀】相同的牌数），且目标角色不能使用亮出牌包含的花色的牌响应此【杀】。
```js
scschihe: {
		audio: 1,
		trigger: { player: "useCardToPlayered" },
...
## scs_guosheng 名字:郭胜 势力:qun

### scsniqu 名字:逆取
描述: 出牌阶段限一次。你可以对一名角色造成1点火焰伤害。
```js
scsniqu: {
		audio: 1,
		enable: "phaseUse",
...
## scs_gaowang 名字:高望 势力:qun

### scsmiaoyu 名字:妙语
描述: 你可以将至多两张相同花色的牌按照以下规则使用或打出：♦牌当作火【杀】，♥牌当作【桃】，♣牌当作【闪】，♠牌当作【无懈可击】。若你以此法使用了两张红色牌，则此牌回复值或伤害值+1。若你以此法使用了两张黑色牌，则你弃置当前回合角色一张牌。
```js
scsmiaoyu: {
		audio: "scsanruo",
		enable: ["chooseToUse", "chooseToRespond"],
...
## mb_lingju 名字:手杀灵雎 势力:qun

### mbjieyuan 名字:竭缘
描述: `你造成伤害时，你可以选择一项：1、弃置一张黑色牌，令此伤害+1；2、从牌堆中获得一张黑色牌。你受到伤害时，你可以选择一项：1、弃置一张红色牌，令此伤害-1；2、从牌堆中获得一张红色牌。${get.poptip("rule_beishui")}：删除另一个时机的效果，将伤害的增减、获得牌的数量改为2，然后失去“背水”选项。`
```js
mbjieyuan: {
...
### mbfenxin 名字:焚心
描述: 被你杀死的角色亮出其身份牌前，你可选择一项：1、其亮出身份牌后，若与你身份不同，你获得其武将牌上所有技能（限定技、觉醒技、使命技、主公技、持恒技除外）；2、与其交换身份牌（你与其的身份牌非明置时方可选择）。
```js
mbfenxin: {
...
## mb_shen_lvbu 名字:手杀神吕布 势力:shen

### baonu
(未在本包skill代码文件中找到该技能定义，可能是全局共享技能或跨包引用)

### mbwumou 名字:无谋
描述: 锁定技，你的普通锦囊牌只能当普通【杀】使用或打出，此【杀】的目标改为原锦囊牌的目标。
```js
mbwumou: {
		audio: "wumou",
...
### mbwuqian 名字:无前
描述: "出牌阶段，你可以弃置2枚“暴怒”，令一名角色防具牌失效且你获得" + get.poptip("wushuang") + "，直至你下次使用伤害牌但未造成伤害；你的出【杀】次数+X（X为当前被“无前”的角色数）；结束阶段，若你的手牌中没有伤害牌，你随机获得一张伤害牌。"
```js
mbwuqian: {
		audio: "ol_wuqian",
		enable: "phaseUse",
