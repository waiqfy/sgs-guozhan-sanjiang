# sxrm merged reference

## sxrm_zhouyu 名字:魔周瑜 势力:devil

### sxrmjiehuo 名字:劫火
描述: 使命技，回合开始时，你可以令场上下次出现的伤害改为3点火焰伤害。失败：若造成此伤害的角色不为你：你减1点体力上限。
```js
sxrmjiehuo: {
		audio: 2,
		dutySkill: true,
...
### sxrmxianger 名字:香饵
描述: 使命技，出牌阶段限一次，你可以令一名角色于其下个结束阶段回复2点体力，期间其不能使用点数大于6的牌。失败：期间其受到的伤害值小于2：你减1点体力上限。
```js
sxrmxianger: {
		audio: 2,
		dutySkill: true,
...
### sxrmmieguo 名字:灭虢
描述: 使命技，额定回合结束后，你可以获得一名其他角色至多三张牌并令其指定等量角色，然后你执行一个不能对其指定角色使用牌的额外回合。失败：此回合你未使用过牌：你减1点体力上限。
```js
sxrmmieguo: {
		audio: 2,
		dutySkill: true,
...
## sxrm_sunshangxiang 名字:嗔孙尚香 势力:wu

### sxrmjiaozong 名字:骄纵
描述: 准备阶段或当你成为【杀】的目标后，你可以移动场上一张装备牌，以此法装备区被置入牌的角色本回合不能使用与之颜色相同的牌且受到的伤害+1。
```js
sxrmjiaozong: {
		audio: 2,
		trigger: {
...
### sxrmfusui 名字:妇随
描述: `限定技，出牌阶段，你可以选择一名其他男性角色的一个非标签技能，你令其失去此技能，然后你与其获得${get.poptip("sxrmbiyi")}并防止你们本轮受到的伤害。`
```js
sxrmfusui: {
		audio: 2,
		enable: "phaseUse",
...
## sxrm_zhangzhao 名字:嗔张昭 势力:wu

### sxrmxiezhong 名字:挟众
描述: 准备阶段，你可令场上半数角色（向上取整）依次选择并执行一项：1.摸两张牌，失去 1 点体力；2.将两张牌当【杀】使用。然后你可以令另一名角色执行两次被选择较多的一项。
```js
sxrmxiezhong: {
		audio: 2,
...
### sxrmqishi 名字:乞施
描述: `${get.poptip("sxrm_qidingSkill")}。结束阶段，你可以获得本回合其他角色进入弃牌堆的至多五张牌，然后你跳过下个摸牌阶段。`
```js
sxrmqishi: {
		audio: 2,
...
## sxrm_lusu 名字:嗔鲁肃 势力:wu

### sxrmwanli 名字:万利
描述: ①首轮开始时，你可以选择一名其他角色并交给其任意张牌，若如此做，第四轮结束时，其交给你三倍数量的牌（不足则全给并失去所有技能）。②摸牌阶段，若你因〖万利①〗选择的角色已还牌或死亡，你多摸三张牌。
```js
sxrmwanli: {
		audio: 2,
...
### sxrmlishui 名字:理说
描述: `当你成为黑色牌的目标后，你可以摸一张牌并与一名未还牌的因〖万利①〗选择的角色拼点：若你赢，其提前一轮还牌。`
```js
sxrmlishui: {
		audio: 2,
...
## sxrm_chen_caocao 名字:嗔曹操 势力:wei

### sxrmlanjiao 名字:揽娇
描述: 出牌阶段每名角色限一次，你可以展示一名其他角色两张手牌，然后你获得展示的牌；其可以在你展示这些牌时失去1点体力并摸一张牌，令你重新选择展示的手牌。若你一回合内因此获得过♥和♦牌，此技能本回合失效。
```js
sxrmlanjiao: {
		audio: 2,
		enable: "phaseUse",
...
## sxrm_zhugeliang 名字:嗔诸葛亮 势力:shu

### sxrmbingqu 名字:并驱
描述: 准备阶段，你可以与一名其他角色各声明一种普通锦囊牌，然后双方依次将半数手牌（向上取整）当对方声明的牌使用。
```js
sxrmbingqu: {
		audio: 2,
		trigger: { player: "phaseZhunbeiBegin" },
...
### sxrmfanxin 名字:燔心
描述: `①游戏开始或你以此法选择的角色死亡后，你可以选择一名其他角色，称为“燔心”角色。其获得${get.poptip("baonu")}和${get.poptip("wumou")}。②“燔心”角色回合开始时，你可以移去其至多5枚“暴怒”标记并摸等量张牌。`
```js
sxrmfanxin: {
		audio: 2,
		trigger: {
...
## sxrm_jiahua 名字:嗔贾华 势力:wu

### sxrmfubei 名字:伏备
描述: 出牌阶段限一次，你可以将手牌摸至体力上限，然后将两张牌正面朝上置于牌堆顶前十张的任意位置（对其他角色可见）；每名角色回合结束时，若牌堆顶的牌正面朝上，你须对其造成1点伤害。
```js
sxrmfubei: {
		audio: 2,
		enable: "phaseUse",
...
### sxrmdancui 名字:殚瘁
描述: `${get.poptip("sxrm_qidingSkill")}。当你造成伤害时，你可以弃置两张牌（无牌则不弃，不足则全弃），令此伤害+1。`
```js
sxrmdancui: {
		audio: 2,
		locked(skill, player) {
...
## sxrm_zhaoyun 名字:嗔赵云 势力:shu

### sxrmzhaduo 名字:诈夺
描述: 结束阶段，你可以获得两名其他角色各一张牌，视为对其中一名角色使用一张【杀】，然后另一名角色视为对你使用一张【决斗】；在和其中一名角色的结算过程中，你视为拥有另一名角色的所有技能。
```js
sxrmzhaduo: {
		audio: 2,
		trigger: { player: "phaseJieshuBegin" },
...
## sxrm_caoren 名字:嗔曹仁 势力:wei

### sxrmyangbei 名字:佯北
描述: 准备阶段或结束阶段，你可以翻面并摸三张牌。当你使用【杀】或普通锦囊牌指定唯一目标后，其可以弃置所有手牌令此技能本回合失效。
```js
sxrmyangbei: {
		audio: 2,
...
### sxrmyinfeng 名字:阴锋
描述: 锁定技，当你每回合首次造成伤害后，你令受伤角色获得以下效果直到你下回合开始：体力上限减至其体力值且其造成和受到的伤害均视为失去体力。
```js
sxrmyinfeng: {
		audio: 2,
...
## sxrm_guanyu 名字:魔关羽 势力:devil

### sxrmhanguo 名字:撼国
描述: `每轮开始时，你可以选择一名上轮未以此法选择过的其他角色，扣置其所有牌直到本轮结束，且本轮内：你对其使用【杀】造成伤害后，其死亡；其可以发动无势力限制的${get.poptip("hujia")}且响应其的角色获得你一张牌。`
```js
sxrmhanguo: {
		audio: 2,
		trigger: {
...
### sxrmweiwo 名字:唯我
描述: `限定技，结束阶段，你可以令至多三名其他角色各获得${get.poptip("rende")}${get.poptip("qingnang")}${get.poptip("longyin")}中的一个不同技能（仅能对你发动），然后你获得${get.poptip("old_wushen")}。`
```js
sxrmweiwo: {
		audio: 2,
		trigger: {
...
## sxrm_guanyinping 名字:慢关银屏 势力:shu

### sxrmyinmou 名字:姻谋
描述: `一名男性角色的结束阶段，其可以连接你与其各一张未被${get.poptip("sxrm_connect")}的手牌。你每回合首次失去${get.poptip("sxrm_connect")}牌后，本次同时失去${get.poptip("sxrm_connect")}牌的角色依次摸X张牌（X为这些角色数且至多为5）。`
```js
sxrmyinmou: {
		audio: 2,
		trigger: {
...
### sxrmquchi 名字:驱斥
描述: `出牌阶段限一次，你可以对一名角色造成1点火焰伤害，若其有${get.poptip("sxrm_connect")}牌，重置这些牌并令此伤害+1。`
```js
sxrmquchi: {
		audio: 2,
		enable: "phaseUse",
...
## sxrm_yujin 名字:慢于禁 势力:shu

### sxrmsuwu 名字:肃伍
描述: `准备阶段，你可以${get.poptip("sxrm_connect")}至多四名角色各一张手牌；若你有${get.poptip("sxrm_connect")}牌，有${get.poptip("sxrm_connect")}牌的角色每回合使用的首张伤害牌不可被响应且结算后其摸两张牌。`
```js
sxrmsuwu: {
		audio: 2,
		trigger: {
...
### sxrmrenwang 名字:仁王
描述: `每回合限一次，你可以将一张${get.poptip("sxrm_connect")}牌当作【桃】使用。`
```js
sxrmrenwang: {
		audio: 2,
		enable: "chooseToUse",
...
## sxrm_mifang 名字:慢糜芳 势力:shu

### sxrmhuoe 名字:火厄
描述: 结束阶段，你可以视为对至多四名其他角色使用一张【火攻】，若你有可弃置的牌，须弃置之并取消剩余目标，否则其观看你的手牌。结算结束后你分配因此展示的牌，若点数之和小于13，你失去1点体力。
```js
sxrmhuoe: {
		audio: 2,
		trigger: {
...
## sxrm_liufeng 名字:慢寇封 势力:shu

### sxrmhuaibing 名字:怀兵
描述: 准备阶段，你可选择两名有牌的角色，获得这些角色各一张手牌（为你则跳过），然后你展示手牌，令其中体力值较少的角色下个摸牌阶段摸牌数、出牌阶段出【杀】次数、弃牌阶段手牌上限改为其中红色牌的数量。
```js
sxrmhuaibing: {
		audio: 2,
		trigger: {
...
## sxrm_luxun 名字:慢陆逊 势力:wu

### sxrmchanyu 名字:谄谀
描述: 出牌阶段限一次，你可以令一名其他角色摸其体力值张牌并与其用各自点数最小的牌拼点，然后双方展示手牌：赢的角色可以交换双方一种颜色或类别的所有手牌。
```js
sxrmchanyu: {
		audio: 2,
		enable: "phaseUse",
...
### sxrmcongfeng 名字:从风
描述: 转换技，你使用牌或成为牌的目标后，你可以阳：与使用者各摸一张牌；阴：弃置使用者两张牌。
```js
sxrmcongfeng: {
		audio: 2,
		mark: true,
...
## sxrm_lvmeng 名字:慢吕蒙 势力:wu

### sxrmkongzhi 名字:空志
描述: 锁定技，你使用【杀】指定唯一目标后，获得其至多三张牌；若你已受伤，普通锦囊牌对你无效且你的基本牌均视为【闪】。
```js
sxrmkongzhi: {
		audio: 2,
		trigger: {
...
### sxrmbizha 名字:鄙诈
描述: 结束阶段，你可以摸一张牌并拼点：若其没赢，其失去2点体力并令你观看其手牌，你可使用其中至多两张点数小于其拼点牌的牌；若你没赢，你减少1点体力上限。
```js
sxrmbizha: {
		audio: 2,
		trigger: {
...
## sxrm_pangde 名字:慢庞德 势力:wei

### sxrmnuozhan 名字:搦战
描述: 准备阶段，你可摸一张牌并令一名其他角色声明一个伤害类牌的牌名，然后你令你或其视为对对方使用此牌且伤害+1；若此牌未对目标造成伤害，使用者失去1点体力且你可以对相同角色重复此流程。
```js
sxrmnuozhan: {
		audio: 2,
		trigger: {
...
## sxrm_yanwen 名字:慢颜良文丑 势力:qun

### sxrmhaibian 名字:骇变
描述: 一名角色的回合开始时，若你上回合使用过手牌，则上回合最后一张黑色牌和红色牌的使用者可以依次视为使用一张【决斗】。
```js
sxrmhaibian: {
		audio: 2,
		trigger: {
...
### sxrmqiewang 名字:怯亡
描述: 锁定技，你距离1以内的角色受到伤害后，你摸一张牌，令你的手牌本回合均视为【无懈可击】。
```js
sxrmqiewang: {
		audio: 2,
		trigger: {
...
## sxrm_caocao 名字:魔曹操 势力:devil

### sxrmkuxin 名字:枯心
描述: 你受到伤害后，可以令所有其他角色依次选择展示任意张手牌，然后你获得所有角色展示的牌或一名角色未展示的手牌并展示之，若你未以此法获得♥牌，你弃置这些牌并翻面。
```js
sxrmkuxin: {
		audio: 2,
...
## sxrm_caopi 名字:疑曹丕 势力:wei

### sxrmzhengsi 名字:争嗣
描述: 出牌阶段，你可以选择包括你在内的三名有手牌的角色，令其中一名角色先展示一张手牌，然后其余角色同时展示一张手牌。以此法展示牌点数最大的角色弃置两张手牌，以此法展示牌点数最小的角色失去1点体力。
```js
sxrmzhengsi: {
		enable: "phaseUse",
		filterTarget(card, player, target) {
...
### sxrmchengming 名字:承命
描述: 出牌阶段结束时，若你此阶段发动过〖争嗣〗，且你在此阶段成为过〖争嗣〗目标的角色中：手牌数最大，你可以回复2点体力；体力值最大，你可以获得这些角色各一张牌。
```js
sxrmchengming: {
		trigger: {
			player: "phaseUseEnd",
...
## sxrm_wanghou 名字:疑王垕 势力:wei

### sxrmjugu 名字:聚谷
描述: 准备阶段，你可以依次选择任意名角色至多共计五张牌，将这些牌正面向上置于牌堆顶；此回合结束时，这些角色获得牌堆顶各自以此法放置的牌（没有则跳过），然后各摸一张牌。
```js
sxrmjugu: {
		trigger: {
			player: "phaseZhunbeiBegin",
...
## sxrm_liubei 名字:疑刘备 势力:qun

### sxrmchengbian 名字:乘变
描述: `准备阶段和结束阶段，你可以与一名角色进行${get.poptip("sxrm_compare")}，然后视为对其使用一张【决斗】，你与其可以将至少一半的手牌当【杀】打出直到此【决斗】结算完成。此【决斗】结算结束后，揭示拼点结果，赢的角色摸至体力上限。`
```js
sxrmchengbian: {
		audio: 2,
		trigger: {
...
## sxrm_jianggan 名字:疑蒋干 势力:wei

### sxrmzongheng 名字:纵横
描述: 准备阶段，你可以观看两名其他角色的手牌，展示并获得其中一张，然后你弃置另一名角色与你此法展示牌花色、点数、类型相同的手牌各一张。
```js
sxrmzongheng: {
		trigger: {
			player: "phaseZhunbeiBegin",
...
### sxrmduibian 名字:对辩
描述: `你每回合首次受到伤害时，你可以与伤害来源${get.poptip("sxrm_compare")}并防止此伤害，然后其可以令你弃置其一张牌并揭示拼点结果，若其赢则你失去1点体力。`
```js
sxrmduibian: {
		trigger: {
			player: "damageBegin4",
...
## sxrm_huatuo 名字:疑华佗 势力:qun

### sxrmmiehai 名字:灭害
描述: 你可以将两张牌当作无距离次数限制的刺【杀】使用。此【杀】结算完成后，此过程中正面失去♠牌且已受伤的角色摸两张牌并回复1点体力。
```js
sxrmmiehai: {
		audio: 2,
		enable: "chooseToUse",
...
## sxrm_lvboshe 名字:疑吕伯奢 势力:qun

### sxrmqingjun 名字:请君
描述: `每轮结束时，你可以选择一名其他角色，令你和攻击范围含有其的其他角色各摸两张牌并发动${get.poptip("sxrmshefu")}；若如此做，其执行一个额外回合，此回合结束时，以此法发动〖设伏〗的角色移去“伏兵”，然后其中此回合未受伤的角色各视为对其使用一张【杀】。`
```js
sxrmqingjun: {
		audio: 2,
		trigger: {
...
## sxrm_fuhuanghou 名字:疑伏寿 势力:qun

### sxrmmitu 名字:密图
描述: 准备阶段，你可以令至多三名已受伤角色各摸一张牌并展示之，然后令这些角色依次选择是否与你指定的另一名角色拼点，拼点赢的角色视为对没赢的角色使用一张【杀】。每有一名角色未用以此法展示的牌进行拼点，你减少1点体力上限。
```js
sxrmmitu: {
		audio: 2,
		trigger: {
...
### sxrmqianliu 名字:潜流
描述: 与你距离1以内的角色成为【杀】的目标后，你可以观看牌堆底四张牌并以任意顺序置于牌堆顶或牌堆底；若这些牌花色均不相同，你可以展示并获得这些牌。
```js
sxrmqianliu: {
		audio: 2,
		trigger: {
...
## sxrm_xunyu 名字:疑荀彧 势力:wei

### sxrmhuice 名字:迴策
描述: 出牌阶段限一次，你可以依次与两名角色拼点，于一次拼点中赢的角色对另一次拼点中没赢的角色各造成1点伤害。
```js
sxrmhuice: {
		enable: "phaseUse",
		usable: 1,
...
### sxrmyihe 名字:异合
描述: 锁定技，你的回合内各限一次，一名角色受到伤害时，若其与伤害来源的手牌数与体力值大小关系：相同，其与伤害来源各摸两张牌；不同，此伤害+1。
```js
sxrmyihe: {
		trigger: {
			global: "damageBegin1",
...
### sxrmjizhi 名字:赍志
描述: 锁定技，其他角色不能对你使用【桃】；你每回合首次进入濒死时，回复1点体力。
```js
sxrmjizhi: {
		trigger: {
			player: "dying",
