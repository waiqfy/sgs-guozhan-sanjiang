	...
	huoe: {
		...
		subSkill: {
			effect: {
				...
				charlotte: true,
				// 上一版把这里的getParent()改成了getParent(2)，是我理解错了事件嵌套层级——
				// 用户确认这个技能本来就该跟官方sxrmhuoe完全一样(只是多了个锁定技)，官方原版
				// filter和这里的else分支用的都是getParent()(1层)，退回原样
				filter(event, player) {
...
// ========== chengpu 程普 ==========
	...
	// 之前误删了locked:true——这是真实存在的引擎属性(get/is.js的get.is.locked()会读取
	// info.locked，用来给技能面板补上"锁定技"标签)，不是瞎编的。这个技能本身既没有
	// trigger+forced、也没有mod，不会被get.is.locked()的自动判定逻辑识别成锁定技，
	// 需要显式locked:true才能正确显示"锁定技"标签，加回来。
