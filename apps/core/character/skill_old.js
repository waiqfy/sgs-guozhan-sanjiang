...
// "大势力"通用判定：己方势力人数不小于场上任何一个势力的人数，或己方本回合被刘宏"朝争"
// (lh_chaozheng)赋予"视为大势力"效果。蒋琬"讲略"(jianglve)、马腾"雄异"(xiongyi)、
// 刘宏"朝争"三处共用，避免各写各的导致"视为大势力"标记形同虚设。
function isMajorGroup(player) {
	if (player.storage.lh_chaozheng_big) {
		return true;
	}
	const groupCount = {};
	...
	return (groupCount[player.group] || 0) >= maxCount;
}
...
// ============ 徐庶 xin_xushu ============
	...
	// 参考: skill_refer/sp/skill.js 的 shoucheng技能（简化了官方版的全局技能实现，核心判定
	// 逻辑一致：其他角色于回合外因失去手牌而变为0张手牌时触发）
	shoucheng: {
		aiShowTag: "draw",
		audio: 2,
		trigger: { global: ["loseAfter", "loseAsyncAfter"] },
	...
	// 存疑未改：qianxi_ignore目前只有ai.unequip这个AI提示，没有找到能真正让
	// "使用牌无视目标某颜色防具"生效的mod钩子，不确定这个引擎版本要怎么实现这个效果，
	// 该效果目前可能只影响AI判断、不影响实际结算，未擅自改动。
	qianxi: {
		aiShowTag: "draw",
		aiShowCost: true,
		audio: 2,
		...
		ai: {
			...
			skillTagFilter(player, tag, arg) {
				...
				if (arg.card.name == "sha") {
					return (
						arg.target.storage.qianxi_lockcolor == "red" &&
						(!arg.target.hasSkillTag("freeShan", false, { player: player, card: arg.card, type: "use" }, true) ||
							player.hasSkillTag("unequip", false, { name: arg.card ? arg.card.name : null, target: arg.target, card: arg.card }) ||
							player.hasSkillTag("unequip_ai", false, { name: arg.card ? arg.card.name : null, target: arg.target, card: arg.card }))
					);
				}
	...
	// 参考: skill_refer/sp/skill.js 的 cunsi技能（核心思路一致：移除武将/失去手牌来换取让一名
	// 角色获得"勇决"，具体代价和目标限制按卡面描述做了改动，未强制要求目标为男性）
	cunsi: {
		aiShowTag: "support",
		audio: 2,
	...
	// 参考: skill_refer/yijiang/skill.js 的 zhiman技能（在官方版基础上按卡面加了"若同势力
	// 可变更副将"的效果）
	zhiman: {
		aiShowTag: "support",
		audio: "zhiman",
	...
	jianglve: {
		...
		async content(event, trigger, player) {
			...
			const executed = [];
			if (junling) {
				const candidates = game.filterPlayer(target => target != player && (target.isFriendOf(player) || target.isUnseen()));
				for (const current of candidates) {
					if (!current.isIn()) {
						continue;
					}
					if (current.isUnseen()) {
						const revealResult = await current
							.chooseBool(`将略：是否明置武将牌以确认势力？若与${get.translation(player)}势力相同，可执行此“军令”`)
							.set("ai", () => current.group == player.group)
							.forResult();
						if (!revealResult.bool) {
							continue;
						}
						await current.showCharacter(0);
						if (!current.isIn() || current.group != player.group) {
							continue;
						}
					}
			...
			if (isMajorGroup(player)) {
				return;
			}
			const changeResult = await player.chooseBool(get.prompt("jianglve"), "你不为大势力，是否变更此武将牌？").forResult();
	...
	// 获得"先驱"标记；持有该标记的同势力角色回合开始时获得额外出牌阶段）。官方"先驱"
	// (xianqu_mark)是国战里多个武将共享的通用标记，还能被弃置换手牌摸至4张+看一张武将牌，
	// 本地未接入这套共享标记系统、改用独立的player.storage.xianqu，对本技能效果没有影响，
	// 但注意本地版本额外加了"自己回合开始也能触发"这一条，官方版未见明确写出这部分。
...
// ===================== 关平：龙吟 =====================
	...
	huaibing: {
		...
		async content(event, trigger, player) {
			...
			// 卡面为“展示手牌，其中红色牌的数量”，X取自己整个手牌的红色牌数，而非获得的两张牌
			const x = player.countCards("h", { color: "red" });
	...
	// 修复：卡面"副将易位"之前被误实现成game.swapSeat()整体交换座位，跟"副将"完全不是一回事，
	// 现改用已确认可用的player.changeCharacter()真正交换双方的副将(name2)。
	...
	zhuanzheng: {
		...
		async content(event, trigger, player) {
			...
			await player.draw(x);
			const result = await target.chooseBool(`转征：是否与${get.translation(player)}交换副将？`).forResult();
			if (result.bool && target.isIn() && player.isIn()) {
				const playerVice = player.name2;
				const targetVice = target.name2;
				await player.changeCharacter([player.name1, targetVice].filter(Boolean));
				await target.changeCharacter([target.name1, playerVice].filter(Boolean));
			}
		},
		ai: {
			order: 4,
	...
	// 待确认: 官方 qinwang 的 check/ai 逻辑与本地重新设计的技能结构不匹配（依赖官方专属的标记技能/主公技机制），未照抄，如需AI优化需重新设计。
	qinwang: {
		aiShowTag: "draw",
		aiShowCost: true,
		enable: "phaseUse",
...
// ===================== 糜竺：资援/巨贾 =====================
	...
	jugu_change: {
		...
		async cost(event, trigger, player) {
			event.result = await player.chooseBool(get.prompt("jugu"), "是否变更此武将牌（主将）？").forResult();
		},
		async content(event, trigger, player) {
			if (!_status.characterlist) {
				game.initCharacterList();
...
// ============ 郝昭（haozhao） ============
	...
	jushou_mark: {
		...
		intro: {
			content() {
				return get.player().isTurnedOver() ? "叠置状态" : "平置状态";
			},
		},
	},
...
// ============ 荀彧（xunyu） ============
	...
	// 自动生成的备用viewAs技能）中调用变更副将逻辑的写法，但将官方国战专属的
	// player.mayChangeVice替换为引擎通用API player.changeCharacter（用法同郝昭
	// xishe_change）。
	...
	qice_change: {
		...
		silent: true,
		async cost(event, trigger, player) {
			event.result = await player.chooseBool(get.prompt("qice"), "是否变更副将？").forResult();
		},
		async content(event, trigger, player) {
			if (!_status.characterlist) {
				game.initCharacterList();
	...
	// ============ 崔琰&毛玠（cuimao） ============
	// 参考: skill_refer/xianding/skill.js 的 zhengbi（沿用其id及"出牌阶段开始"触发点，选项1由国战"未定势力"身份机制改写为通用的"无距离次数限制"效果，选项2同效）
...
// ============ 程昱（chengyu） ============
	...
	// 经核实，主副将变更机制（player.changeCharacter）在本项目中真实可用，此前"引擎无
	// '变更武将牌'对应机制"的假设有误，现补回"变更此武将牌"效果：用法参考xunyou的
	// qice_change，通过group+subSkill在useCardAfter（event.skill=="caishi"）中调用。
	// 才识：限定技，你可以变更此武将牌，视为使用一张【无懈可击】。
	caishi: {
		aiShowTag: "support",
		audio: 2,
		limited: true,
			...
			if (get.suit(card, target) == suit) {
				...
				if (others) {
					const giveResult = await player
						.chooseCard("h", true, c => get.suit(c, player) != suit, `盗书：交给${get.translation(target)}一张其他花色的手牌`)
						.forResult();
					if (giveResult.bool && giveResult.cards && giveResult.cards.length) {
						await player.give(giveResult.cards, target);
					}
	...
	dcmingfa_effect: {
		...
		async content(event, trigger, player) {
			...
			if (player.countCards("h") < source.countCards("h")) {
				// 卡面为“你对其造成1点伤害”，你=source，其=player，伤害承受方应是player而非source
				await player.damage(1, source);
...
// ============ 丁奉（dingfeng） ============
	...
	// 度势：出牌阶段，你可以将一张红色手牌当【以逸待劳】使用（真实卡牌使用流程，可被无懈可击等干扰）。
	// 当你因【以逸待劳】的效果失去最后的手牌时，摸一张牌，失去"度势"，获得"连营"。
	// 参考: skill_refer_guozhan/_merged_guozhan.md 的 gz_mb_duoshi 的 yiyi 子技能（viewAs结构）
	lxdushi: {
		aiShowTag: "draw",
		audio: 2,
		enable: "phaseUse",
	...
	lxdushi_effect: {
		...
		filter(event, trigger, player) {
			return trigger.skill == "lxdushi" && !player.countCards("h");
		},
		forced: true,
		popup: false,
		async content(event, trigger, player) {
			...
			player.removeSkill("lxdushi_effect");
		...
		async content(event, trigger, player) {
			...
			if (control == 0 && target.countCards("h")) {
				...
				if (options.length == 1) {
					...
					const result = await player.chooseControl(options.concat("cancel2")).set("prompt", "尚义：你可以选择明置" + get.translation(target) + "的一张暗置的武将牌").forResult();
					if (result.control == "cancel2") {
						return;
					}
...
// ============ 诸葛瑾（zhugejin） ============
		...
		subSkill: {
			...
			draw: {
				...
				filter(event, player) {
					return (
						!!event.getParent() &&
						["jiahe"].includes(event.getParent().name) &&
						game.hasPlayer(current => current != player && current.isFriendOf(player))
					);
				},
				async content(event, trigger, player) {
					const result = await player
	...
	ganlu: {
		...
		async content(event, trigger, player) {
			...
			// 卡面："若这两名角色均与你势力相同，则本回合结束时，你可以再次发动此技能"
			if (targets[0].group == player.group && targets[1].group == player.group) {
				player.addTempSkill("ganlu_extra", "phaseJieshuBegin");
			}
		},
	},
	// 复制上面ganlu自身的filterTarget/content作为再次发动的实现
	ganlu_extra: {
		charlotte: true,
		sourceSkill: "ganlu",
		trigger: { player: "phaseJieshuBegin" },
		filter(event, player) {
			return game.countPlayer(target => !target.isMin()) >= 2;
		},
		async cost(event, trigger, player) {
			event.result = await player
				.chooseTarget({
					prompt: get.prompt("ganlu"),
					prompt2: "甘露：是否再次选择两名装备区里的牌数之差不大于你已损失体力值的角色，交换其装备区里的所有牌？",
					selectTarget: 2,
					filterTarget(card, player, target) {
						if (target.isMin()) {
							return false;
	...
	kuangbi_borrow: {
		...
		filter(event, player) {
			...
			return game.hasPlayer(
				cur =>
					cur.isFriendOf(player) &&
					cur.hasSkill("kuangbi") &&
					_status.currentPhase != cur &&
					cur.getExpansions("kuangbi").some(card => event.filterCard(card, player, event))
			);
		},
		async content(event, trigger, player) {
			const evt = event.getParent(2);
...
// ============ 步骘（buzhi） ============
	...
	recanshi: {
		...
		async content(event, trigger, player) {
			// 归命卡面为“同势力角色视为已受伤”，是与受伤角色取并集，而非全场角色数
			const num = player.hasSkill("guiming") ? game.countPlayer(current => current.isDamaged() || current.group == player.group) : game.countPlayer(current => current.isDamaged());
			trigger.num += num;
...
// ============ 许贡（xugong） ============
	...
	// “队列”沿用引擎已有的 player.inline() 判定（见本文件 siege/inline 用法）；
	// 需保护的是队列中其他角色的装备，故仿照 xinanguo/xinanguo_share 的写法注册为全局技能。
	fengyang: {
		aiShowTag: "defense",
		audio: 2,
		locked: true,
	...
	// 断肠：锁定技，当你死亡时，你令杀死你的角色失去其一张武将牌上的所有技能。
	// 参考: skill_refer_guozhan/_merged_guozhan.md 的 gz_duanchang，照抄其"主将/副将二选一，
	// 明置该武将牌并移除该武将牌技能表里的技能"结构（本项目无"士兵"机制，
	// 用target.name2是否存在代替原版的hasViceCharacter判断）
	duanchang: {
		aiShowTag: "control",
		audio: 2,
		trigger: { player: "die" },
		...
		async content(event, trigger, player) {
			...
			if (target.name2) {
				const result = await player
					.chooseControl("主将", "副将")
					.set("prompt", "断肠：令" + get.translation(target) + "失去一张武将牌上的所有技能")
					.forResult();
				control = result.control;
			}
			let skills;
			...
			const removeList = skills.filter(skillId => target.hasSkill(skillId, null, null, false) && !(lib.skill[skillId] && lib.skill[skillId].charlotte));
			if (removeList.length) {
				target.removeSkill(removeList);
			}
		},
	},
	...
	chuanxin: {
		aiShowTag: "defense",
		audio: 2,
		// 卡面为“你使用杀/决斗对目标造成伤害时”，你=技能持有者应为伤害来源，须用source scope
		// （原写法{player:"damageBegin1"}要求event.player===player，与filter里的
		// event.player==player排除条件自相矛盾，永远无法触发）
		trigger: { source: "damageBegin1" },
		filter(event, player) {
			if (!player.isPhaseUsing()) {
				return false;
	...
	zfengshi: {
		...
		async content(event, trigger, player) {
			...
			const choice = await player
				.chooseControl(["弃置装备区里的一张牌", "本回合锁定技失效"])
				.set("prompt", get.prompt2("zfengshi"))
				.forResult();
	...
	jsrgzhaobing: {
		...
		async content(event, trigger, player) {
			...
			const targetResult = await player
				.chooseTarget([1, Math.min(num, others.length)], get.prompt2("jsrgzhaobing"), (card, plyr, tgt) => tgt != plyr)
				.forResult();
	...
	jili_virtual: {
		...
		filter(event, player) {
			return (
				event.jili &&
				event.jili.includes(player) &&
				!event.addedTarget &&
				event.player &&
				event.player.isAlive() &&
				event.player.canUse({ name: event.card.name, nature: event.card.nature, isCard: true }, player)
			);
		},
		async content(event, trigger, player) {
	...
	anyong: {
		...
		async cost(event, trigger, player) {
			event.result = await player
				.chooseBool("暗涌：是否令" + get.translation(trigger.source) + "对" + get.translation(trigger.player) + "造成的伤害翻倍？")
				.forResult();
		},
		async content(event, trigger, player) {
			player.storage.anyong_used_round = game.roundNumber;
	...
	xinfu_xionghuo_punish: {
		charlotte: true,
		// 卡面为"其出牌阶段开始时"，应为phaseUseBegin而非准备阶段phaseZhunbeiBegin
		trigger: { player: "phaseUseBegin" },
		forced: true,
		filter(event, player) {
	...
	xishe: {
		...
		async content(event, trigger, player) {
			...
			while (target?.isIn() && player.countCards("e") > 0) {
				...
				// 卡面为“视为对其使用一张杀，若其体力值小于你，此杀不能被响应”，须是真实的杀使用而非裸伤害
				const useEvent = player.useCard({ name: "sha", isCard: true }, target, false);
				if (target.hp < player.hp) {
					useEvent.directHit.add(target);
				}
				await useEvent;
			...
			if (killed) {
				// 传裸字符串会展开成{global:"phaseAfter"}，导致被别人的phaseAfter提前清空；
				// 须显式限定为自己的phaseAfter
				player.addTempSkill("xishe_change", { player: "phaseAfter" });
			}
		},
	},
	xishe_change: {
	...
	// 修复: 卡面为锁定技的硬性规则，此前只有ai.norespond提示不构成真实限制，真人玩家仍可响应，
	// 现参照本文件邹氏"祸水"(rehuoshui/rehuoshui_block)已验证可用的global mod写法补上硬规则。
	jnf_liedu: {
		aiShowTag: "control",
		audio: 2,
		locked: true,
		global: "jnf_liedu_block",
		ai: {
			norespond: true,
			skillTagFilter(player, tag, arg) {
				if (tag !== "norespond") {
	...
	jnf_liedu_block: {
		...
		mod: {
			cardRespondable(card, player) {
				...
				const source = evt.getParent && evt.getParent().player;
				if (!source || source == player || !source.isIn() || !source.hasSkill("jnf_liedu")) {
					return;
				}
				const maxCount = Math.max(0, ...game.filterPlayer().map(p => p.countCards("h")));
				const isMaxHand = maxCount > 0 && player.countCards("h") === maxCount;
				if (player.sex !== "female" && !isMaxHand) {
					return;
				}
				return false;
			},
