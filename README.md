...
4. 按出牌顺序的**反序**依次选：本局最后行动的玩家先选，`playerFirst`（第一个行动的玩家）最后选——弥补"先手要先亮出攻击目标"在这个环节里的信息劣势。AI 挑选时会尽量选强的，所以越先选的人越可能拿到公共池里最强的那个。
5. 选完剩 1 个没人要，直接弃置。
...
6. **武将强度分档（`guozhanRank`）**——`apps/core/mode/guozhan/src/info/rank.js` 按 1～8 分了 8 档，**数字越大代表这名武将越强**，`get.guozhanRank(name)`（`apps/core/mode/guozhan/src/patch/get.js`）查不到的武将会默认返回 `0`，也就是比最弱的 1 档还弱。这套分档不影响单个技能的价值判断，只用在几处"选哪个武将/选哪个身份"的决策上：
   - 三将规则里，"往公共池里贡献一个武将"选**最弱**的（宁愿把强将留给自己选），"从公共池挑第三个武将"选**最强**的——见 [`content.js`](apps/core/mode/guozhan/src/patch/content.js) 的 `chooseThirdCharacter`。
   - "怀异"一类会移除/惩罚武将的技能，用它来判断该动哪个目标（一般是挑对面最强的下手）——`apps/core/mode/guozhan/src/skill/character/rest.js`。
   - 目前只覆盖了 232 名武将里的 124 名，剩下 108 名没有分档、统一按 `0` 处理，相当于"比最弱还弱"；想让某个武将在这些决策里表现更准确，去 `rank.js` 里补一档就行。
